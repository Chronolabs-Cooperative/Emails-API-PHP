...
class acl extends chronomail_plugin
{
    ...
    {
        $action = trim(chronomail_utils::get_input_value('_act', chronomail_utils::INPUT_GPC));
    ...
    function acl_autocomplete()
    {
        ...
        $search = chronomail_utils::get_input_value('_search', chronomail_utils::INPUT_GPC, true);
        $reqid  = chronomail_utils::get_input_value('_reqid', chronomail_utils::INPUT_GPC);
        $users  = array();
        $keys   = array();
        ...
        if ($this->init_ldap()) {
            ...
            foreach ($result->records as $record) {
                ...
                if ($user) {
                    $display = chronomail_addressbook::compose_search_name($record);
...
*/
        ...
        $args['form']['props']['fieldsets']['info']['content']['myrights'] = array(
            'label' => chronomail::Q($this->gettext('myrights')),
        ...
        $args['form']['sharing'] = array(
            'name'    => chronomail::Q($this->gettext('sharing')),
    ...
    private function list_rights($attrib=array())
    {
        ...
        foreach ($acl as $user => $rights) {
            ...
            $userid     = chronomail_utils::html_identifier($user);
            $title      = null;
            ...
            $table->add(array('class' => 'user text-nowrap', 'title' => $title),
                html::a(array('id' => 'rcmlinkrow' . $userid), chronomail::Q($username)));
    ...
    {
        $mbox  = trim(chronomail_utils::get_input_value('_mbox', chronomail_utils::INPUT_POST, true)); // UTF7-IMAP
        $user  = trim(chronomail_utils::get_input_value('_user', chronomail_utils::INPUT_POST));
        $acl   = trim(chronomail_utils::get_input_value('_acl', chronomail_utils::INPUT_POST));
        $oldid = trim(chronomail_utils::get_input_value('_old', chronomail_utils::INPUT_POST));
        ...
        foreach ($users as $user) {
            ...
            if ($prefix && strpos($user, $prefix) === 0) {
                ...
                if (!strpos($user, '@') && ($realm = $this->get_realm())) {
                    $user .= '@' . chronomail_utils::idn_to_ascii(preg_replace('/^@/', '', $realm));
                }
                ...
                if (strpos($user, '@') && !chronomail_utils::check_email($user, false)) {
                    $user = null;
                }
            ...
            if ($user != $_SESSION['username'] && $username != $_SESSION['username']) {
                if ($this->rc->storage->set_acl($mbox, $user, $acl)) {
                    ...
                    $this->rc->output->command('acl_update', array(
                            'id'       => chronomail_utils::html_identifier($user),
                            'username' => $username,
                            'title'    => $title,
                            'display'  => $display,
                            'acl'      => implode($acl),
    ...
    private function action_delete()
    {
        ...
        foreach ($user as $u) {
            ...
            if ($this->rc->storage->delete_acl($mbox, $u)) {
                $this->rc->output->command('acl_remove_row', chronomail_utils::html_identifier($u));
            }
            else {
                $error = true;
            }
    ...
    private function action_list()
    {
        ...
        $this->mbox = trim(chronomail_utils::get_input_value('_mbox', chronomail_utils::INPUT_GPC, true)); // UTF7-IMAP
        $advanced   = trim(chronomail_utils::get_input_value('_mode', chronomail_utils::INPUT_GPC));
    ...
    function acl2text($rights)
    {
        ...
        foreach ($supported as $right) {
            if (in_array($right, $rights)) {
                $list[] = html::tag('li', null, chronomail::Q($this->gettext('acl' . $right)));
            }
        }

        if (count($list) == count($supported)) {
            return chronomail::Q($this->gettext('aclfull'));
        }
    ...
    private function init_ldap()
    {
        ...
        $this->ldap = new chronomail_ldap($config,
    ...
    protected function resolve_acl_identifier($id, &$title = null)
    {
        if ($this->init_ldap()) {
            ...
            if ($groups && $prefix && $group_field === 'name' && strpos($id, $prefix) === 0) {
                ...
                $result = $this->ldap->list_groups($gid, chronomail_addressbook::SEARCH_STRICT);
            ...
            $result = $this->ldap->search('uid', $id, chronomail_addressbook::SEARCH_STRICT);

            if ($result->count === 1 && ($record = $result->first())) {
                if ($record['uid'] === $id) {
                    $title   = chronomail_addressbook::compose_search_name($record);
                    $display = chronomail_addressbook::compose_list_name($record);
