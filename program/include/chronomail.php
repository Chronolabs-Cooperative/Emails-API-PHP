...
/**
 * Application class of chronomail Webmail
 * implemented as singleton
 *
 * @package Webmail
 */
class chronomail extends chronomail
{
    /**
     * Main tasks.
     *
     ...
     *
     * @param integer $mode Ignored chronomail::get_instance() argument
     * @param string  $env  Environment name to run (e.g. live, dev, test)
     *
     * @return chronomail The one and only instance
     */
    static function get_instance($mode = 0, $env = '')
    {
        if (!self::$instance || !is_a(self::$instance, 'chronomail')) {
            self::$instance = new chronomail($env);
    ...
    protected function startup()
    {
        ...
        // create user object
        $this->set_user(new chronomail_user($_SESSION['user_id']));

        // set task and action properties
        $this->set_task(chronomail_utils::get_input_value('_task', chronomail_utils::INPUT_GPC));
        $this->action = asciiwords(chronomail_utils::get_input_value('_action', chronomail_utils::INPUT_GPC));
     ...
     *
     * @param chronomail_user $user Current user instance
     */
     ...
     *
     * @return chronomail_contacts Address book object
     */
    public function get_address_book($id, $writeable = false)
    {
        ...
        if (isset($this->address_books[$id]) && ($this->address_books[$id] instanceof chronomail_addressbook)) {
            $contacts = $this->address_books[$id];
        }
        else if ($id && $ldap_config[$id]) {
            $domain   = $this->config->mail_domain($_SESSION['storage_host']);
            $contacts = new chronomail_ldap($ldap_config[$id], $this->config->get('ldap_debug'), $domain);
        }
        else if ($id === '0') {
            $contacts = new chronomail_contacts($this->db, $this->get_user_id());
        }
        else {
            $plugin = $this->plugins->exec_hook('addressbook_get', array('id' => $id, 'writeable' => $writeable));

            // plugin returned instance of a chronomail_addressbook
            if ($plugin['instance'] instanceof chronomail_addressbook) {
     ...
     *
     * @param chronomail_addressbook $object Addressbook source object
     *
    ...
    public function get_address_sources($writeable = false, $skip_hidden = false)
    {
        ...
        if ($abook_type === 'sql') {
            if (!isset($this->address_books['0'])) {
                $this->address_books['0'] = new chronomail_contacts($this->db, $this->get_user_id());
            }
     ...
     * This will instantiate a chronomail_output_html object and set
     * environment vars according to the current session and configuration
     *
     * @param boolean $framed True if this request is loaded in a (i)frame
     *
     * @return chronomail_output Reference to HTML output object
     */
    public function load_gui($framed = false)
    {
        // init output page
        if (!($this->output instanceof chronomail_output_html)) {
            $this->output = new chronomail_output_html($this->task, $framed);
        }
     ...
     *
     * @return chronomail_output Reference to JSON output object
     */
    public function json_init()
    {
        if (!($this->output instanceof chronomail_output_json)) {
            $this->output = new chronomail_output_json($this->task);
        }
    ...
    function login($username, $password, $host = null, $cookiecheck = false)
    {
        ...
        // host is validated in chronomail::autoselect_host(), so here
        ...
        if (!empty($username_domain)) {
            ...
            if ($domain = chronomail_utils::parse_host((string)$domain, $host)) {
                $pos = strpos($username, '@');
        ...
        if (strpos($username, '@') && ($virtuser = chronomail_user::email2user($username))) {
            $username = $virtuser;
        }

        // Here we need IDNA ASCII
        // Only chronomail_contacts class is using domain names in Unicode
        ...
        if ($user = chronomail_user::query($username, $host)) {
        ...
        // create new system user
            ...
            if ($created = chronomail_user::create($username, $host)) {
                $user = $created;
            }
            else {
                self::raise_error(array(
        ...
        if (is_object($user) && $user->ID) {
            ...
            $timezone = chronomail_utils::get_input_value('_timezone', chronomail_utils::INPUT_GPC);
    ...
    public function session_error()
    {
        ...
        $task = chronomail_utils::get_input_value('_task', chronomail_utils::INPUT_GPC);
        ...
        if ($task && !in_array($task, array('login', 'logout')) && ($sess_id = $_COOKIE[ini_get('session.name')])) {
            ...
            $cookie          = chronomail_utils::request_header('Cookie');
            $cookie_sessid   = $this->config->get('session_name') ?: 'chronomail_sessid';
            $cookie_sessauth = $this->config->get('session_auth_name') ?: 'chronomail_sessauth';
    ...
    public function autoselect_host()
    {
        ...
        if (is_array($default_host)) {
            $post_host = chronomail_utils::get_input_value('_host', chronomail_utils::INPUT_POST);
            $post_user = chronomail_utils::get_input_value('_user', chronomail_utils::INPUT_POST);
            ...
            $host = chronomail_utils::get_input_value('_host', chronomail_utils::INPUT_POST);
        }
        else {
            $host = chronomail_utils::parse_host($default_host);
        }
    ...
    /**
     * Build a valid URL to this instance of chronomail
     *
    ...
    public function url($p, $absolute = false, $full = false, $secure = false)
    {
        ...
        if ($absolute || $full) {
            // add base path to this chronomail installation
            if ($base_path == '') $base_path = '/';
            $prefix = $base_path;

            // prepend protocol://hostname:port
            if ($full) {
                $prefix = chronomail_utils::resolve_url($prefix);
            }
    ...
    public function shutdown()
    {
        ...
        foreach ($this->address_books as $book) {
            if (is_object($book) && is_a($book, 'chronomail_addressbook')) {
                $book->close();
            }
        }
     ...
     */
    public function request_security_check($mode = chronomail_utils::INPUT_POST)
    {
        // check request token
     ...
     * Old chronomail versions were using folder names with removed namespace prefix.
     * Now we need to add the prefix on servers where personal namespace has prefix.
     *
     * @param chronomail_user $user User object
     */
    ...
    public function log_login($user = null, $failed_login = false, $error_code = 0)
    {
        ...
        if ($failed_login) {
            ...
            $message = sprintf('Failed login for %s from %s in session %s (error: %d)',
                $user, chronomail_utils::remote_ip(), $session_id, $error_code);
        }
        // successful login
        else {
            ...
            $message = sprintf('Successful login for %s (ID: %d) from %s in session %s',
                $user_name, $user_id, chronomail_utils::remote_ip(), $session_id);
        }
    ...
    public function table_output($attrib, $table_data, $show_cols, $id_col)
    {
        ...
        if (!is_array($table_data)) {
            ...
            while ($table_data && ($sql_arr = $db->fetch_assoc($table_data))) {
                $table->add_row(array('id' => 'rcmrow' . chronomail_utils::html_identifier($sql_arr[$id_col])));
            ...
            foreach ($table_data as $row_data) {
                ...
                $rowid = 'rcmrow' . chronomail_utils::html_identifier($row_data[$id_col]);
    ...
    public function build_folder_tree(&$arrFolders, $folder, $delm = '/', $path = '')
    {
        ...
        if (!isset($arrFolders[$currentFolder])) {
            $arrFolders[$currentFolder] = array(
                'id'      => $path,
                'name'    => chronomail_charset::convert($currentFolder, 'UTF7-IMAP'),
                'virtual' => $virtual,
                'folders' => array()
            );
        }
    ...
    public function render_folder_tree_html(&$arrFolders, &$mbox_name, &$jslist, $attrib, $nestLevel = 0)
    {
        ...
        foreach ($arrFolders as $folder) {
            ...
            $folder_id = chronomail_utils::html_identifier($folder['id'], true);
            ...
            $link_attrib = $folder['virtual'] ? array() : array(
                ...
                'onclick' => sprintf("return %s.command('list','%s',this,event)", chronomail_output::JS_OBJECT_NAME, $js_name),
                'rel'     => $folder['id'],
                'title'   => $title,
            );
    ...
    public function localize_foldername($name, $with_path = false, $path_remove = false)
    {
            ...
            if ($count > 1) {
                for ($i = 1; $i < $count; $i++) {
                    ...
                    if ($folder_class = $this->folder_classname($folder)) {
                        ...
                        $name = chronomail_charset::convert($name, 'UTF7-IMAP');
        ...
        return chronomail_charset::convert($name, 'UTF7-IMAP');
    }
    ...
    public function localize_folderpath($path)
    {
        ...
        foreach ($path as $idx => $dir) {
            ...
            if ($protect_folders && $this->storage->is_special_folder($directory)) {
                ...
                $result[] = chronomail_charset::convert($dir, 'UTF7-IMAP');
            }
        }
    ...
    public static function quota_display($attrib)
    {
        ...
        $chronomail->output->add_gui_object('quotadisplay', $attrib['id']);

        $quota = $chronomail->quota_content($attrib);

        $chronomail->output->add_script('chronomail.set_quota('.chronomail_output::json_serialize($quota).');', 'docready');
    ...
    public function display_server_error($fallback = null, $fallback_args = null, $suffix = '', $params = array())
    {
        ...
        if ($res_code == chronomail_storage::NOPERM) {
            $error = 'errornoperm';
        }
        else if ($res_code == chronomail_storage::READONLY) {
            $error = 'errorreadonly';
        }
        else if ($res_code == chronomail_storage::OVERQUOTA) {
            ...
            // there can be e.g. "Quota exceeded" / "quotum would exceed" / "Over quota"
                ...
                $args  = array('msg' => chronomail::Q($err_str));
            }
        }
        else if ($err_code < 0) {
            $error = 'storageerror';
    ...
    public function storage_fatal_error()
    {
        ...
        case chronomail_imap_generic::ERROR_NO:
        case chronomail_imap_generic::ERROR_BAD:
        case chronomail_imap_generic::ERROR_BYE:
    ...
    public function upload_init($max_size = null)
    {
        ...
        $max_filesize = chronomail_utils::max_upload_size();
    ...
    public function upload_form($attrib, $name, $action, $input_attr = array())
    {
        ...
        $event   = chronomail_output::JS_OBJECT_NAME . ".command('$action', this.form)";
        ...
        if (chronomail_utils::get_boolean($attrib['buttons'])) {
    ...
    public function display_uploaded_file($file)
    {
        ...
        if ($file['status']) {
            ...
            if (!empty($_GET['_thumbnail'])) {
                ...
                $cache_file     = chronomail_utils::temp_filename($thumb_name, false, false);
                ...
                if (!is_file($cache_file)) {
                    ...
                    $image = new chronomail_image($filename);
     ...
     *
     * @param chronomail_message_part $part Message part
     *
     ...
     * @param int    $mode           Request mode. Default: chronomail_utils::INPUT_GPC.
     *
    ...
    public static function get_uids($uids = null, $mbox = null, &$is_multifolder = false, $mode = null)
    {
        ...
        $_uid  = $uids ?: chronomail_utils::get_input_value('_uid', $mode ?: chronomail_utils::INPUT_GPC);
        $_mbox = $mbox ?: (string) chronomail_utils::get_input_value('_mbox', $mode ?: chronomail_utils::INPUT_GPC);
    ...
    public function html2text($html, $options = array())
    {
        ...
        // Convert to text
        if (!$options['abort']) {
            $converter = new chronomail_html2text($options['body'],
