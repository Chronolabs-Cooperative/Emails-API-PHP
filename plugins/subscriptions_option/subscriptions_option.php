 ...
 *
 * Chronomail caches folder lists.  When a user changes this option or visits
 ...
 * @license GNU GPLv3+
 */
class subscriptions_option extends chronomail_plugin
{
    public $task = 'mail|settings';

    function init()
    {
        $dont_override = chronomail::get_instance()->config->get('dont_override', array());
    ...
    function settings_blocks($args)
    {
        if ($args['section'] == 'server') {
            ...
            $use_subscriptions = chronomail::get_instance()->config->get('use_subscriptions', true);
            ...
            $args['blocks']['main']['options']['use_subscriptions'] = array(
                'title' => html::label($field_id, chronomail::Q($this->gettext('useimapsubscriptions'))),
    ...
    function save_prefs($args)
    {
        if ($args['section'] == 'server') {
            ...
            $use_subscriptions = $chronomail->config->get('use_subscriptions', true);
    ...
    function mailboxes_list($args)
    {
        ...
        if (!$chronomail->config->get('use_subscriptions', true)) {
            $storage = $chronomail->get_storage();
