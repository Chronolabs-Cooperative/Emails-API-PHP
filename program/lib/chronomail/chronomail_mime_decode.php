...
class chronomail_mime_decode
{
     ...
     * @param bool   $convert Convert result to chronomail_message_part structure
     *
        ...
        if (isset($content_type)) {
            ...
            case 'text/plain':
                ...
                if ($this->params['include_bodies']) {
                    $return->body = $this->params['decode_bodies'] ? chronomail_mime::decode($body, $encoding) : $body;
                }
            ...
            case 'message/rfc822':
                $obj = new chronomail_mime_decode($this->params);
            ...
            default:
                if ($this->params['include_bodies']) {
                    $return->body = $this->params['decode_bodies'] ? chronomail_mime::decode($body, $content_transfer_encoding['value']) : $body;
                }
            ...
            if ($this->params['include_bodies']) {
                $return->body = $this->params['decode_bodies'] ? chronomail_mime::decode($body) : $body;
            }
        }
    ...
    {
        return chronomail_mime::decode_mime_string($input, $this->params['default_charset']);
    }

    /**
     * Recursive method to convert a chronomail_mime_decode structure
     * into a chronomail_message_part object.
     *
     * @param object $part   A message part struct
     * @param int    $count  Part count
     * @param string $parent Parent MIME ID
     *
     * @return object chronomail_message_part
     * @see self::decode()
     */
