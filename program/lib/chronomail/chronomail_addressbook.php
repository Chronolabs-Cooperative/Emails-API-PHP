 ...
 */
abstract class chronomail_addressbook
{
     ...
     *
     * @return object chronomail_result_set List of contact records and 'count' value
     */
     ...
     *
     * @return chronomail_result_set Result set with values for 'count' and 'first'
     */
     ...
     *
     * @return chronomail_result_set Current result set or NULL if nothing selected yet
     */
    ...
    public function validate(&$save_data, $autofix = false)
    {
        ...
        foreach ($this->get_col_values('email', $save_data, true) as $email) {
            if (strlen($email)) {
                if (!chronomail_utils::check_email(chronomail_utils::idn_to_ascii($email))) {
                    $error = $chronomail->gettext(array('name' => 'emailformaterror', 'vars' => array('email' => $email)));
        ...
        $plugin = $chronomail->plugins->exec_hook('contact_validate', array(
            'record'  => $save_data,
            'autofix' => $autofix,
            'valid'   => $valid,
        ));
     ...
     *
     * @param chronomail_result_set $recset Recordset to insert
     * @param boolean          $check  True to check for duplicates first
     *
    ...
    {
        $ids = array();
        if (is_object($recset) && is_a($recset, chronomail_result_set)) {
    ...
    {
        return chronomail_utils::normalize_string($str);
    }
    ...
    {
        $contact = chronomail::get_instance()->plugins->exec_hook('contact_displayname', $contact);
        $fn = $contact['name'];
    ...
    public static function compose_list_name($contact)
    {
        ...
        if (!isset($compose_mode)) {
            $compose_mode = chronomail::get_instance()->config->get('addressbook_name_listing', 0);
        }
        ...
        case 0:
            ...
            break;
        default:
            $plugin = chronomail::get_instance()->plugins->exec_hook('contact_listname', array('contact' => $contact));
            $fn     = $plugin['fn'];
        }
    ...
    public static function compose_search_name($contact, $email = null, $name = null, $templ = null)
    {
        ...
        if (empty($templ) && !isset($template)) {  // cache this
            $template = chronomail::get_instance()->config->get('contact_search_name');
            if (empty($template)) {
                $template = '{name} <{email}>';
            }
        }
    ...
    protected function compare_search_value($colname, $value, $search, $mode)
    {
        ...
        if (in_array($colname, $this->date_cols)) {
            return (($value = chronomail_utils::anytodatetime($value))
                && ($search = chronomail_utils::anytodatetime($search))
