...
/**
 * Interface class for accessing chronomail messages cache
 *
 * @package    Framework
 * @subpackage Storage
 */
class chronomail_imap_cache
{
    const MODE_INDEX   = 1;
    const MODE_MESSAGE = 2;

    /**
     * Instance of chronomail_imap
     *
     * @var chronomail_imap
     */
    private $imap;
     ...
     * Object constructor.
     *
     * @param chronomail_db   $db           DB handler
     * @param chronomail_imap $imap         IMAP handler
    ...
    function get_index($mailbox, $sort_field = null, $sort_order = null, $existing = false)
    {
        ...
        // and many chronomail_imap changes to connect when needed
    ...
    /**
     * Returns list of messages (headers). See chronomail_imap::fetch_headers().
     *
     * @param string $mailbox  Folder name
     * @param array  $msgs     Message UIDs
     *
     * @return array The list of messages (chronomail_message_header) indexed by UID
     */
     ...
     *
     * @return chronomail_message_header Message data
     */
     ...
     *
     * @param string               $mailbox  Folder name
     * @param chronomail_message_header $message  Message data
    ...
    private function get_index_row($mailbox)
    {
        ...
        if ($sql_arr = $this->db->fetch_assoc($sql_result)) {
            ...
            if (empty($index)) {
                $index = new chronomail_result_index($mailbox);
            }
    ...
    private function get_thread_row($mailbox)
    {
        ...
        if ($sql_arr = $this->db->fetch_assoc($sql_result)) {
            ...
            if (empty($thread)) {
                $thread = new chronomail_result_thread($mailbox);
            }
    ...
    {
        $object    = $index['object'];
        $is_thread = is_a($object, 'chronomail_result_thread');
        ...
        if (!empty($this->skip_deleted)) {
            ...
            if (!empty($mbox_data['UNDELETED'])) {
                ...
                $ids = $this->imap->search_once($mailbox, 'ALL UNDELETED NOT UID '.
                    chronomail_imap_generic::compressMessageSet($object->get()));
    ...
    function synchronize($mailbox)
    {
        ...
        if (!empty($uids)) {
            ...
            if ($qresync) {
                ...
                $uids = chronomail_imap_generic::uncompressMessageSet($mbox_data['VANISHED']);
                if (!empty($uids)) {
     ...
     *
     * @return chronomail_message_header Message object
     */
     ...
     *
     * @param chronomail_message_header|chronomail_message_part
     */
    private function message_object_prepare(&$msg, &$size = 0)
    {
        ...
        // Another solution would be to use object's copy in chronomail_message class
    ...
    private function get_index_data($mailbox, $sort_field, $sort_order, $mbox_data = array())
    {
        ...
        if ($mbox_data['EXISTS']) {
            ...
            $index = new chronomail_result_index($mailbox, '* SORT');
        }
...
// for backward compat.
class chronomail_mail_header extends chronomail_message_header { }
