 ...
 * @subpackage Storage
 */
abstract class chronomail_storage
{
    /**
     * Instance of connection object e.g. chronomail_imap_generic
     *
     * @var mixed
     */
    public $conn;
    ...
    /**
     * All (additional) headers used (in any way) by chronomail
     * Not listed here: DATE, FROM, TO, CC, REPLY-TO, SUBJECT, CONTENT-TYPE, LIST-POST
     * (used for messages listing) are hardcoded in chronomail_imap_generic::fetchHeaders()
     *
     * @var array
     */
     ...
     * @param  chronomail_message_part $o_part Part object created by get_structure()
     ...
     * @see    chronomail_imap::get_message_part()
     */
    public function get_body($uid, $part = 1)
    {
        $headers = $this->get_message_headers($uid);
        return chronomail_charset::convert($this->get_message_part($uid, $part, null),
    ...
    protected function parse_uids($uids)
    {
        if ($uids === '*' || $uids === '1:*') {
            ...
            if (is_array($uids)) {
                ...
                $uids = implode(',', chronomail_imap_generic::uncompressMessageSet($uids));
            }
    ...
    public function create_default_folders()
    {
        ...
        foreach (self::$folder_types as $type) {
            if ($folder = $chronomail->config->get($type . '_mbox')) {
