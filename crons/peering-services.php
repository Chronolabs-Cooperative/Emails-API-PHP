...
$response = json_decode(getURIData(API_MASTERHOST_API_URL . '/v1/peers/json.api', 200, 200, array('user' => array('uname' => $user['uname'], 'name' => $user['name'], 'pass' => $user['pass'], 'email' => $user['email']), 'callback' => API_URL . '/v1/callback.api', 'company' => API_LICENSE_COMPANY, 'serial' => getNumerical(md5(API_LICENSE_KEY).sha1(API_LICENSE_KEY)), 'email' => API_LICENSE_EMAIL, 'protocol' => API_PROT, 'host' => parse_url(API_URL, PHP_URL_HOST) . (parse_url(API_URL, PHP_URL_PORT)!=''?":".parse_url(API_URL, PHP_URL_PORT):""), 'path' => parse_url(API_URL, PHP_URL_PATH), 'version' => 'v1', 'type' => API_TYPE)), true);
foreach($response['peers']['urls'] as $urls)
    @getURIData($urls . '/v1/peers/json.api', 200, 200, array('user' => array('uname' => $user['uname'], 'name' => $user['name'], 'pass' => $user['pass'], 'email' => $user['email']), 'callback' => API_URL . '/v1/callback.api', 'company' => API_LICENSE_COMPANY, 'serial' => getNumerical(md5(API_LICENSE_KEY).sha1(API_LICENSE_KEY)), 'email' => API_LICENSE_EMAIL, 'protocol' => API_PROT, 'host' => parse_url(API_URL, PHP_URL_HOST) . (parse_url(API_URL, PHP_URL_PORT)!=''?":".parse_url(API_URL, PHP_URL_PORT):""), 'path' => parse_url(API_URL, PHP_URL_PATH), 'version' => 'v1', 'type' => API_TYPE))
?>
