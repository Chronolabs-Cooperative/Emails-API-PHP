...
class chronomail_imap_search
{
    ...
    public function exec($folders, $str, $charset = null, $sort_field = null, $threading=null)
    {
        ...
        $results = new chronomail_result_multifolder($folders);
        ...
        foreach ($folders as $folder) {
            ...
            if ($result && !$result->incomplete) {
                ...
                $job = new chronomail_imap_search_job($folder, $search, $charset, $sort_field, $threading);
                $job->worker = $this;
                $this->jobs[] = $job;
            }
        }
 ...
 */
class chronomail_imap_search_job /* extends Stackable */
{
    private $folder;
    private $search;
    private $charset;
    ...
    public function __construct($folder, $str, $charset = null, $sort_field = null, $threading=false)
    {
        ...
        $this->result = new chronomail_result_index($folder);
    ...
    /**
     * Copy of chronomail_imap::search_index()
     */
    protected function search_index()
    {
        ...
        if (!$imap->connected()) {
            ...
            if ($this->threading) {
                return new chronomail_result_thread($this->folder);
            }
            else {
                return new chronomail_result_index($this->folder);
            }
        }
        ...
        if ($this->threading) {
            ...
            if ($threads->is_error() && $charset && $charset != 'US-ASCII') {
                $threads = $imap->thread($this->folder, $this->threading,
                    chronomail_imap::convert_criteria($criteria, $charset), true, 'US-ASCII');
            }
        ...
        if (!$messages || $messages->is_error()) {
            ...
            if ($messages->is_error() && $charset && $charset != 'US-ASCII') {
                $messages = $imap->search($this->folder,
                    chronomail_imap::convert_criteria($criteria, $charset), true);
            }
        }
