...
class markasjunk_dir_learn
{
    ...
    private function _do_messagemove($uids, $spam)
    {
        ...
        $dest_dir = unslashify($chronomail->config->get($spam ? 'markasjunk_spam_dir' : 'markasjunk_ham_dir'));
        ...
        $filename = $chronomail->config->get('markasjunk_filename');
        $filename = str_replace('%u', $_SESSION['username'], $filename);
        $filename = str_replace('%t', ($spam) ? 'spam' : 'ham', $filename);
        $filename = str_replace('%l', $chronomail->user->get_username('local'), $filename);
        $filename = str_replace('%d', $chronomail->user->get_username('domain'), $filename);
        ...
        foreach ($uids as $uid) {
            ...
            if ($debug) {
                chronomail::write_log('markasjunk', $tmpfname);
            }
        }
    }
}
