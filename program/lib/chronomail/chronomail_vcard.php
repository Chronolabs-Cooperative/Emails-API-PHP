...
class chronomail_vcard
{
    ...
    /**
     * Return vCard data as associative array to be unsed in chronomail address books
     *
    ...
    public function get_assoc()
    {
        ...
        // convert from raw vcard data into associative data for chronomail
    ...
    public function set($field, $value, $type = 'HOME')
    {
        ...
        case 'birthday':
        case 'anniversary':
            if (($val = chronomail_utils::anytodatetime($value)) && ($fn = self::$fieldmap[$field])) {
                ...
                if (is_array($subnode) && (($charset = $force_charset) || ($subnode['charset'] && ($charset = $subnode['charset'][0])))) {
                    foreach ($subnode as $j => $value) {
                        if (is_numeric($j) && is_string($value)) {
                            $card[$key][$i][$j] = chronomail_charset::convert($value, $charset);
                        }
                    }
     ...
     *
     * @return chronomail_vcard[] List of chronomail_vcard objects
     */
    public static function import($data)
    {
        ...
        // detect charset and convert to utf-8
            ...
            $data = chronomail_charset::convert($data, $charset);
        ...
        foreach (preg_split("/[\r\n]+/", $data) as $line) {
            ...
            if (preg_match('/^END:VCARD$/i', $line)) {
                // parse vcard
                $obj = new chronomail_vcard($vcard_block, $charset, true, self::$fieldmap);
    ...
    {
        $fallback = chronomail::get_instance()->config->get('default_charset', 'ISO-8859-1'); // fallback to Latin-1

        return chronomail_charset::detect($string, $fallback);
    }
}
