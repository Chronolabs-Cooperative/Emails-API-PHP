 ...
 *
 * index.php (just after $Chronomail->plugins->init()):
 *
 ...
 *   [17-Feb-2009 16:51:37 -0500]       sql: select * from example
 */
class debug_logger extends chronomail_plugin
{
    function init()
    {
        ...
        if(!chronomail::get_instance()->config->get('log_dir')){
            chronomail::get_instance()->config->set('log_dir',INSTALL_PATH.'logs');
        }

        $log_config = chronomail::get_instance()->config->get('debug_logger',array());

        foreach ($log_config as $type => $file){
            $this->runlog->set_file(chronomail::get_instance()->config->get('log_dir').'/'.$file, $type);
        }

        $start_string = "";
        $action = chronomail::get_instance()->action;
        $task = chronomail::get_instance()->task;
        if($action){
    ...
    function console($args){
        ...
        if(!isset($args[1])){
            ...
            // file which called console. For now only chronomail_imap/chronomail_storage is supported
            $bt = debug_backtrace();
            $file  = $bt[3]['file'];
            switch(basename($file)){
                case 'chronomail_imap.php':
                    $type = 'imap';
                    break;
                case 'chronomail_storage.php':
                    $type = 'storage';
                    break;
                default:
                    $type = FALSE; 
