...
class markasjunk_amavis_blacklist
{
    ...
    private function _do_list($uids, $spam)
    {
        ...
        $this->user_email = $chronomail->user->data['username'];

        if (is_file($chronomail->config->get('markasjunk_amacube_config')) && !$chronomail->config->load_from_file($chronomail->config->get('markasjunk_amacube_config'))) {
            chronomail::raise_error(array('code' => 527, 'type' => 'php',
                'file' => __FILE__, 'line' => __LINE__,
                'message' => "Failed to load config from " . $chronomail->config->get('markasjunk_amacube_config')
            ), true, false);

            return false;
        }

        $db = chronomail_db::factory($chronomail->config->get('amacube_db_dsn'), '', true);
        ...
        if ($sql_result && ($res_array = $db->fetch_assoc($sql_result))) {
            ...
            if ($debug) {
                chronomail::write_log('markasjunk', $this->user_email . ' not found in users table');
            }
        ...
        foreach ($uids as $uid) {
            ...
            if ($sql_result && ($res_array = $db->fetch_assoc($sql_result))) {
                ...
                if ($debug) {
                    chronomail::write_log('markasjunk', $email . ' not found in mailaddr table - add it');
                }
                ...
                if ($sql_result) {
                    ...
                    if ($debug) {
                        chronomail::write_log('markasjunk', 'Cannot add ' . $email . ' to mailaddr table: ' . $db->is_error($sql_result));
                    }
            ...
            if (!$wb || (!$spam && preg_match('/^([BbNnFf])[\s]*\z/', $wb)) || ($spam && preg_match('/^([WwYyTt])[\s]*\z/', $wb))) {
                ...
                if (!$sql_result) {
                    if ($debug) {
                        chronomail::write_log('markasjunk', 'Cannot update wblist for user ' . $this->user_email . ' with ' . $email);
                    }
