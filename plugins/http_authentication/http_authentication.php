 ...
 */
class http_authentication extends chronomail_plugin
{
    ...
    function startup($args)
    {
        if (!empty($_SERVER['PHP_AUTH_USER'])) {
            ...
            $chronomail->add_shutdown_function(array('http_authentication', 'shutdown'));
            ...
            // Set user password in session (see shutdown() method for more info)
                ...
                $_SESSION['password'] = $chronomail->encrypt($_SERVER['PHP_AUTH_PW']);
            }
        }
    ...
    function authenticate($args)
    {
        ...
        $host = chronomail::get_instance()->config->get('http_authentication_host');
        if (is_string($host) && trim($host) !== '' && empty($args['host'])) {
            $args['host'] = chronomail_utils::idn_to_ascii(chronomail_utils::parse_host($host));
        }
    ...
    function logout($args)
    {
        ...
        if (!empty($_SERVER['PHP_AUTH_USER']) && $args['user'] == $_SERVER['PHP_AUTH_USER']) {
            if ($url = chronomail::get_instance()->config->get('logout_url')) {
    ...
    function shutdown()
    {
        ...
        chronomail::get_instance()->session->remove('password');
    }
