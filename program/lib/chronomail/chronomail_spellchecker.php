...
class chronomail_spellchecker
{
    ...
    function __construct($lang = 'en')
    {
        ...
        $cls = 'chronomail_spellchecker_' . $this->engine;
        if (class_exists($cls)) {
    ...
    function languages()
    {
        ...
        foreach ((array) $langs as $lang) {
            ...
            $alias = $chronomail_language_aliases[$langc];
            if (!$alias) {
                $alias = $langc.'_'.strtoupper($langc);
            }
            if ($chronomail_languages[$lang]) {
                $languages[$lang] = $chronomail_languages[$lang];
            }
            else if ($chronomail_languages[$alias]) {
                $languages[$lang] = $chronomail_languages[$alias];
            }
            else {
    ...
    {
        $h2t = new chronomail_html2text($text, false, false, 0);
        return $h2t->get_text();
    }
