 ...
 */
class chronomail_contacts extends chronomail_addressbook
{
     ...
     * Object constructor
     *
     * @param object  $dbconn Instance of the chronomail_db class
    ...
    function list_groups($search = null, $mode = 0)
    {
        ...
        if ($search) {
            if ($mode & chronomail_addressbook::SEARCH_STRICT) {
                $sql_filter = $this->db->ilike('name', $search);
            }
            else if ($mode & chronomail_addressbook::SEARCH_PREFIX) {
                $sql_filter = $this->db->ilike('name', $search . '%');
            }
            else {
                $sql_filter = $this->db->ilike('name', '%' . $search . '%');
     ...
     * @param int     $mode     Search mode. Sum of chronomail_addressbook::SEARCH_*
     * @param boolean $select   True if results are requested, False if count only
     * @param boolean $nocount  True to skip the count query (select only)
     * @param array   $required List of fields that cannot be empty
     *
     * @return object chronomail_result_set Contact records and 'count' value
     */
    function search($fields, $value, $mode = 0, $select = true, $nocount = false, $required = array())
    {
        ...
        // fulltext search in all fields
            ...
            $words = ($mode & chronomail_addressbook::SEARCH_STRICT) ? array($value) : chronomail_utils::tokenize_string($value, 1);
        ...
        if (!empty($post_search) || !empty($required)) {
            ...
            if ($ids == '0') {
                $this->set_search_set($where);
                return ($this->result = new chronomail_result_set(0, 0));
            }
        }
    ...
    private function fulltext_sql_where($value, $mode, $col = 'words', $bool = 'AND')
    {
        ...
        $words = $col == 'words' ? chronomail_utils::normalize_string($value, true) : array($value);
     ...
     *
     * @return chronomail_result_set Result object
     */
    function count()
    {
     ...
     *
     * @return chronomail_result_set|array Result object with all record fields
     */
    ...
    private function convert_db_data($sql_arr)
    {
        ...
        if ($sql_arr['vcard']) {
            unset($sql_arr['email']);
            $vcard = new chronomail_vcard($sql_arr['vcard'], RCUBE_CHARSET, false, $this->vcard_fieldmap);
    ...
    private function convert_save_data($save_data, $record = array())
    {
        ...
        $vcard = new chronomail_vcard($record['vcard'] ?: $save_data['vcard'], RCUBE_CHARSET, false, $this->vcard_fieldmap);
        $vcard->reset();
        ...
        foreach ($save_data as $key => $values) {
            ...
            foreach ((array)$values as $value) {
                ...
                if ($fulltext && is_array($value))
                    $words .= ' ' . chronomail_utils::normalize_string(implode(' ', $value));
                else if ($fulltext && strlen($value) >= 3)
                    $words .= ' ' . chronomail_utils::normalize_string($value);
            }
        }
