...
/**
 * Configuration class for chronomail
 *
 * @package    Framework
 * @subpackage Core
 */
class chronomail_config
{
    ...
    public function __construct($env = '')
    {
        ...
        if ($paths = getenv('RCUBE_CONFIG_PATH')) {
            ...
            foreach ($this->paths as $i => $path) {
                if (!chronomail_utils::is_absolute_path($path)) {
    ...
    private function load()
    {
        ...
        foreach (array('drafts_mbox', 'junk_mbox', 'sent_mbox', 'trash_mbox') as $folder) {
            $this->prop[$folder] = chronomail_charset::convert($this->prop[$folder], RCUBE_CHARSET, 'UTF7-IMAP');
        }
    ...
    {
        $files    = array();
        $abs_path = chronomail_utils::is_absolute_path($file);
    ...
    public function get($name, $def = null)
    {
        ...
        $plugin = $chronomail->plugins->exec_hook('config_get', array(
    ...
    public function mail_domain($host, $encode=true)
    {
        ...
        if (is_array($this->prop['mail_domain'])) {
            ...
            $domain = chronomail_utils::parse_host($this->prop['mail_domain']);
        }

        if ($encode) {
            $domain = chronomail_utils::idn_to_ascii($domain);
        }
