 ...
 */
class zipdownload extends chronomail_plugin
{
    public $task = 'mail';
    ...
    public function init()
    {
        ...
        $this->load_config();
        $this->charset = $chronomail->config->get('zipdownload_charset', CHRONOMAIL_CHARSET);

        if ($chronomail->config->get('zipdownload_attachments', 1) > -1 && ($chronomail->action == 'show' || $chronomail->action == 'preview')) {
        ...
        if (!$chronomail->action && $chronomail->config->get('zipdownload_selection', $this->default_limit)) {
            $this->add_texts('localization');
            $this->download_menu();
        }
    }
    ...
    public function attachment_ziplink($p)
    {
        ...
        if (substr_count($p['content'], '<li') > $chronomail->config->get('zipdownload_attachments', 1)) {
            $href = $chronomail->url(array(
                '_action' => 'plugin.zipdownload.attachments',
                '_mbox'   => $chronomail->output->env['mailbox'],
                '_uid'    => $chronomail->output->env['uid'],
            ), false, false, true);

            $link = html::a(array('href' => $href, 'class' => 'button zipdownload'),
                chronomail::Q($this->gettext('downloadall'))
            );

            // append link to attachments list, slightly different in some skins
            switch (chronomail::get_instance()->config->get('skin')) {
                case 'classic':
    ...
    public function download_menu()
    {
        ...
        if ($chronomail->config->get('skin') != 'classic') {
            $ul_attr['class'] = 'toolbarmenu menu';
        }

        foreach (array('eml', 'mbox', 'maildir') as $type) {
            $menu[] = html::tag('li', null, $chronomail->output->button(array(
        ...
        $chronomail->output->add_footer(html::div(array('id' => 'zipdownload-menu', 'class' => 'popupmenu', 'aria-hidden' => 'true'),
    ...
    public function download_attachments()
    {
        ...
        $chronomail->request_security_check(chronomail_utils::INPUT_GET);

        $tmpfname  = chronomail_utils::temp_filename('zipdownload');
        $tempfiles = array($tmpfname);
        $message   = new chronomail_message(chronomail_utils::get_input_value('_uid', chronomail_utils::INPUT_GET));
        ...
        foreach ($message->attachments as $part) {
            ...
            $tmpfn       = chronomail_utils::temp_filename('zipattach');
            $tmpfp       = fopen($tmpfn, 'w');
            $tempfiles[] = $tmpfn;
    ...
    public function download_messages()
    {
        ...
        if ($chronomail->config->get('zipdownload_selection', $this->default_limit)) {
            $messageset = chronomail::get_uids(null, null, $multi, chronomail_utils::INPUT_POST);
            if (count($messageset)) {
    ...
    private function _create_displayname($part)
    {
        ...
        if ($filename === null || $filename === '') {
            $ext      = (array) chronomail_mime::get_mime_extensions($part->mimetype);
            $ext      = array_shift($ext);
            $filename = $chronomail->gettext('messagepart') . ' ' . $part->mime_id;
            if ($ext) {
                $filename .= '.' . $ext;
            }
        }
    ...
    private function _download_messages($messageset)
    {
        ...
        $mode      = chronomail_utils::get_input_value('_mode', chronomail_utils::INPUT_POST);
        $limit     = $chronomail->config->get('zipdownload_selection', $this->default_limit);
        ...
        foreach ($messageset as $mbox => $uids) {
            ...
            foreach ($uids as $uid) {
                ...
                if ($mode == 'mbox') {
                    // Sender address
                    $from = chronomail_mime::decode_address_list($headers->from, null, true, $headers->charset, true);
                    $from = array_shift($from);
                    $from = preg_replace('/\s/', '-', $from);

                    // Received (internal) date
                    $date = chronomail_utils::anytodatetime($headers->internaldate, $timezone);
                    if ($date) {
                    ...
                    $subject = chronomail_mime::decode_header($headers->subject, $headers->charset);
                ...
                if ($limit > 0 && $size > $limit) {
                    ...
                    $msg = $this->gettext(array(
                            'name' => 'sizelimiterror',
                            'vars' => array('$size' => $chronomail->show_bytes($limit))
                    ));

                    $chronomail->output->show_message($msg, 'error');
                    $chronomail->output->send('iframe');
                    exit;
                }
            }
        }
        ...
        foreach ($messages as $key => $value) {
            ...
            if ($mode == 'mbox') {
                ...
                $tmpfn = chronomail_utils::temp_filename('zipmessage');
                $tmpfp = fopen($tmpfn, 'w');
                $imap->get_raw_body($uid, $tmpfp);
                $tempfiles[] = $tmpfn;
                fclose($tmpfp);
    ...
    private function _deliver_zipfile($tmpfname, $filename)
    {
        ...
        $chronomail->output->download_headers($filename, array('length' => filesize($tmpfname)));
    ...
    private function _convert_filename($str)
    {
        ...
        return chronomail_charset::convert($str, CHRONOMAIL_CHARSET, $this->charset);
    }
