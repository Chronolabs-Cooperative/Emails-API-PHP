/**
 * Chronomail webmail functions for the Elastic skin
 *
...
function chronomail_elastic_ui()
{
    var prefs, ref = this,
        ...
        ios = false,
        popups_close_lock,
        is_framed = chronomail.is_framed(),
        env = {
            config: {
                standard_windows: chronomail.env.standard_windows,
                message_extwin: chronomail.env.message_extwin,
                compose_extwin: chronomail.env.compose_extwin,
                help_open_extwin: chronomail.env.help_open_extwin
            },
            checkboxes: 0,
            small_screen_config: {
    ...
    function setup()
    {
        ...
        if (is_framed && !chronomail.env.extwin && !parent.$('.ui-dialog:visible').length) {
        ...
        if (!is_framed && layout.content.length && !layout.content.is('.no-navbar')
            ...
            env.frame_nav = $('<div class="footer menu toolbar content-frame-navigation hide-nav-buttons">')
                .append($('<a class="button prev">')
                    .append($('<span class="inner"></span>').text(chronomail.gettext('previous'))))
                .append($('<span class="buttons">'))
                .append($('<a class="button next">')
                    .append($('<span class="inner"></span>').text(chronomail.gettext('next'))))
        ...
        if (form = chronomail.gui_objects.messageform) {
            form = $('form[name="' + form + '"]');
            // Show input elements with non-empty value
            // These event handlers need to be registered before chronomail 'init' event
        ...
        $('#dragmessage-menu,#dragcontact-menu').each(function() {
            chronomail.gui_object('dragmenu', this.id);
        });
        ...
        $('#taskmenu > a').each(function() {
            if (/button-([a-z]+)/.test(this.className)) {
                ...
                if (button && (data = button.data)) {
                    ...
                    chronomail.buttons[button.command][button.index] = data;
                    chronomail.init_button(button.command, data);
                }
            ...
            $(this).on('mouseover', function() { chronomail_webmail.long_subject_title(this, 0, $('span.inner', this)); });
        });
        ...
        $('.listbutton').each(function() {
            ...
            chronomail.buttons[button.command][button.index] = button.data;
            chronomail.init_button(button.command, button.data);
        });
        ...
        $('.treelist').each(function() {
            ...
            $('li.mailbox > a').on('mouseover', function() { chronomail_webmail.long_subject_title_ex(this); });
        });
    ...
    function register_cloned_button(old_id, new_id, active_class)
    {
        ...
        if (button) {
            chronomail.register_button(button.command, new_id, button.data.type, active_class, button.data.sel);
        }
    };
    ...
    function create_cloned_button(target, menu_button, add_class, always_active)
    {
        ...
        if (!menu_button) {
            ...
            popup_init(button[0]);
            click = false;
            chronomail.register_menu_button(button[0], popup);
        }
    ...
    /**
     * Finds an chronomail button
     */
    function find_button(id)
    {
        var i, button, command;

        for (command in chronomail.buttons) {
            for (i = 0; i < chronomail.buttons[command].length; i++) {
                button = chronomail.buttons[command][i];
                if (button.id == id) {
                    return {
                        command: command,
                        index: i,
    ...
    function layout_init()
    {
        ...
        // Enable chronomail.open_window intercepting
        env.open_window = chronomail.open_window;
        chronomail.open_window = window_open;
        ...
        chronomail
            ...
            .addEventListener('autocomplete_create', chronomail_popup_init)
            .addEventListener('googiespell_create', chronomail_popup_init)
        ...
        if ((layout.list.length || layout.content.length) && is_mobile()) {
            ...
            $('[data-fab]').each(function() {
                ...
                if ((task == '*' || task == chronomail.env.task)
                    && (action == '*' || action == chronomail.env.action || (action == 'none' && !chronomail.env.action))
                ) {
    ...
    function init()
    {
        ...
        $('[data-list]').filter('ul,table').each(function() {
            ...
            if (chronomail[list] && chronomail[list].multiselect) {
                var repl, button,
                ...
                chronomail[list].enable_checkbox_selection();

                // Add Select button to the list navigation bar
                if (!button) {
                    button = $('<a>').attr({'class': 'button selection disabled', role: 'button', title: chronomail.gettext('select')})
                        .on('click', function() { if ($(this).is('.active')) table.toggleClass('withselection'); })
                        .append($('<span class="inner">').text(chronomail.gettext('select')));
                ...
                chronomail.addEventListener('listupdate', function(prop) {
                    if (prop.list && prop.list == chronomail[list]) {
                        if (prop.rowcount) {
            ...
            // https://github.com/chronomail/elastic/issues/45
            // Draggable blocks scrolling on touch devices, we'll disable it there
            if (touch && chronomail[list]) {
                if (typeof chronomail[list].draggable == 'function') {
                    chronomail[list].draggable('destroy');
                }
                else if (typeof chronomail[list].draggable == 'boolean') {
                    chronomail[list].draggable = false;
                }
            }
        });
        ...
        if (window.MutationObserver) {
            $('[data-label-msg]').filter('ul,table').each(function() {
                ...
                fn = function() {
                    ...
                    if (!chronomail.env.search_request && !chronomail.env.qsearch
                        && msg && !list.children(':visible').length
                    ) {
                        ext = table.data('label-ext');
                        command = table.data('create-command');

                        if (ext && (!command || chronomail.commands[command])) {
                            msg += ' ' + ext;
                        }
                ...
                callback = function() {
                    ...
                    if (chronomail.busy || !table.is(':visible')) {
        ...
        if (chronomail.env.action != 'print') {
        ...
        var phone_confirmation = function(label) {
            if (mode == 'phone') {
                chronomail.display_message(chronomail.gettext(label), 'confirmation');
            }
        };

        chronomail.addEventListener('fileappended', function(e) {
                if (e.attachment.complete) {
                    attachmentmenu_append(e.item);
                    if (e.attachment.mimetype == 'text/vcard' && chronomail.commands['attach-vcard']) {
        ...
        chronomail.init_pagejumper('.pagenav > input');

        if (chronomail.task == 'mail') {
            if (chronomail.env.action == 'compose') {
                chronomail.addEventListener('compose-encrypted', function(e) {
                ...
                $('#layout-sidebar > .footer:not(.pagenav) > a.button').click(function() {
                    if ($(this).is('.disabled')) {
                        chronomail.display_message(chronomail.gettext('nocontactselected'), 'warning');
                    }
                });
            ...
            if (!chronomail.env.extwin && (chronomail.env.action == 'compose' || chronomail.env.action == 'show')) {
                $('a.mail', layout.menu).attr('onclick', "return chronomail.command('list','',this,event)");
            }

            // Append contact menu to all mailto: links
            if (chronomail.env.action == 'preview' || chronomail.env.action == 'show') {
            ...
            chronomail.addEventListener('identity-encryption-show', function(p) {
                bootstrap_style(p.container);
            });
            chronomail.addEventListener('identity-encryption-update', function(p) {
                bootstrap_style(p.container);
            });
        }
        ...
        if (chronomail.env.devel_mode && window.less) {
            less.pageLoadFinished.then(function() {
                resize();
                // Re-focus the focused input field on mail compose
                if (chronomail.env.compose_focus_elem) {
                    $(chronomail.env.compose_focus_elem).focus();
                }
            });
        }
        else {
        ...
        var func, format = chronomail.env.date_format_localized;
        if (format) {
            func = function(input) {
            ...
            chronomail.addEventListener('insert-edit-field', func);
        }
    };
    ...
    function bootstrap_style(context)
    {
        ...
        $('input[type=file]:not(.custom-file-input)', context).each(function() {
            var label_text = chronomail.gettext('choosefile' + (this.multiple ? 's' : '')),
                label = $('<label>').attr({'class': 'custom-file-label',
                    'data-browse': chronomail.gettext('browse')}).text(label_text);
        ...
        if (chronomail.env.task == 'login' && context == document) {
    ...
    function dropdowns_init()
    {
        ...
        chronomail_webmail.set_iframe_events({mousedown: popups_close, touchstart: popups_close});
    };
    ...
    function content_frame_init()
    {
        ...
        var list_handler = function(e) {
            ...
            if (chronomail.env.task == 'addressbook' || chronomail.env.task == 'mail') {
                args.force = true;
            }

            // display current folder name in list header
            if (chronomail.env.task == 'mail' && !chronomail.env.action) {
                var name = $.type(e) == 'string' ? e : chronomail.env.mailbox,
                    folder = chronomail.env.mailboxes[name];
        ...
        layout.content.find('iframe').on('load', function(e) {
            ...
            try {
                ...
                show = !href.endsWith(chronomail.env.blankpage);
    ...
    function content_frame_navigation(href, event)
    {
        ...
        if (!_list || !(list = chronomail[_list])) {
        ...
        var prev, next,
            frame = $('#' + chronomail.env.contentframe),
        ...
        if ((next = list.get_next()) || chronomail.env.current_page < chronomail.env.pagecount) {
            next_button.removeClass('disabled').on('click', function() {
                ...
                if (next) {
                    ...
                    chronomail.env.list_uid = 'FIRST';
                    chronomail.command('nextpage');
                }
            });
        }

        if (((prev = list.get_prev()) && (prev != '*' || _list != 'subscription_list')) || chronomail.env.current_page > 1) {
            prev_button.removeClass('disabled').on('click', function() {
                ...
                if (prev) {
                    ...
                    chronomail.env.list_uid = 'LAST';
                    chronomail.command('previouspage');
                }
            });
        }
    };
    ...
    function tinymce_init(o)
    {
        ...
        if (chronomail.task == 'mail' && chronomail.env.action == 'compose') {
        ...
        if (is_editor) {
            ...
            o.config.setup_callback = function(ed) {
                ed.addButton('plaintext', {
                    tooltip: chronomail.gettext('plaintoggle'),
                    icon: 'plaintext',
                    onclick: function(e) {
                        if (chronomail.command('toggle-editor', {id: ed.id, html: false}, '', e.originalEvent)) {
        ...
        chronomail.addEventListener('editor-load', function(e) {
    ...
    /**
     * Handler for some Chronomail core popups
     */
    function chronomail_popup_init(o)
    {
        ...
        if (is_mobile() && $(o.obj).is('.googie_window')) {
            ...
            var title = chronomail.gettext('close'),
    ...
    function enable_command_handler(args)
    {
        ...
        if (chronomail.task == 'mail') {
            switch (args.command) {
            case 'reply-list':
                if (chronomail.env.reply_all_mode == 1) {
                    var label = chronomail.gettext(args.status ? 'replylist' : 'replyall');
    ...
    function resize()
    {
        ...
        if (mobile = is_mobile()) {
            chronomail.set_env(env.small_screen_config);
            chronomail.enable_command('extwin', false);
        }
        else {
            chronomail.set_env(env.config);
            chronomail.enable_command('extwin', true);
        }
    ...
    function hide_content()
    {
        ...
        $('[data-list]', layout.list).each(function() {
            ...
            if (chronomail[list]) {
                if (chronomail[list].clear_selection) {
                    chronomail[list].clear_selection(); // list widget
                }
                else if (chronomail[list].select) {
                    chronomail[list].select(); // treelist widget
                }
            }
        });
    };
    ...
    function message_displayed(p)
    {
        if (p.type == 'loading' && $('.iframe-loader:visible').length) {
            ...
            chronomail.hide_message(p.object);
            return;
        }
    ...
    function searchbar_init(bar)
    {
        var unread_button = $(),
            ...
            is_search_pending = function() {
                ...
                if (chronomail.task == 'mail' && $('#s_interval').val()) {
                    return true;
                }

                if (chronomail.gui_objects.search_filter && $(chronomail.gui_objects.search_filter).val() != 'ALL') {
                    return true;
                }

                if (chronomail.gui_objects.foldersfilter && $(chronomail.gui_objects.foldersfilter).val() != '---') {
                    return true;
                }
            },
            close_func = function() {
            ...
            update_func = function() {
                ...
                unread_button[chronomail.gui_objects.search_filter && $(chronomail.gui_objects.search_filter).val() == 'UNSEEN' ? 'addClass' : 'removeClass']('selected');
            };

        // Add Unread filter button
        if (input.is('#mailsearchform')) {
            unread_button = $('<a>')
                .attr({'class': 'button unread', href: '#', role: 'button', title: chronomail.gettext('showunread')})
                .on('click', function(e) {
                    $(chronomail.gui_objects.search_filter).val($(e.target).is('.selected') ? 'ALL' : 'UNSEEN');
                    chronomail.command('search');
                })
        ...
        $('a.reset', bar).on('click', function(e) {
            ...
            // Reset filter
            if (chronomail.gui_objects.search_filter) {
                $(chronomail.gui_objects.search_filter).val('ALL');
            }

            if (chronomail.gui_objects.foldersfilter) {
                $(chronomail.gui_objects.foldersfilter).val('---').change();
                chronomail.folder_filter('---');
            }

            update_func();
        });

        chronomail.addEventListener('init', update_func)
    ...
    function toolbar_init()
    {
        ...
        $('ul[data-menu="toolbar-small"] > li > a').each(function() {
            ...
            // TODO: chronomail.register_button()
            ...
            .on('shown.bs.popover', function(event) {
                ...
                if (mobile) {
                    var label = level > 1 ? 'back' : 'close',
                        title = chronomail.gettext(label),
    ...
    function menu_toggle(p)
    {
        ...
        if (p.name == 'messagelistmenu') {
            ...
            if (p.name.match(/^drag/)) {
                ...
                pos = chronomail_event.get_mouse_pos(p.originalEvent);
                target = $('<a>').css({
                        position: 'absolute',
                        left: pos.x,
                        top: pos.y,
            ...
            fn = function() {
                ...
                if (!$(target).data('popup')) {
                    $(target).data({
                        event: chronomail_event.is_keyboard(p.originalEvent) ? 'key' : 'mouse',
                        popup: p.name,
                        'popup-pos': pos,
    ...
    function menu_messagelist(p)
    {
        ...
        // set form values
        $('select[name="sort_col"]', dialog).val(chronomail.env.sort_col || '');
        $('select[name="sort_ord"]', dialog).val(chronomail.env.sort_order || 'ASC');
        $('select[name="mode"]', dialog).val(chronomail.env.threading ? 'threads' : 'list');
        ...
        var save_func = function(e) {
            if (chronomail_event.is_keyboard(e.originalEvent)) {
                $('#listmenulink').focus();
            }
            ...
            chronomail.set_list_options([], col, ord, mode == 'threads' ? 1 : 0);
            return true;
        };

        dialog = chronomail.simple_dialog(dialog, chronomail.gettext('listoptionstitle'), save_func, {
            closeOnEscape: true,
            minWidth: 400
        });
    };
    ...
    function about_dialog(elem)
    {
        var support_url, support_func, support_button = false,
            dialog = $('<iframe>').attr({id: 'aboutframe', src: chronomail.url('settings/about', {_framed: 1})}),
        ...
        chronomail.simple_dialog(dialog, $(elem).text(), support_func, {
    ...
    {
        var props = {_uid: chronomail.env.uid, _mbox: chronomail.env.mailbox, _framed: 1},
            dialog = $('<iframe>').attr({id: 'headersframe', src: chronomail.url('headers', props)});

        chronomail.simple_dialog(dialog, chronomail.gettext('arialabelmessageheaders'), null, {
            cancel_button: 'close',
            height: 400
        });
    };
    ...
    {
        if (!chronomail.commands['import-messages']) {
            return;
        }
        ...
        var save_func = function(e) {
            return chronomail.command('import-messages', $(dialog.find('form')[0]));
        };

        chronomail.simple_dialog(dialog, chronomail.gettext('importmessages'), save_func, {
            button: 'import',
            closeOnEscape: true,
            minWidth: 400
        });
    ...
    function searchmenu(obj)
    {
        var n, all,
            ...
            mbox = chronomail.env.mailbox,
            mods = chronomail.env.search_mods,
            scope = chronomail.env.search_scope || 'base';

        if (!$(obj).data('initialized')) {
            $(obj).data('initialized', true);
            if (list.length) {
                list.on('change', function() { set_searchmod(obj, this); });
                chronomail.addEventListener('beforesearch', function() { set_searchmod(obj); });
            }
        }

        if (chronomail.env.search_mods) {
            if (chronomail.env.task == 'mail') {
                if (scope == 'all') {
                    mbox = '*';
                }
    ...
    {
        var all, m, task = chronomail.env.task,
            mods = chronomail.env.search_mods,
            mbox = chronomail.env.mailbox,
        ...
        if (task == 'mail') {
            if (!mods[mbox]) {
                mods[mbox] = chronomail_clone_object(mods['*']);
            }
            m = mods[mbox];
            all = 'text';

            chronomail.env.search_scope = scope;
            chronomail.env.search_interval = interval;
        }
        else { //addressbook
            m = mods;
            all = '*';
    ...
    {
        var i, link, li, list = [],
            lang = chronomail.spellcheck_lang(),
            ul = $('ul', obj);

        if (!ul.length) {
            ul = $('<ul class="selectable listing iconized" role="menu">');

            for (i in chronomail.env.spell_langs) {
                li = $('<li role="menuitem">');
                link = $('<a href="#'+ i +'" tabindex="0"></a>')
                    .text(chronomail.env.spell_langs[i])
                    .addClass('active').data('lang', i)
                    .on('click keypress', function(e) {
                        if (e.type != 'keypress' || chronomail_event.get_keycode(e) == 13) {
                            chronomail.spellcheck_lang_set($(this).data('lang'));
                            chronomail.hide_menu('spell-menu', e);
                            return false;
                        }
                    });
    ...
    function attachmentmenu(obj, button, event)
    {
        ...
        $.each(['open', 'download', 'rename'], function() {
            ...
            $('#attachmenu' + action, obj).off('click').attr('onclick', '').click(function(e) {
                return chronomail.command(action + '-attachment', id, this, e.originalEvent);
            });
        });

        // call menu-open so core can set state of menu commands
        return chronomail.command('menu-open', {menu: 'attachmentmenu', id: id}, obj, event);
    };
    ...
    function attachmentmenu_append(item)
    {
        ...
        if (!item.is('.no-menu') && !item.children('.dropdown').length) {
            var label = chronomail.gettext('options'),
    ...
    function mailtomenu(obj, button, event, onclick)
    {
        ...
        if (chronomail.env.has_writeable_addressbook) {
            $('.addressbook', obj).addClass('active')
                .on('click', function(e) {
                    ...
                    return chronomail.command('add-contact', contact, this, e.originalEvent);
                });
        }
        ...
        $('.compose', obj).addClass('active').on('click', function(e) {
            ...
            if (onclick) {
                ...
                chronomail.command('compose', mailto, this, e.originalEvent);
            }

            return false; // for Chrome
        });

        return chronomail.command('menu-open', {menu: 'mailto-menu', link: button}, button, event.originalEvent);
    };
    ...
    function recipient_selector(field, opts)
    {
        ...
        var title = chronomail.gettext(opts.title || 'insertcontact'),
            dialog = $('#recipient-dialog'),
            parent = dialog.parent(),
            close_func = function() {
                if (dialog.is(':visible')) {
                    chronomail.env.recipient_dialog.dialog('close');
                }
            },
            insert_func = function() {
                ...
                chronomail.command('add-recipient');
            };

        if (!chronomail.env.recipient_selector_initialized) {
            chronomail.addEventListener('add-recipient', close_func);
            chronomail.env.recipient_selector_initialized = true;
        }

        if (field) {
            chronomail.env.focused_field = '#_' + field;
        }

        chronomail.contact_list.clear_selection();
        chronomail.contact_list.multiselect = 'multiselect' in opts ? opts.multiselect : true;

        chronomail.env.recipient_dialog = chronomail.simple_dialog(dialog, title, insert_func, {
            button: chronomail.gettext(opts.button || 'insert'),
            ...
            close: function() {
                ...
                $(this).remove();
                $(opts.focus || chronomail.env.focused_field).focus();
            }
        });
    };
        ...
        if (p.table) {
            element.css('cursor', 'pointer').data('popup-pos', 'top')
                .off('click').on('click', function(e) {
                    chronomail.simple_dialog(p.table, 'quota', null, {cancel_button: 'close'});
                });
        }
        else {
    ...
    function recipient_input(obj)
    {
        var list, input,
            ...
            keydown_func = function(e) {
                ...
                else if (e.key == ',' || e.key == ';' || (e.key == 'Enter' && !chronomail.ksearch_visible())) {
                    if (update_func()) {
                        return false;
                    }
                }
        ...
        // Init autocompletion
        chronomail.init_address_input_events(input);
    };
    ...
    function recipient_input_edit_dialog(e, callback)
    {
        var element = $(e.target).parents('.recipient'),
            ...
            content = $('<label>').text(chronomail.gettext('recipient')).append(input);

        chronomail.simple_dialog(content, 'recipientedit', function() {
    ...
    function image_upload_input(obj)
    {
        var reset_button = $('<a>')
                ...
                .click(function(e) { chronomail.command('delete-photo', '', this, e); return false; }),
            img = $(obj).find('img')[0],
            img_onload = function() {
                var state = (img.currentSrc || img.src).indexOf(chronomail.env.photo_placeholder) != -1;
                $(obj)[state ? 'removeClass' : 'addClass']('changed');
            };

        $(obj).append(reset_button).click(function() { chronomail.upload_input('upload-form'); });
        ...
        if (frame.length) {
            var loader = $('<div class="iframe-loader">')
                .append($('<div class="spinner spinner-border" role="status">')
                    .append($('<span class="sr-only">').text(chronomail.gettext('loading'))));
        ...
        var open_func = function(e) {
            ...
            select.popover('dispose')
                ...
                .on('shown.bs.popover', function() {
                    ...
                    list.parent().prev()
                        .empty()
                        .append($('<a class="button icon cancel">').text(chronomail.gettext('close'))
                            .on('click', function(e) {
                                e.stopPropagation();
                                return close_func();
                            })
                    ...
                    if (chronomail_event.is_keyboard(e)) {
    ...
    function html_editor_init(obj)
    {
        ...
        var sw, is_table = false,
            ...
            plain_btn = $('<a class="mce-i-html" href="#" tabindex="-1"></a>')
                .attr('title', chronomail.gettext('htmltoggle'))
                .on('click', function(e) {
                    if (chronomail.command('toggle-editor', {id: editor.attr('id'), html: true}, '', e.originalEvent)) {
    ...
    function splitter_init(node)
    {
        ...
        var list_id = node.find('.scroller .listing').first().attr('id'),
            key = chronomail.env.task + '.' + (list_id || (chronomail.env.action + '.' + node.attr('id'))),
            pos = get_pref(key),
                ...
                $(document)
                    .on('mousemove.resizer', function(e) {
                        ...
                        ts = setTimeout(function() {
                            ...
                            var cursor_position = chronomail_event.get_mouse_pos(e).x,
    ...
    /**
     * Wrapper for chronomail.open_window to intercept window opening
    ...
    function window_open(url)
    {
        ...
        chronomail.simple_dialog(frame, title, null, props);
    ...
    {
        if (!prefs) {
            prefs = chronomail.local_storage_get_item('prefs.elastic', {});
        }

        // fall-back to cookies
        if (prefs[key] == null) {
            var cookie = chronomail.get_cookie(key);
            if (cookie != null) {
                prefs[key] = cookie;

                // copy value to local storage and remove cookie (if localStorage is supported)
                if (chronomail.local_storage_set_item('prefs.elastic', prefs)) {
                    chronomail.set_cookie(key, cookie, new Date());  // expire cookie
                }
            }
        }
    ...
    function save_pref(key, val)
    {
        ...
        if (!chronomail.local_storage_set_item('prefs.elastic', prefs)) {
            ...
            chronomail.set_cookie(key, val, exp);
        }
    };
}
...
if (window.chronomail) {
     ...
     */
    chronomail.show_menu = function(prop, show, event)
    {
        ...
        // just delegate the action to chronomail_elastic_ui
        return chronomail.triggerEvent(show === false ? 'menu-close' : 'menu-open', {name: name, obj: obj, props: prop, originalEvent: event});
    }

    /**
     * Elastic version of hide_menu as we don't need e.g. menus stack handling
     */
    chronomail.hide_menu = function(name, event)
    {
        // delegate to chronomail_elastic_ui
        return chronomail.triggerEvent('menu-close', {name: name, props: {menu: name}, originalEvent: event});
    }
}
else {
    // chronomail does not exists e.g. on the error template inside a frame
    // we fake the engine a little
    var chronomail = parent.chronomail,
        chronomail_webmail = parent.chronomail_webmail,
        bw = {};
}

var UI = new chronomail_elastic_ui();
