...
require_once(CHRONOMAIL_PLUGINS_DIR . 'filesystem_attachments/filesystem_attachments.php');
...
class redundant_attachments extends filesystem_attachments
{
    ...
    // chronomail_cache instance for SQL DB
    private $cache;

    // chronomail_cache instance for memcache/redis
    private $mem_cache;
    ...
    private function _load_drivers()
    {
        ...
        $ttl      = $chronomail->config->get('redundant_attachments_cache_ttl', $ttl);
        $fallback = $chronomail->config->get('redundant_attachments_fallback');
        $prefix   = self::PREFIX;
        ...
        if ($fallback === null) {
            $fallback = $chronomail->config->get('redundant_attachments_memcache') ? 'memcache' : null; // BC
        }

        // Init SQL cache (disable cache data serialization)
        $this->cache = $chronomail->get_cache($prefix, 'db', $ttl, false);

        // Init memcache/redis (fallback) cache
        if ($fallback) {
            $this->mem_cache = $chronomail->get_cache($prefix, $fallback, $ttl, false);
        }
