...
class chronomail_sendmail
{
    ...
    public function __construct($data = array(), $options = array())
    {
        ...
        $this->options['sendmail_delay'] = (int) $this->chronomail->config->get('sendmail_delay');
    ...
    public function headers_input()
    {
        if ($this->options['sendmail'] && $this->options['sendmail_delay']) {
            $last_time = $this->chronomail->config->get('last_message_time');
        ...
        if (!($charset = $this->options['charset'])) {
            $charset = chronomail_utils::get_input_value('_charset', chronomail_utils::INPUT_POST) ?: $this->chronomail->output->get_charset();
            $this->options['charset'] = $charset;
        }

        $this->parse_data = array();

        $mailto  = $this->email_input_format(chronomail_utils::get_input_value('_to', chronomail_utils::INPUT_POST, true, $charset), true);
        $mailcc  = $this->email_input_format(chronomail_utils::get_input_value('_cc', chronomail_utils::INPUT_POST, true, $charset), true);
        $mailbcc = $this->email_input_format(chronomail_utils::get_input_value('_bcc', chronomail_utils::INPUT_POST, true, $charset), true);
        ...
        if (($max_recipients = (int) $this->chronomail->config->get('max_recipients')) > 0) {
        ...
        $dont_override = (array) $this->chronomail->config->get('dont_override');
        $mdn_enabled   = in_array('mdn_default', $dont_override) ? $this->chronomail->config->get('mdn_default') : !empty($_POST['_mdn']);
        $dsn_enabled   = in_array('dsn_default', $dont_override) ? $this->chronomail->config->get('dsn_default') : !empty($_POST['_dsn']);
        $subject       = chronomail_utils::get_input_value('_subject', chronomail_utils::INPUT_POST, true, $charset);
        $from          = chronomail_utils::get_input_value('_from', chronomail_utils::INPUT_POST, true, $charset);
        $replyto       = chronomail_utils::get_input_value('_replyto', chronomail_utils::INPUT_POST, true, $charset);
        $followupto    = chronomail_utils::get_input_value('_followupto', chronomail_utils::INPUT_POST, true, $charset);
        ...
        $from_string = chronomail_charset::convert($from_string, RCUBE_CHARSET, $charset);
        $message_id  = $this->data['param']['message-id'];

        if (!$message_id) {
            $message_id = $this->chronomail->gen_message_id($from);
        }
        ...
        if ($this->options['savedraft']) {
            ...
            if (($uid = $this->data['reply_uid']) && !preg_match('/^\d+\.[0-9.]+$/', $uid)) {
                ...
                && ($uid = chronomail_imap_generic::compressMessageSet($this->data['forward_uid']))
    ...
    public function set_message_encoding($message, $flowed = false)
    {
        ...
        if (preg_match('/ISO-2022/i', $text_charset)) {
            ...
            $transfer_encoding = $this->chronomail->config->get('force_7bit') ? 'quoted-printable' : '8bit';
        }
    ...
    public function create_message($headers, $body, $isHtml = false, $attachments = array())
    {
        ...
        $line_length = $this->chronomail->config->get('line_length', 72);
        $charset     = $this->options['charset'];
        $flowed      = $this->options['savedraft'] || $this->chronomail->config->get('send_format_flowed', true);
        ...
        if (is_array($attachments)) {
            ...
            if (!chronomail_utils::mem_check($memory * 1.33 * 12)) {
                $MAIL_MIME->setParam('delay_file_io', true);
            }
        }

        $plugin = $this->chronomail->plugins->exec_hook('message_outgoing_body', array(
                'body'    => $body,
        ...
        if ($isHtml) {
            ...
            $plain_body = $this->chronomail->html2text($plugin['body'], array('width' => 0, 'charset' => $charset));
            $plain_body = chronomail_mime::wordwrap($plain_body, $line_length, "\r\n", false, $charset);
            ...
            if ($flowed) {
                $body = chronomail_mime::format_flowed($body, min($line_length + 2, 79), $charset);
            }
            else {
                $body = chronomail_mime::wordwrap($body, $line_length, "\r\n", false, $charset);
            }
    ...
    public function deliver_message($message)
    {
        ...
        $sent = $this->chronomail->deliver_message($message,
            $this->options['from'],
            $this->options['mailto'],
        ...
        if ($this->options['sendmail_delay']) {
            $this->chronomail->user->save_prefs(array('last_message_time' => time()));
        }

        // set replied/forwarded flag
        if ($this->data['reply_uid']) {
            foreach (chronomail::get_uids($this->data['reply_uid'], $this->data['mailbox']) as $mbox => $uids) {
                // skip <UID>.<PART> replies
                if (!preg_match('/^\d+\.[0-9.]+$/', implode(',', (array) $uids))) {
                    $this->chronomail->storage->set_flag($uids, 'ANSWERED', $mbox);
                }
            }
        }
        else if ($this->data['forward_uid']) {
            foreach (chronomail::get_uids($this->data['forward_uid'], $this->data['mailbox']) as $mbox => $uids) {
                // skip <UID>.<PART> forwards
                if (!preg_match('/^\d+\.[0-9.]+$/', implode(',', (array) $uids))) {
                    $this->chronomail->storage->set_flag($uids, 'FORWARDED', $mbox);
                }
            }
        }
    ...
    public function save_message($message)
    {
        ...
        if ($this->options['savedraft']) {
            $store_target = $this->chronomail->config->get('drafts_mbox');
        }
        else if (!$this->chronomail->config->get('no_save_sent_messages')) {
            if (isset($_POST['_store_target'])) {
                $store_target = chronomail_utils::get_input_value('_store_target', chronomail_utils::INPUT_POST, true);
            }
            else {
                $store_target = $this->chronomail->config->get('sent_mbox');
            }
        }

        if ($store_target) {
            $storage = $this->chronomail->get_storage();
            ...
            if ($store_folder) {
                ...
                if ($message->mailbody_file || $message->getParam('delay_file_io')) {
                    ...
                    if ($message->mailbody_file) {
                        ...
                        $message->mailbody_file = chronomail_utils::temp_filename('msg');
            ...
            if (!$saved) {
                chronomail::raise_error(array('code' => 800, 'type' => 'imap',
    ...
    {
        if ($this->chronomail->config->get('http_received_header')) {
            $nldlm       = "\r\n\t";
            $http_header = 'from ';
            ...
            // BY
            $http_header .= $nldlm . 'by ' . chronomail_utils::server_name('HTTP_HOST');
    ...
    {
        if ($this->chronomail->config->get('http_received_header_encrypt')) {
            return $this->chronomail->encrypt($host);
        }
    ...
    {
        if ($sql_arr = $this->chronomail->user->get_identity($id)) {
            $out = $sql_arr;

            if ($this->options['charset'] != RCUBE_CHARSET) {
                foreach ($out as $k => $v) {
                    $out[$k] = chronomail_charset::convert($v, RCUBE_CHARSET, $this->options['charset']);
                }
            }
    ...
    public static function extract_inline_images($message, $from)
    {
        ...
        if (preg_match_all($regexp, $body, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            foreach ($matches[1] as $idx => $m) {
                ...
                if (empty($mime_type)) {
                    $mime_type = chronomail_mime::image_content_type($data);
                }
    ...
    public function email_input_format($mailto, $count = false, $check = true)
    {
        ...
        $items  = chronomail_utils::explode_quoted_string("[$delim]", $mailto);
        $result = array();
        ...
        foreach ($items as $item) {
            ...
            if (preg_match('/^<'.$email_regexp.'>$/', $item)) {
                $item     = chronomail_utils::idn_to_ascii(trim($item, '<>'));
                $result[] = $item;
            }
            // address without brackets and without name (add brackets)
            else if (preg_match('/^'.$email_regexp.'$/', $item)) {
                $item     = chronomail_utils::idn_to_ascii($item);
                $result[] = $item;
            }
            // address with name (handle name)
                ...
                $address  = chronomail_utils::idn_to_ascii(trim($address, '<>'));
            ...
            $item = trim($item, '<>');
            if ($item && $check && !chronomail_utils::check_email($item)) {
    ...
    {
        if ($isHtml && ($file = $this->chronomail->config->get('generic_message_footer_html'))) {
            $html_footer = true;
        }
        else {
            $file = $this->chronomail->config->get('generic_message_footer');
            $html_footer = false;
        }
        ...
        if ($file && realpath($file)) {
            ...
            if (!preg_match('/\.(php|ini|conf)$/', $file) && strpos($file, '/etc/') === false) {
                ...
                if ($isHtml && !$html_footer) {
                    $t2h    = new chronomail_text2html($footer, false);
                    $footer = $t2h->get_html();
                }

                if ($this->options['charset'] && $this->options['charset'] != RCUBE_CHARSET) {
                    $footer = chronomail_charset::convert($footer, RCUBE_CHARSET, $this->options['charset']);
                }
    ...
    public function headers_output($attrib)
    {
        ...
        $this->chronomail->autocomplete_init();
    ...
    protected function compose_header_from($attrib)
    {
        ...
        if (!empty($this->options['message']->identities)) {
            $a_signatures = array();
            $identities   = array();
            $top_posting  = intval($this->chronomail->config->get('reply_mode')) > 0
                && !$this->chronomail->config->get('sig_below')
                && ($this->data['mode'] == self::MODE_REPLY || $this->data['mode'] == self::MODE_FORWARD);

            $separator     = $top_posting ? '---' : '-- ';
            $add_separator = (bool) $this->chronomail->config->get('sig_separator');

            $field_attrib['onchange'] = chronomail_output::JS_OBJECT_NAME . ".change_identity(this)";
            ...
            foreach ($this->options['message']->identities as $sql_arr) {
                ...
                if (!empty($sql_arr['signature']) && empty($this->data['param']['nosig'])) {
                    ...
                    if ($sql_arr['html_signature']) {
                        $text = $this->chronomail->html2text($html, array('links' => false));
                        $text = trim($text, "\r\n");
                    }
                    else {
                        $t2h  = new chronomail_text2html($text, false);
                        $html = $t2h->get_html();
                    }
            ...
            $this->chronomail->output->set_env('signatures', $a_signatures);
            $this->chronomail->output->set_env('identities', $identities);
        }
    ...
    protected function compose_header_value($header, $mode)
    {
        ...
        if ($header == 'to' && ($mailto_id = $this->data['param']['mailto'])
            ...
            $decode_header = false;
            $charset       = $this->chronomail->output->charset;

            // make session to not grow up too much
            $this->chronomail->session->remove("mailto.$mailto_id");
        }
        else if (!empty($_POST['_' . $header])) {
            $fvalue  = chronomail_utils::get_input_value('_' . $header, chronomail_utils::INPUT_POST, true);
            $charset = $this->chronomail->output->charset;
        }
            ...
            if ($header == 'to') {
                ...
                if (!empty($message->compose['ident']) && empty($replyto)) {
                    foreach (array($fvalue, $message->headers->from) as $sender) {
                        $senders = chronomail_mime::decode_address_list($sender, null, false, $charset, true);
            ...
            // add recipient of original message if reply to all
                ...
                if ($v = $message->headers->from) {
                    ...
                    $from    = chronomail_mime::decode_address_list($v, null, false, $charset, true);
                    $to      = chronomail_mime::decode_address_list($to, null, false, $charset, true);
                    $replyto = chronomail_mime::decode_address_list($replyto, null, false, $charset, true);
        ...
        if (!empty($fvalue) && in_array($header, array('to', 'cc', 'bcc'))) {
            ...
            $to_addresses = chronomail_mime::decode_address_list($fvalue, null, $decode_header, $charset);
            $fvalue       = array();
            ...
            foreach ($to_addresses as $addr_part) {
                ...
                $mailto    = format_email(chronomail_utils::idn_to_utf8($addr_part['mailto']));
    ...
    public function compose_subject($attrib)
    {
        ...
        if (isset($_POST['_subject'])) {
            $subject = chronomail_utils::get_input_value('_subject', chronomail_utils::INPUT_POST, TRUE);
        }
    ...
    {
        if (chronomail_utils::get_boolean((string) $attrib['noform'])) {
            return array('', '');
        }

        $form_start = '';
        if (!$this->message_form) {
            $hiddenfields = new html_hiddenfield(array('name' => '_task', 'value' => $this->chronomail->task));
            ...
            if (empty($attrib['form'])) {
                ...
                $form_start = $this->chronomail->output->form_tag($form_attr);
            }
        ...
        if (!$this->message_form) {
            $this->chronomail->output->add_gui_object('messageform', $form_name);
        }
    ...
    public function folder_selector($attrib)
    {
        ...
        $select = $this->chronomail->folder_selector(array_merge($attrib, array(
                'noselection'   => '- ' . $this->chronomail->gettext('dontsave') . ' -',
                'folder_filter' => 'mail',
                'folder_rights' => 'w',
        )));
    ...
    public function mdn_checkbox($attrib)
    {
        ...
        if (isset($_POST['_mdn'])) {
            ...
            $mdn_default = $this->chronomail->config->get('mdn_default');
        }
    ...
    public function dsn_checkbox($attrib)
    {
        ...
        if (isset($_POST['_dsn'])) {
            ...
            $dsn_value = $this->chronomail->config->get('dsn_default');
        }
    ...
    public function priority_selector($attrib)
    {
        ...
        $prio_list = array(
            $this->chronomail->gettext('lowest')  => 5,
            $this->chronomail->gettext('low')     => 4,
            $this->chronomail->gettext('normal')  => 0,
            $this->chronomail->gettext('high')    => 2,
            $this->chronomail->gettext('highest') => 1,
        );
    ...
    public static function check_sent_folder($folder, $create = false)
    {
        ...
        if ($chronomail->config->get('no_save_sent_messages')) {
            return true;
        }

        if ($chronomail->storage->folder_exists($folder, true)) {
            return true;
        }

        // folder may exist but isn't subscribed (#1485241)
        if ($create) {
            if (!$chronomail->storage->folder_exists($folder))
                return $chronomail->storage->create_folder($folder, true);
            else
                return $chronomail->storage->subscribe($folder);
        }
    ...
    protected function compose_init($message)
    {
        ...
        $message->identities = $this->chronomail->user->list_identities(null, true);

        // Set From field value
        if (!empty($_POST['_from'])) {
            $message->compose['from'] = chronomail_utils::get_input_value('_from', chronomail_utils::INPUT_POST);
        }
        ...
        $this->chronomail->output->add_handlers(array(
        ...
        $this->chronomail->output->add_label('nosubject', 'nosenderwarning', 'norecipientwarning',
        ...
        $this->chronomail->output->set_env('max_disclosed_recipients', (int) $this->chronomail->config->get('max_disclosed_recipients', 5));
    }

    /**
     * Detect recipient identity from specified message
     *
     * @param chronomail_message $message    Message object
    ...
    public static function identity_select($message, $identities = null, $mode = null)
    {
        ...
        if ($identities === null) {
            $identities = chronomail::get_instance()->user->list_identities(null, true);
        }
        ...
        if (is_object($message->headers) && in_array($mode, array(self::MODE_REPLY, self::MODE_FORWARD))) {
            $a_to = chronomail_mime::decode_address_list($message->headers->to, null, true, $message->headers->charset);
            ...
            if (!empty($message->headers->cc)) {
                $a_cc = chronomail_mime::decode_address_list($message->headers->cc, null, true, $message->headers->charset);
        ...
        // decode From: address
        $from = chronomail_mime::decode_address_list($message->headers->from, null, true, $message->headers->charset);
        ...
        $plugin = chronomail::get_instance()->plugins->exec_hook('identity_select', array(
                'message'    => $message,
