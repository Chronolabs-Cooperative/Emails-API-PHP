 ...
 * @subpackage Storage
 */
class chronomail_imap extends chronomail_storage
{
    /**
     * Instance of chronomail_imap_generic
     *
     * @var chronomail_imap_generic
     */
    public $conn;

    /**
     * Instance of chronomail_imap_cache
     *
     * @var chronomail_imap_cache
     */
    protected $mcache;

    /**
     * Instance of chronomail_cache
     *
     * @var chronomail_cache
     */
    protected $cache;
    ...
    {
        $this->conn    = new chronomail_imap_generic();
        $this->plugins = chronomail::get_instance()->plugins;
    ...
    public function connect($host, $user, $pass, $port=143, $use_ssl=null)
    {
        ...
        if ($use_ssl && extension_loaded('openssl')) {
            ...
            chronomail::raise_error(array('code' => 403, 'type' => 'imap',
        ...
        do {
            ...
            chronomail_utils::parse_socket_options($data['socket_options'], $data['host']);
        ...
        // write error log
            ...
            if ($pass && $user) {
                $message = sprintf("Login failed for %s against %s from %s. %s",
                    $user, $host, chronomail_utils::remote_ip(), $this->conn->error);
     ...
     *
     * @param array $set Search set, result from chronomail_imap::get_search_set():
     *                      0 - searching criteria, string
     *                      1 - search result, chronomail_result_index|chronomail_result_thread
    ...
    public function set_search_set($set)
    {
        ...
        $this->search_threads    = is_a($this->search_set, 'chronomail_result_thread');

        if (is_a($this->search_set, 'chronomail_result_multifolder')) {
    ...
    public function get_capability($cap)
    {
        ...
        if (!isset($_SESSION[$sess_key])) {
            ...
            if ($cap == chronomail_storage::DUAL_USE_FOLDERS) {
    ...
    protected function set_env()
    {
        ...
        $config         = chronomail::get_instance()->config;
    ...
    protected function countmessages($folder, $mode = 'ALL', $force = false, $status = true, $no_search = false)
    {
        ...
        //        maybe in chronomail_imap_search
     ...
     *
     * @return array Indexed array with message header objects
     * @see chronomail_imap::list_messages
     */
     ...
     * @param string $folder Folder name
     *
     * @return chronomail_imap_thread Thread data object
     */
    ...
    {
        if (!$this->check_connection()) {
            return new chronomail_result_thread();
        }
     ...
     *
     * @param string              $folder     Folder name
     * @param chronomail_result_thread $threads    Threads data object
    ...
    protected function list_search_messages($folder, $page, $slice=0)
    {
            ...
            if (!empty($sort_field) && $search_set->get_parameters('SORT') != $sort_field) {
                ...
                foreach ($search_set->sets as $resultset) {
                    if (!$resultset->is_empty()) {
                        ...
                        $this->search_threads = $resultset instanceof chronomail_result_thread;
                ...
                if (!empty($a_msg_headers)) {
                    $a_msg_headers = chronomail_imap_generic::sortHeaders($a_msg_headers, $sort_field, $this->sort_order);
                }
        ...
        if (($cnt > 300 && $cnt > $this->page_size) || !$this->sort_field) {
            ...
            $a_msg_headers = chronomail_imap_generic::sortHeaders(
     ...
     *
     * @return array Indexed array with message header objects
     * @see chronomail_imap::list_search_messages()
     */
    ...
    function fetch_headers($folder, $msgs, $sort = true, $force = false)
    {
        ...
        if ($sort) {
            ...
            $sorter = new chronomail_message_header_sorter();
     ...
     *
     * @return chronomail_result_index|chronomail_result_thread List of messages (UIDs)
     */
    public function index($folder = '', $sort_field = NULL, $sort_order = NULL,
        ...
        if ($this->search_string) {
            if ($this->search_set->is_empty()) {
                return new chronomail_result_index($folder, '* SORT');
            }

            if ($this->search_set instanceof chronomail_result_multifolder) {
                $index = $this->search_set;
                $index->folder = $folder;
                // TODO: handle changed sorting
            }
            // search result is an index with the same sorting?
            else if (($this->search_set instanceof chronomail_result_index)
            ...
            // This makes possible to use index from cache
                ...
                if (!$this->sort_field) {
                    ...
                    $index  = new chronomail_result_index($folder, '* ESEARCH ALL ' . $search);
                }
                else {
     ...
     * @param string         $sort_field Sort column
     * @param string         $sort_order Sort order [ASC, DESC]
     * @param chronomail_result_* $search     Optional messages set to limit the result
     *
     * @return chronomail_result_index Sorted list of message UIDs
     */
    public function index_direct($folder, $sort_field = null, $sort_order = null, $search = null)
    {
        ...
        if (!$sort_field) {
            ...
            if (empty($search) && $this->options['skip_deleted']
                ...
                return new chronomail_result_index();
            }
            else {
     ...
     * @param string $sort_order Sort order [ASC, DESC]
     *
     * @return chronomail_result_thread Message UIDs
     */
     ...
     *
     * @param chronomail_result_thread $threads Threads result set
     */
     ...
     * @param  string  $sort_field Header field to sort by
     *
     * @return chronomail_result_index  Search result object
    ...
    public function search($folder = '', $search = 'ALL', $charset = null, $sort_field = null)
    {
        ...
        if (!$results && is_array($folder) && count($folder) > 1 && $search != 'ALL') {
            ...
            $searcher = new chronomail_imap_search($this->options, $this->conn);
     ...
     *
     * @return chronomail_result_index  Search result (UIDs)
     */
     ...
     *
     * @return chronomail_result_index|chronomail_result_thread  Search results (UIDs)
     * @see chronomail_imap::search()
     */
    ...
    public static function convert_criteria($str, $charset, $dest_charset='US-ASCII')
    {
        ...
        if (preg_match_all('/\{([0-9]+)\}\r\n/', $str, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            foreach ($matches[1] as $m) {
                ...
                $string = chronomail_charset::convert($string, $charset, $dest_charset);

                if ($string === false || !strlen($string)) {
                    continue;
                }

                $res .= substr($str, $last, $m[1] - $last - 1) . chronomail_imap_generic::escape($string);
    ...
    {
        if ($this->search_set && is_a($this->search_set, 'chronomail_result_multifolder')) {
     ...
     *
     * @return chronomail_message_header Message headers
     */
     ...
     * @param string $folder Folder to read from
     *
     * @return object chronomail_message_header Message data
     */
    public function get_message($uid, $folder = null)
    {
        ...
        if ($headers->ctype && !is_array($structure[0]) && $headers->ctype != 'text/plain'
            ...
            // we can handle single-part messages, by simple fix in structure (#1486898)
                ...
                // Try to parse the message using chronomail_mime_decode.
                ...
                if ($headers->size && chronomail_utils::mem_check($headers->size * 10)) {
                    $raw_msg = $this->get_raw_body($uid);
                    $struct = chronomail_mime::parse_message($raw_msg);
                }
                else {
                    return $headers;
                }
    ...
    {
        $struct = new chronomail_message_part;
        ...
        */
        ...
        if ($struct->ctype_primary == 'message' || ($struct->ctype_parameters['name'] && !$struct->content_id)) {
            ...
            if (is_string($mime_headers)) {
                $struct->headers = chronomail_mime::parse_headers($mime_headers) + $struct->headers;
            }
     ...
     *
     * @param chronomail_message_part $part    Part object
    ...
    protected function set_part_filename(&$part, $headers = null)
    {
        ...
        if (!empty($filename_mime)) {
            if (!empty($part->charset)) {
                ...
                $charset = chronomail_charset::detect($filename_mime, $this->default_charset);
            }

            $part->filename = chronomail_mime::decode_mime_string($filename_mime, $charset);
        }
            ...
            $part->filename = chronomail_charset::convert(urldecode($filename_encoded), $filename_charset);
        }
    }
     ...
     * @param chronomail_message_part Part object created by get_structure()
        ...
        $skip_charset_conv = false, $max_bytes = 0, $formatted = true)
    {
        ...
        if (!is_object($o_part)) {
            ...
            $part_data = chronomail_imap_generic::getStructurePartData($structure, $part);

            $o_part = new chronomail_message_part;
        ...
        if ($body && preg_match('/^(text|message)$/', $o_part->ctype_primary)) {
            ...
            if (!$skip_charset_conv) {
                ...
                $body = chronomail_charset::convert($body, $o_part->charset);
            }
        }
    ...
    public function move_message($uids, $to_mbox, $from_mbox = '')
    {
        ...
        if (!$moved && $to_trash && $this->get_response_code() == chronomail_storage::TRYCREATE) {
    ...
    public function expunge_message($uids, $folder = null, $clear_cache = true)
    {
        ...
        if (!$this->conn->data['READ-WRITE']) {
            $this->conn->setError(chronomail_imap_generic::ERROR_READONLY, "Folder is read-only");
            return false;
        }
    ...
    {
        $cache_key = chronomail_cache::key_name('mailboxes', array($root, $name, $filter, $rights));
     ...
     * @see chronomail_imap::list_folders_subscribed()
     */
    ...
    {
        $cache_key = chronomail_cache::key_name('mailboxes.list', array($root, $name, $filter, $rights));
     ...
     *
     * @return array List of folders
     * @see chronomail_imap::list_folders()
     */
    ...
    public function get_special_folders($forced = false)
    {
        ...
        if ($chronomail->config->get('lock_special_folders')) {
            return $result;
        }
        ...
        $types   = array_map(function($value) { return "\\" . ucfirst($value); }, chronomail_storage::$folder_types);
        $special = array();
    ...
    public function set_special_folders($specials)
    {
        ...
        foreach ($specials as $type => $folder) {
            if (in_array($type, chronomail_storage::$folder_types)) {
                $old_folder = $old[$type];
    ...
    public function folder_info($folder)
    {
        ...
        // get cached metadata
        $cache_key = chronomail_cache::key_name('mailboxes.folder-info', array($folder));
    ...
    /**
     * Get message header names for chronomail_imap_generic::fetchHeader(s)
     *
    ...
    public function set_acl($folder, $user, $acl)
    {
        ...
        $this->clear_cache(chronomail_cache::key_name('mailboxes.folder-info', array($folder)));
    ...
    public function get_metadata($folder, $entries, $options = array(), $force = false)
    {
        ...
        if (!$force) {
            $cache_key = chronomail_cache::key_name('mailboxes.metadata', array($folder, $options, $entries));
     ...
     *
     * @param string $type Cache type (@see chronomail::get_cache)
     */
    ...
    protected function get_cache_engine()
    {
        if ($this->caching && !$this->cache) {
            $chronomail = chronomail::get_instance();
            $ttl   = $chronomail->config->get('imap_cache_ttl', '10d');
            $this->cache = $chronomail->get_cache('IMAP', $this->caching, $ttl);
        }
    ...
    protected function get_mcache_engine()
    {
        if ($this->messages_caching && !$this->mcache) {
            $chronomail = chronomail::get_instance();
            if (($dbh = $chronomail->get_dbh()) && ($userid = $chronomail->get_user_id())) {
                $ttl       = $chronomail->config->get('messages_cache_ttl', '10d');
                $threshold = $chronomail->config->get('messages_cache_threshold', 50);
                $this->mcache = new chronomail_imap_cache(
    ...
    {
        $val = chronomail::get_instance()->config->get('imap_dual_use_folders');
        if ($val !== null) {
            return (bool) $val;
        }
    ...
    public function sort_folder_list($a_folders, $skip_default = false)
    {
        ...
        foreach ($a_folders as $folder) {
            ...
            $folders[$folder] = strpos($folder, '&') === false ? $folder : chronomail_charset::convert($folder, 'UTF7-IMAP');
        }
    ...
    protected function date_format($date)
    {
        ...
        if (!is_object($date) || !is_a($date, 'DateTime')) {
            try {
                $timestamp = chronomail_utils::strtotime($date);
    ...
    {
        chronomail::write_log('imap', $message);
    }
}
