 ...
 * by Galen Johnson.
 *
 * It only works with saslpasswd2 on the same host where Chronomail runs
...
class chronomail_sasl_password
{
    function save($currpass, $newpass, $username)
    {
        $curdir   = CHRONOMAIL_PLUGINS_DIR . 'password/helpers';
        $username = escapeshellarg($username);
        $args     = chronomail::get_instance()->config->get('password_saslpasswd_args', '');
