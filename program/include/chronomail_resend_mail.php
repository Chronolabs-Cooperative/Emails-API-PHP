 ...
 * @package Webmail
 */
class chronomail_resend_mail extends Mail_mime
{
    protected $orig_head;
    protected $orig_body;
     ...
     *   'bounce_message' - chronomail_message object of the original message
    ...
    protected function init_message()
    {
        ...
        $path     = chronomail_utils::temp_filename('bounce');

        // We'll write the body to the file and the headers to a variable
        if ($fp = fopen($path, 'w')) {
            stream_filter_register('bounce_source', 'chronomail_bounce_stream_filter');
            ...
            $this->orig_head = chronomail_bounce_stream_filter::$headers;
            $this->orig_body = $path;
        }
    }
}
 ...
 * @package Webmail
 */
class chronomail_bounce_stream_filter extends php_user_filter
{
