...
class chronomail_imap_generic
{
     ...
     *
     * @return chronomail_result_index Response data
     */
    public function sort($mailbox, $field = 'ARRIVAL', $criteria = '', $return_uid = false, $encoding = 'US-ASCII')
    {
        ...
        if (!in_array($field, $supported)) {
            return new chronomail_result_index($mailbox);
        }
        ...
        if ($old_sel != $mailbox && !$this->data['EXISTS']) {
            return new chronomail_result_index($mailbox, '* SORT');
        }
        ...
        return new chronomail_result_index($mailbox, $response);
    }
     ...
     * @param string $encoding   Character set
     *
     * @return chronomail_result_thread Thread data
     */
    public function thread($mailbox, $algorithm = 'REFERENCES', $criteria = '', $return_uid = false, $encoding = 'US-ASCII')
    {
        ...
        if (!$this->select($mailbox)) {
            return new chronomail_result_thread($mailbox);
        }

        // return empty result when folder is empty and we're just after SELECT
        if ($old_sel != $mailbox && !$this->data['EXISTS']) {
            return new chronomail_result_thread($mailbox, '* THREAD');
        }
        ...
        return new chronomail_result_thread($mailbox, $response);
    }
     ...
     *
     * @return chronomail_result_index Result data
     */
    public function search($mailbox, $criteria, $return_uid = false, $items = array())
    {
        ...
        // return empty result when folder is empty and we're just after SELECT
        if ($old_sel != $mailbox && !$this->data['EXISTS']) {
            return new chronomail_result_index($mailbox, '* SEARCH');
        }
        ...
        $uidfetch=false, $return_uid=false)
    {
        ...
        return new chronomail_result_index($mailbox, $msg_index);
    }
            ...
            if (preg_match('/^\* ([0-9]+) FETCH/', $line, $m)) {
                ...
                if ($mode == 1 && $index_field == 'DATE') {
                    if (preg_match('/BODY\[HEADER\.FIELDS \("*DATE"*\)\] (.*)/', $line, $matches)) {
                        ...
                        $value = trim($value);
                        $result[$id] = chronomail_utils::strtotime($value);
                    }
                    // non-existent/empty Date: header, use INTERNALDATE
                    if (empty($result[$id])) {
                        if (preg_match('/INTERNALDATE "([^"]+)"/', $line, $matches)) {
                            $result[$id] = chronomail_utils::strtotime($matches[1]);
                        }
                        else {
                            $result[$id] = 0;
                        }
     ...
     *
     * @return array List of chronomail_message_header elements, False on error
     * @since 0.6
     */
        ...
        $mod_seq = null, $vanished = false)
    {
        ...
        do {
            ...
            if (preg_match('/^\* ([0-9]+) FETCH/', $line, $m)) {
                ...
                $result[$id]            = new chronomail_message_header;
                $result[$id]->id        = $id;
                $result[$id]->subject   = '';
                ...
                while (list($name, $value) = $this->tokenizeResponse($line, 2)) {
                    if ($name == 'UID') {
                        ...
                        $result[$id]->timestamp    = chronomail_utils::strtotime($value);
                    }
                    else if ($name == 'FLAGS') {
                        if (!empty($value)) {
                ...
                if (!empty($headers)) {
                    ...
                    foreach ($lines as $str) {
                        ...
                        case 'date';
                            ...
                            $result[$id]->timestamp = chronomail_utils::strtotime($string);
                            break;
                        case 'to':
     ...
     *
     * @return bool|array List of chronomail_message_header elements, False on error
     */
     ...
     * @param array  $add_headers List of additional headers
     *
     * @return bool|chronomail_message_header Message data, False on error
     */
     ...
     *
     * @param array  $messages Array of chronomail_message_header objects
    ...
    public static function sortHeaders($messages, $field, $flag)
    {
        ...
        foreach ($messages as $key => $headers) {
            ...
            case 'internaldate':
            case 'timestamp':
                $value = chronomail_utils::strtotime($headers->$field);
