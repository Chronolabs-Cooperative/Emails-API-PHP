/**
 * Chronomail functions for default skin interface
 *
...
function chronomail_mail_ui()
{
  ...
  {
    if (!prefs) {
      prefs = chronomail.local_storage_get_item('prefs.larry', {});
    }
    ...
    if (prefs[key] == null) {
      ...
      if (cookie != null) {
        ...
        if (chronomail.local_storage_set_item('prefs.larry', prefs)) {
  ...
  function init()
  {
    chronomail.addEventListener('message', message_displayed);
    ...
    if (chronomail.env.task == 'mail') {
      chronomail.addEventListener('menu-open', menu_toggle)
        .addEventListener('menu-close', menu_toggle)
        .addEventListener('menu-save', save_listoptions)
        .addEventListener('enable-command', enable_command)
        .addEventListener('responseafterlist', function(e){ switch_view_mode(chronomail.env.threading ? 'thread' : 'list', true) })
        .addEventListener('responseaftersearch', function(e){ switch_view_mode(chronomail.env.threading ? 'thread' : 'list', true) });

      var dragmenu = $('#dragmessagemenu');
      if (dragmenu.length) {
        chronomail.gui_object('dragmenu', 'dragmessagemenu');
        popups.dragmenu = dragmenu;
      }

      if (chronomail.env.action == 'show' || chronomail.env.action == 'preview') {
        chronomail.addEventListener('aftershow-headers', function() { layout_messageview(); })
        ...
        $('#previewheaderstoggle').click(function(e) {
            ...
            if (this.blur && !chronomail_event.is_keyboard(e))
                this.blur();
            return false;
        });
      ...
      else if (chronomail.env.action == 'compose') {
        chronomail.addEventListener('fileappended', function(e) { if (e.attachment.complete) attachmentmenu_append(e.item); })
        ...
        var opener_rc = chronomail.opener();
        if (opener_rc && opener_rc.env.action == 'compose' && $('#composeoptionstoggle', opener.document).hasClass('remove'))
          $('#composeoptionstoggle').click();

        new chronomail_splitter({ id:'composesplitterv', p1:'#composeview-left', p2:'#composeview-right',
      ...
      else if (chronomail.env.action == 'bounce') {
        init_compose_editfields();
      }
      else if (chronomail.env.action == 'list' || !chronomail.env.action) {
        mail_layout();

        $('#maillistmode').addClass(chronomail.env.threading ? '' : 'selected').click(function(e) { switch_view_mode('list'); return false; });
        $('#mailthreadmode').addClass(chronomail.env.threading ? 'selected' : '').click(function(e) { switch_view_mode('thread'); return false; });

        chronomail.init_pagejumper('#pagejumper');

        chronomail.addEventListener('setquota', update_quota)
          .addEventListener('layout-change', mail_layout);
      }
      else if (chronomail.env.action == 'get') {
        new chronomail_splitter({ id:'mailpartsplitterv', p1:'#messagepartheader', p2:'#messagepartcontainer',
          orientation:'v', relative:true, start:226, min:150, size:12}).init();
      }

      if ($('#mailview-left').length) {
        new chronomail_splitter({ id:'mailviewsplitterv', p1:'#mailview-left', p2:'#mailview-right',
          orientation:'v', relative:true, start:206, min:150, size:12, callback:render_mailboxlist, render:resize_leftcol }).init();
      }
    }
    /***  settings task  ***/
    else if (chronomail.env.task == 'settings') {
      chronomail.addEventListener('init', function(){
        var tab = '#settingstabpreferences';
        if (chronomail.env.action)
          tab = '#settingstab' + (chronomail.env.action.indexOf('identity')>0 ? 'identities' : chronomail.env.action.replace(/\./g, ''));

        $(tab).addClass('selected')
          .children().first().removeAttr('onclick').click(function() { return false; });
      });

      if (chronomail.env.action == 'folders') {
        new chronomail_splitter({ id:'folderviewsplitter', p1:'#folderslist', p2:'#folder-details',
      ...
      else if (chronomail.env.action == 'identities') {
        new chronomail_splitter({ id:'identviewsplitter', p1:'#identitieslist', p2:'#identity-details',
          orientation:'v', relative:true, start:266, min:180, size:12 }).init();
      }
      else if (chronomail.env.action == 'responses') {
        new chronomail_splitter({ id:'responseviewsplitter', p1:'#responseslist', p2:'#response-details',
          orientation:'v', relative:true, start:266, min:180, size:12 }).init();
      }
      else if (chronomail.env.action == 'preferences' || !chronomail.env.action) {
        new chronomail_splitter({ id:'prefviewsplitter', p1:'#sectionslist', p2:'#preferences-box',
          orientation:'v', relative:true, start:266, min:180, size:12 }).init();
      }
      else if (chronomail.env.action == 'edit-prefs') {
        var legend = $('#preferences-details fieldset.advanced legend'),
          toggle = $('<a href="#toggle"></a>')
            .text(chronomail.gettext('toggleadvancedoptions'))
            .attr('title', chronomail.gettext('toggleadvancedoptions'))
    ...
    /***  addressbook task  ***/
    else if (chronomail.env.task == 'addressbook') {
      chronomail.addEventListener('beforepushgroup', push_contactgroup)
        .addEventListener('beforepopgroup', pop_contactgroup)
        .addEventListener('menu-open', menu_toggle)
        .addEventListener('menu-close', menu_toggle);

      if (chronomail.env.action == '') {
        new chronomail_splitter({ id:'addressviewsplitterd', p1:'#addressview-left', p2:'#addressview-right',
          orientation:'v', relative:true, start:206, min:150, size:12, render:resize_leftcol }).init();
        new chronomail_splitter({ id:'addressviewsplitter', p1:'#addresslist', p2:'#contacts-box',
          orientation:'v', relative:true, start:266, min:260, size:12 }).init();
      }

      var dragmenu = $('#dragcontactmenu');
      if (dragmenu.length) {
        chronomail.gui_object('dragmenu', 'dragcontactmenu');
        popups.dragmenu = dragmenu;
      }
    }
  ...
  function resize(e)
  {
    ...
    if (chronomail.resize_timeout)
      window.clearTimeout(chronomail.resize_timeout);

    chronomail.resize_timeout = window.setTimeout(function() {
      if (chronomail.env.task == 'mail') {
        if (chronomail.env.action == 'show' || chronomail.env.action == 'preview')
          layout_messageview();
        else if (chronomail.env.action == 'compose')
  ...
  function message_displayed(p)
  {
    ...
    if (p.type == 'error' && chronomail.env.task != 'login') {
      ...
      me.messagedialog.html(msg)
        .dialog({
          ...
          closeOnEscape: true,
          dialogClass: p.type,
          title: chronomail.gettext('errortitle'),
          close: dialog_close,
          hide: {effect: 'fadeOut'},
          width: 420,
          minHeight: 90
  ...
  {
    var layout = p ? p.new_layout : chronomail.env.layout,
      top = $('#mailview-top'),
    ...
    chronomail.env.contentframe = layout == 'list' ? null : 'messagecontframe';
    ...
    if (layout == 'widescreen') {
      ...
      if (!mailviewsplit2) {
        mailviewsplit2 = new chronomail_splitter({ id:'mailviewsplitter2', p1:'#mailview-top', p2:'#mailview-bottom',
      ...
      if (!mailviewsplit) {
        mailviewsplit = new chronomail_splitter({ id:'mailviewsplitter', p1:'#mailview-top', p2:'#mailview-bottom',
  ...
  function init_compose_editfields()
  {
    ...
    chronomail.addEventListener('autocomplete_insert', function(p){ adjust_compose_editfields(p.field); });
  }
...
//    $('#composeformbuttons')[(btns ? 'show' : 'hide')]();
  ...
  function update_quota(p)
  {
    ...
    if (p.table) {
      ...
      element.css('cursor', 'pointer').off('click').on('click', function(e) {
        return chronomail.command('menu-open', 'quotamenu', e.target, e);
      });
    }
  }
  ...
  {
    if (p.command == 'reply-list' && chronomail.env.reply_all_mode == 1) {
      var label = chronomail.gettext(p.status ? 'replylist' : 'replyall');
      if (chronomail.env.action == 'preview')
  ...
  function toggle_popup(popup, e, config)
  {
    ...
    return chronomail.command('menu-open', popup, e.target, e);
  }
  ...
  function show_popup(popup, show, config)
  {
    ...
    return chronomail.command('menu-open',
  ...
  {
    if (show && chronomail.env.search_mods) {
      var n, all,
  ...
  function attachmentmenu(elem, event)
  {
    ...
    $.each(['open', 'download', 'rename'], function() {
      ...
      $('#attachmenu' + action).off('click').attr('onclick', '').click(function(e) {
        return chronomail.command(action + '-attachment', id, this);
      });
    });

    popupconfig.attachmentmenu.link = elem;
    chronomail.command('menu-open', {menu: 'attachmentmenu', id: id}, elem, event);
  }
  ...
  function spellmenu(show, p)
  {
    ...
    if (!ul.length) {
      ...
      for (k in chronomail.env.spell_langs) {
        li = $('<li role="menuitem">');
        link = $('<a href="#'+k+'" tabindex="0"></a>').text(chronomail.env.spell_langs[k])
          ...
          .on('click keypress', function(e) {
              if (e.type != 'keypress' || chronomail_event.get_keycode(e) == 13) {
                  ...
                  chronomail.hide_menu('spellmenu', e);
                  return false;
              }
          });
  ...
  function show_listoptions(p)
  {
    ...
    // set form values
    $('input[name="sort_col"][value="'+chronomail.env.sort_col+'"]').prop('checked', true);
    $('input[name="sort_ord"][value="DESC"]').prop('checked', chronomail.env.sort_order == 'DESC');
    $('input[name="sort_ord"][value="ASC"]').prop('checked', chronomail.env.sort_order != 'DESC');

    $.each(['widescreen', 'desktop', 'list'], function() {
      $('input[name="layout"][value="' + this + '"]').prop('checked', chronomail.env.layout == this);
    });
    $('#listoptions-columns', $dialog)[chronomail.env.layout == 'widescreen' ? 'hide' : 'show']();

    // set checkboxes
    $('input[name="list_col[]"]').each(function() {
      $(this).prop('checked', $.inArray(this.value, chronomail.env.listcols) != -1);
    });
    ...
    $dialog.dialog({
      ...
      close: function(e) {
        ...
        if (e.originalEvent && chronomail_event.is_keyboard(e.originalEvent))
          $('#listmenulink').focus();
      },
      minWidth: 500,
  ...
  function save_listoptions(p)
  {
    ...
    if (chronomail_event.is_keyboard(p.originalEvent))
      $('#listmenulink').focus();
    ...
    chronomail.set_list_options(cols, sort, ord, chronomail.env.threading, layout);
  }
  ...
  {
    chronomail.set_searchscope(elem.value);
  }
  ...
  {
    var frame = $('<iframe>').attr({id: 'aboutframe', src: chronomail.url('settings/about'), frameborder: '0'});
...
/**
 * Chronomail Scroller class
 *
 * @deprecated Use treelist widget
 */
function chronomail_scroller(list, top, bottom)
{
  var ref = this;
  ...
  this.top
    .mouseenter(function() { if (chronomail.drag_active) ref.ts = window.setTimeout(function() { ref.scroll('down'); }, ref.delay); })
    .mouseout(function() { if (ref.ts) window.clearTimeout(ref.ts); });

  this.bottom
    .mouseenter(function() { if (chronomail.drag_active) ref.ts = window.setTimeout(function() { ref.scroll('up'); }, ref.delay); })
    .mouseout(function() { if (ref.ts) window.clearTimeout(ref.ts); });
...
/**
 * Chronomail UI splitter class
 *
 * @constructor
 */
function chronomail_splitter(p)
{
  this.p = p;
  this.id = p.id;
  ...
  var me = this;
  chronomail_splitter._instances[this.id] = me;
  ...
  function onDragStart(e)
  {
    ...
    if (chronomail.message_list)
      chronomail.message_list.drag_active = true;
  ...
  function onDragStop(e)
  {
    ...
    if (chronomail.message_list)
      chronomail.message_list.drag_active = false;
    ...
    return bw.safari ? true : chronomail_event.cancel(e);
  };
...
} // end class chronomail_splitter


// static getter for splitter instances
chronomail_splitter._instances = {};

chronomail_splitter.get_instance = function(id)
{
  return chronomail_splitter._instances[id];
};
