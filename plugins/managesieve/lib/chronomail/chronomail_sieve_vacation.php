...
class chronomail_sieve_vacation extends chronomail_sieve_engine
{
    protected $error;
    ...
    protected function vacation_post()
    {
        ...
        $status        = chronomail_utils::get_input_value('vacation_status', chronomail_utils::INPUT_POST);
        $from          = chronomail_utils::get_input_value('vacation_from', chronomail_utils::INPUT_POST);
        $subject       = chronomail_utils::get_input_value('vacation_subject', chronomail_utils::INPUT_POST, true);
        $reason        = chronomail_utils::get_input_value('vacation_reason', chronomail_utils::INPUT_POST, true);
        $addresses     = chronomail_utils::get_input_value('vacation_addresses', chronomail_utils::INPUT_POST, true);
        $interval      = chronomail_utils::get_input_value('vacation_interval', chronomail_utils::INPUT_POST);
        $interval_type = chronomail_utils::get_input_value('vacation_interval_type', chronomail_utils::INPUT_POST);
        $date_from     = chronomail_utils::get_input_value('vacation_datefrom', chronomail_utils::INPUT_POST);
        $date_to       = chronomail_utils::get_input_value('vacation_dateto', chronomail_utils::INPUT_POST);
        $time_from     = chronomail_utils::get_input_value('vacation_timefrom', chronomail_utils::INPUT_POST);
        $time_to       = chronomail_utils::get_input_value('vacation_timeto', chronomail_utils::INPUT_POST);
        $after         = chronomail_utils::get_input_value('vacation_after', chronomail_utils::INPUT_POST);
        $action        = chronomail_utils::get_input_value('vacation_action', chronomail_utils::INPUT_POST);
        $target        = chronomail_utils::get_input_value('action_target', chronomail_utils::INPUT_POST, true);
        $target_domain = chronomail_utils::get_input_value('action_domain', chronomail_utils::INPUT_POST);
        ...
        if (!empty($vacation_action['from']) && !chronomail_utils::check_email($vacation_action['from'])) {
            $error = 'noemailwarning';
        }
        ...
        if ($date_extension) {
            ...
            foreach (array('date_from', 'date_to') as $var) {
                ...
                $date = chronomail_utils::format_datestr($$var, $date_format);
                $date = trim($date . ' ' . $time);

                if ($date && ($dt = chronomail_utils::anytodatetime($date, $timezone))) {
                    if ($time) {
        ...
        if ($action == 'redirect' || $action == 'copy') {
            ...
            if (empty($target) || !chronomail_utils::check_email($target)) {
                $error = 'noemailwarning';
            }
        }
    ...
    public function vacation_form($attrib)
    {
        ...
        $addresses_link = new html_inputfield(array(
                ...
                'href'    => '#',
                'class' => 'button',
                'onclick' => chronomail_output::JS_OBJECT_NAME . '.managesieve_vacation_addresses()'
            ));
        ...
        $action_target = ' <span id="action_target_span" class="input-group"' . (!$redirect ? ' style="display:none"' : '') . '>'
            ...
            . ' value="' .($redirect ? chronomail::Q($this->vacation['target'], 'strict', false) : '') . '"'
    ...
    {
        $tests    = array();
        $dt_from  = chronomail_utils::anytodatetime($date_from);
        $dt_to    = chronomail_utils::anytodatetime($date_to);
    ...
    public static function vacation_interval(&$vacation, $extensions = array())
    {
        ...
        else if ($interval_cfg = $chronomail->config->get('managesieve_vacation_interval')) {
    ...
    public function set_vacation($data)
    {
        ...
        if (!empty($vacation['from']) && !chronomail_utils::check_email($vacation['from'])) {
        ...
        if ($data['action'] == 'redirect' || $data['action'] == 'copy') {
            if (empty($data['target']) || !chronomail_utils::check_email($data['target'])) {
