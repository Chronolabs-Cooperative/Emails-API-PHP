...
if (window.chronomail) {
  chronomail.addEventListener('init', function(evt) {
    ...
    if (chronomail.env.task == 'mail') {
      if (chronomail.env.action != 'show')
        chronomail.env.message_commands.push('managesieve-create');
      else
        chronomail.enable_command('managesieve-create', true);
    }

    if (chronomail.env.task == 'mail' || chronomail.env.action.startsWith('plugin.managesieve')) {
      // Create layer for form tips
      if (!chronomail.env.framed) {
        chronomail.env.ms_tip_layer = $('<div id="managesieve-tip" class="popupmenu"></div>');
        chronomail.env.ms_tip_layer.appendTo(document.body);
      }
    }

    // register commands
    chronomail.register_command('plugin.managesieve-save', function() { chronomail.managesieve_save() });
    chronomail.register_command('plugin.managesieve-act', function() { chronomail.managesieve_act() });
    chronomail.register_command('plugin.managesieve-add', function() { chronomail.managesieve_add() });
    chronomail.register_command('plugin.managesieve-del', function() { chronomail.managesieve_del() });
    chronomail.register_command('plugin.managesieve-move', function() { chronomail.managesieve_move() });
    chronomail.register_command('plugin.managesieve-setadd', function() { chronomail.managesieve_setadd() });
    chronomail.register_command('plugin.managesieve-setdel', function() { chronomail.managesieve_setdel() });
    chronomail.register_command('plugin.managesieve-setact', function() { chronomail.managesieve_setact() });
    chronomail.register_command('plugin.managesieve-setget', function() { chronomail.managesieve_setget() });
    chronomail.register_command('plugin.managesieve-seteditraw', function() { chronomail.managesieve_seteditraw() });

    if (chronomail.env.action.startsWith('plugin.managesieve')) {
      if (chronomail.gui_objects.sieveform) {
        chronomail.enable_command('plugin.managesieve-save', true);
        sieve_form_init();
      }
      else if (chronomail.gui_objects.sievesetrawform) {
        chronomail.enable_command('plugin.managesieve-save', true);
        sieve_raw_editor_init();
      }
      else {
        chronomail.enable_command('plugin.managesieve-add', !chronomail.env.sieveconnerror && $.inArray('new_rule', chronomail.env.managesieve_disabled_actions) == -1);
        chronomail.enable_command('plugin.managesieve-setadd', !chronomail.env.sieveconnerror && $.inArray('new_set', chronomail.env.managesieve_disabled_actions) == -1);
      }

      var setcnt, set = chronomail.env.currentset;

      if (chronomail.gui_objects.filterslist) {
        chronomail.filters_list = new chronomail_list_widget(chronomail.gui_objects.filterslist,
          {multiselect:false, draggable:true, keyboard:true});

        chronomail.filters_list
          .addEventListener('select', function(o) { chronomail.managesieve_select(o); })
          .addEventListener('keypress', function(o) { chronomail.list_keypress(o, {del: 'plugin.managesieve-del'}); })
          .addEventListener('dragstart', function(o) { chronomail.managesieve_dragstart(o); })
          .addEventListener('dragend', function(o) { chronomail.managesieve_dragend(o); })
          .addEventListener('initrow', function(row) {
            row.obj.onmouseover = function() { chronomail.managesieve_focus_filter(row); };
            row.obj.onmouseout = function() { chronomail.managesieve_unfocus_filter(row); };
          })
          .init();
      }

      if (chronomail.gui_objects.filtersetslist) {
        chronomail.filtersets_list = new chronomail_list_widget(chronomail.gui_objects.filtersetslist,
          {multiselect:false, draggable:false, keyboard:true});

        chronomail.filtersets_list.init().focus();

        if (set != null) {
          $('#filterset-name').text(set);
          set = chronomail.managesieve_setid(set);
          chronomail.filtersets_list.select(set);
        }

        // attach select event after initial record was selected
        chronomail.filtersets_list.addEventListener('select', function(e) { chronomail.managesieve_setselect(e); });

        setcnt = chronomail.filtersets_list.rowcount;
        chronomail.enable_command('plugin.managesieve-set', true);
        chronomail.enable_command('plugin.managesieve-setact', setcnt > 0 && $.inArray('enable_disable_set', chronomail.env.managesieve_disabled_actions) == -1);
        chronomail.enable_command('plugin.managesieve-setget', setcnt > 0 && $.inArray('download_set', chronomail.env.managesieve_disabled_actions) == -1);
        chronomail.enable_command('plugin.managesieve-setdel', setcnt > 1 && $.inArray('delete_set', chronomail.env.managesieve_disabled_actions) == -1);
        chronomail.enable_command('plugin.managesieve-seteditraw', setcnt > 0 && chronomail.env.raw_sieve_editor);

        // Fix dragging filters over sets list
        $('tr', chronomail.gui_objects.filtersetslist).each(function (i, e) { chronomail.managesieve_fixdragend(e); });
      }
    }

    if (chronomail.gui_objects.sieveform && chronomail.env.rule_disabled)
...
chronomail_webmail.prototype.managesieve_del = function()
{
...
chronomail_webmail.prototype.managesieve_act = function()
{
  var id = this.filters_list.get_single_selection(),
...
// Filter selection
chronomail_webmail.prototype.managesieve_select = function(list)
{
  ...
  this.enable_command('plugin.managesieve-del', has_id && $.inArray('delete_rule', chronomail.env.managesieve_disabled_actions) == -1);
};

// Set selection
chronomail_webmail.prototype.managesieve_setselect = function(list)
{
  this.enable_command('plugin.managesieve-setdel', list.rowcount > 1 && $.inArray('delete_set', chronomail.env.managesieve_disabled_actions) == -1);
  this.enable_command('plugin.managesieve-setact', list.rowcount > 0 && $.inArray('enable_disable_set', chronomail.env.managesieve_disabled_actions) == -1);
  this.enable_command('plugin.managesieve-setget', list.rowcount > 0 && $.inArray('delete_set', chronomail.env.managesieve_disabled_actions) == -1);
  this.enable_command('plugin.managesieve-seteditraw', list.rowcount > 0 && this.env.raw_sieve_editor);

 if (chronomail.env.contextmenu_opening)
   return;
...
chronomail_webmail.prototype.managesieve_rowid = function(id)
{
...
chronomail_webmail.prototype.managesieve_setid = function(name)
{
...
chronomail_webmail.prototype.managesieve_list = function(script)
{
...
chronomail_webmail.prototype.managesieve_setget = function()
{
...
chronomail_webmail.prototype.managesieve_setact = function()
{
  var id = this.filtersets_list.get_single_selection(),
   lock = this.set_busy(true, 'loading'),
    script = this.env.filtersets[id],
...
// Set delete request
chronomail_webmail.prototype.managesieve_setdel = function()
{
  var id = this.filtersets_list.get_single_selection();
...
chronomail_webmail.prototype.managesieve_seteditraw = function()
{
  var id = this.filtersets_list.get_single_selection(),
    script = this.env.filtersets[id];
...
// Set add request
chronomail_webmail.prototype.managesieve_setadd = function()
{
  this.load_managesieveframe('_nav=hide&_newset=1', true);
};

chronomail_webmail.prototype.managesieve_updatelist = function(action, o)
{
  this.set_busy(true);
  ...
  switch (action) {
    ...
    case 'add':
      ...
      this.enable_command('plugin.managesieve-del', $.inArray('delete_rule', chronomail.env.managesieve_disabled_actions) == -1);
...
chronomail_webmail.prototype.reset_filters_list = function(reload)
{
...
// load filter frame
chronomail_webmail.prototype.load_managesieveframe = function(add_url, reset)
{
  if (reset)
    this.reset_filters_list();
...
// load filter frame
chronomail_webmail.prototype.managesieve_dragstart = function(list)
{
  var id = this.filters_list.get_single_selection();
...
chronomail_webmail.prototype.managesieve_dragend = function(e)
{
  if (this.drag_active) {
...
chronomail_webmail.prototype.managesieve_fixdragend = function(elem)
{
  var p = this;
...
chronomail_webmail.prototype.managesieve_focus_filter = function(row)
{
...
chronomail_webmail.prototype.managesieve_unfocus_filter = function(row)
{
  if (this.drag_active) {
...
// Form submition
chronomail_webmail.prototype.managesieve_save = function()
{
  ...
  if (this.gui_objects.sieveform) {
    if (parent.chronomail && parent.chronomail.filters_list && this.gui_objects.sieveform.name != 'filtersetform') {
      var id = parent.chronomail.filters_list.get_single_selection();
      if (id != null)
        this.gui_objects.sieveform.elements['_fid'].value = parent.chronomail.filters_list.rows[id].uid;
    }
...
chronomail_webmail.prototype.managesieve_ruleadd = function(id)
{
  this.http_post('plugin.managesieve-action', '_act=ruleadd&_rid='+id);
};

chronomail_webmail.prototype.managesieve_rulefill = function(content, id, after)
{
  if (content != '') {
    // create new element
    var div = $('#rules')[0],
...
chronomail_webmail.prototype.managesieve_ruledel = function(id)
{
...
chronomail_webmail.prototype.managesieve_actionadd = function(id)
{
  this.http_post('plugin.managesieve-action', '_act=actionadd&_aid='+id);
};

chronomail_webmail.prototype.managesieve_actionfill = function(content, id, after)
{
  if (content != '') {
    var div = $('#actions')[0],
...
chronomail_webmail.prototype.managesieve_actiondel = function(id)
{
...
chronomail_webmail.prototype.managesieve_insertrow = function(div, row, after)
{
...
chronomail_webmail.prototype.managesieve_formbuttons = function(div)
{
...
chronomail_webmail.prototype.managesieve_vacation_addresses = function(id)
{
  var lock = this.set_busy(true, 'loading');
...
// update vacation addresses field with user identities
chronomail_webmail.prototype.managesieve_vacation_addresses_update = function(id, addresses)
{
...
function smart_field_init(field)
{
  ...
  if (field.hasClass('error')) {
    area.addClass('error');
    chronomail.managesieve_tip_register([[id, field.data('tip-class'), field.data('tip-msg')]]);
  }
};
...
chronomail_webmail.prototype.managesieve_tip_register = function(tips)
{
  if (window.UI && UI.form_errors)
    return UI.form_errors(tips);

  var n, framed = parent.chronomail,
    tip = framed ? parent.chronomail.env.ms_tip_layer : chronomail.env.ms_tip_layer;
  ...
  for (n in tips) {
    $('#'+tips[n][0])
      ...
      .mouseenter(function(e) {
        ...
        if (framed) {
          offset = $((chronomail.env.task == 'mail'  ? '#sievefilterform > iframe' : '#filter-box'), parent.document).offset();
          top  += offset.top;
          left += offset.left;
        }
...
{
  var i, c, h, time = '', format = chronomail.env.time_format || 'H:i';
...
{
  var form = chronomail.gui_objects.sieveform;

  // resize dialog window
  if (chronomail.env.action == 'plugin.managesieve' && chronomail.env.task == 'mail') {
    parent.chronomail.managesieve_dialog_resize(form);
  }
  ...
  if ($.datepicker && chronomail.env.date_format) {
    $.datepicker.setDefaults({
      dateFormat: chronomail.env.date_format,
      changeMonth: true,
...
function sieve_raw_editor_init()
{
  ...
  if (textArea && !cmeditor) {
    ...
    $.each(chronomail.env.sieve_errors || [], function(i, err) {
...
chronomail_webmail.prototype.managesieve_create = function(force)
{
  ...
  $.each(this.env.sieve_headers, function(i, v) {
    var attr = {type: 'checkbox', name: 'headers[]', id: 'sievehdr' + i, value: i,  checked: true},
      label = chronomail.env.sieve_headers[i][0] + ': ' + chronomail.env.sieve_headers[i][1];
  ...
  buttons[this.get_label('managesieve.nextstep')] = function () {
    ...
    if (!hdrs.length) {
      chronomail.alert_dialog(chronomail.get_label('managesieve.nodata'));
      return;
    }

    // build frame URL
    var url = chronomail.get_task_url('mail');
    url = chronomail.add_url(url, '_action', 'plugin.managesieve');
    url = chronomail.add_url(url, '_framed', 1);

    hdrs.map(function() {
      var val = chronomail.env.sieve_headers[this.value];
      url = chronomail.add_url(url, 'r['+this.value+']', val[0]+':'+val[1]);
    });
    ...
    buttons[chronomail.get_label('save')] = function() {
      var win = $('iframe', dialog).get(0).contentWindow;
      win.chronomail.managesieve_save();
    };
    buttons[chronomail.get_label('cancel')] = function() { $(this).dialog('destroy'); };

    dialog.dialog('destroy');

    chronomail.env.managesieve_dialog = dialog = chronomail.show_popup_dialog(
...
chronomail_webmail.prototype.managesieve_dialog_close = function()
{
  this.env.managesieve_dialog.dialog('destroy');
}

chronomail_webmail.prototype.managesieve_dialog_resize = function(o)
{
