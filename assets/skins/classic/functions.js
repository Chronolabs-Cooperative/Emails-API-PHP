...
function chronomail_init_settings_tabs()
{
  var el, cl, container = $('#tabsbar'),
    last_tab = $('span', container).last(),
    tab = '#settingstabpreferences',
    action = window.chronomail && chronomail.env.action ? chronomail.env.action : null;
...
function chronomail_init_tabs(id, current)
{
  var content = $('#'+id),
  ...
  fs.each(function(idx) {
    ...
    tab = $('<span>').attr({'id': 'tab'+idx, 'class': 'tablink'})
        .click(function() { chronomail_show_tab(id, idx); return false })
...
function chronomail_show_tab(id, index)
{
...
show_popupmenu: function(popup, show)
{
  var obj = this.popups[popup].obj,
    ...
    ref = $(this.popups[popup].link ? this.popups[popup].link : chronomail_find_object(popup+'link'));
...
{
  $("input[name='forwardtype'][value="+(chronomail.env.forward_attachment ? 1 : 0)+"]", this.popups.forwardmenu.obj)
    .prop('checked', true);
...
uploadmenu: function(show)
{
  ...
  if (chronomail.mailvelope_editor)
    return;
...
searchmenu: function(show)
{
  var obj = this.popups.searchmenu.obj,
    ref = chronomail_find_object('searchmenulink');
...
show_listmenu: function(p)
{
  ...
  $('input[name="view"][value="thread"]').prop('checked', chronomail.env.threading ? true : false);
  $('input[name="view"][value="list"]').prop('checked', chronomail.env.threading ? false : true);
  ...
  buttons[chronomail.gettext('save')] = function(e) {
...
save_listmenu: function()
{
  ...
  chronomail.set_list_options(cols, sort, ord, thread == 'thread' ? 1 : 0, layout);
},
...
spellmenu: function(show)
{
  ...
  if (!ul.length) {
    ...
    for (i in chronomail.env.spell_langs) {
      li = $('<li>');
      link = $('<a href="#"></a>').text(chronomail.env.spell_langs[i])
...
{
  var element = chronomail_find_object(elementid);
...
set_layout: function(p)
{
  ...
  if (!this.mailviewsplitv) {
    this.mailviewsplitv = new chronomail_splitter({id:'mailviewsplitterv', p1: 'mailleftcontainer', p2: 'mailrightcontainer',
      orientation: 'v', relative: true, start: 165, callback: chronomail_render_mailboxlist });
  ...
  if (layout == 'widescreen') {
    ...
    if (!this.mailviewsplit2) {
      this.mailviewsplit2 = new chronomail_splitter({id:'mailviewsplitter2', p1: 'mailcontframe', p2: 'mailpreviewframe',
    ...
    if (!this.mailviewsplit) {
      this.mailviewsplit = new chronomail_splitter({id:'mailviewsplitter', p1: 'mailcontframe', p2: 'mailpreviewframe',
...
init_compose_form: function()
{
  ...
  for (f=0; f<fields.length; f++) {
    ...
    if (field.length) {
      field.on('change', {v:v}, function(e) { if (this.value) chronomail_ui.show_header_form(e.data.v); });
      if (field.val() != '')
        chronomail_ui.show_header_form(v);
    }
  }

  // prevent from form data loss when pressing ESC key in IE
  if (bw.ie) {
    var form = chronomail_find_object('form');
    form.onkeydown = function (e) {
      if (chronomail_event.get_keycode(e) == 27)
        chronomail_event.cancel(e);
    };
  }

  $(window).resize(function() {
    chronomail_ui.resize_compose_body();
  });
  ...
  $('#quicksearchbox').keydown(function(e) {
    if (chronomail_event.get_keycode(e) == 13)
...
{
  window.setTimeout(function(){chronomail_ui.resize_compose_body();}, 100);
},
...
/**
 * Chronomail generic layer (floating box) class
 *
 * @constructor
 */
function chronomail_layer(id, attributes)
{
  this.name = id;
...
function chronomail_render_mailboxlist()
{
...
function fit_string_to_size(str, elem, len)
{
  ...
  if (!chronomail.env.tmp_span) {
    ...
    chronomail.env.tmp_span = span;
  }
  else {
    span = chronomail.env.tmp_span;
  }
...
{
  percent_indicator(chronomail.gui_objects.quotadisplay, data);
...
function percent_indicator(obj, data)
{
  ...
  var limit_high = 80,
    limit_mid  = 55,
    width = data.width ? data.width : chronomail.env.indicator_width ? chronomail.env.indicator_width : 100,
    height = data.height ? data.height : chronomail.env.indicator_height ? chronomail.env.indicator_height : 14,
  ...
  chronomail.env.indicator_width = width;
  chronomail.env.indicator_height = height;
  ...
  if (data.title)
    data.title = chronomail.get_label('quota') + ': ' +  data.title;
  ...
  $(item).append(
    $('<a class="drop"></a>').on('click keypress', function(e) {
      if (e.type != 'keypress' || e.which == 13) {
        chronomail_ui.show_attachmentmenu(this, e);
        return false;
      }
    })
  );
...
var chronomail_editor_settings = {};

var chronomail_ui;

function chronomail_init_mail_ui()
{
  chronomail_ui = new chronomail_mail_ui();

  $(document.body).mouseup(function(e) { chronomail_ui.body_mouseup(e); })
    .mousedown(function(e) { chronomail_ui.body_keydown(e); });

  chronomail.addEventListener('init', function() {
    if (chronomail.env.quota_content)
      update_quota(chronomail.env.quota_content);
    chronomail.addEventListener('setquota', update_quota);

    chronomail_webmail.set_iframe_events({mouseup: function(e) { return chronomail_ui.body_mouseup(e); }});

    if (chronomail.env.task == 'mail') {
      chronomail.addEventListener('enable-command', 'enable_command', chronomail_ui)
        .addEventListener('menu-open', 'menu_open', chronomail_ui)
        .addEventListener('aftersend-attachment', 'uploadmenu', chronomail_ui)
        .addEventListener('aftertoggle-editor', 'resize_compose_body_ev', chronomail_ui)
        .addEventListener('afterbounce', function(){ chronomail_ui.show_popup('forwardmenu', false); })
        .gui_object('dragmenu', 'dragmenu');

      if (chronomail.gui_objects.mailboxlist) {
        chronomail.treelist.addEventListener('expand', chronomail_render_mailboxlist);
        chronomail.addEventListener('responseaftermark', chronomail_render_mailboxlist)
          .addEventListener('responseaftergetunread', chronomail_render_mailboxlist)
          .addEventListener('responseaftercheck-recent', chronomail_render_mailboxlist)
          .addEventListener('responseafterrefresh', chronomail_render_mailboxlist)
          .addEventListener('afterimport-messages', function(){ chronomail_ui.show_popup('uploadform', false); });
      }

      chronomail.init_pagejumper('#pagejumper');

      // fix message list header on window resize (#1490213)
      if (bw.ie && chronomail.message_list)
        $(window).resize(function() {
          setTimeout(function() { chronomail.message_list.resize(); }, 10);
        });

      if (chronomail.env.action == 'list' || !chronomail.env.action) {
        chronomail.addEventListener('layout-change', 'set_layout', chronomail_ui);
        chronomail_ui.set_layout();
      }
      else if (chronomail.env.action == 'compose') {
        chronomail_ui.init_compose_form();
      ...
      else if (chronomail.env.action == 'show' || chronomail.env.action == 'preview') {
        // add menu link for each attachment
      ...
      chronomail.addEventListener('afterupload-photo', function(){ chronomail_ui.show_popup('uploadform', false); })
        .gui_object('dragmenu', 'dragmenu');
    }
    else if (chronomail.env.task == 'settings') {
      if (chronomail.env.action == 'folders') {
        chronomail_ui.folder_search_init($('#folder-manager'));
      }
