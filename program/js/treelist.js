...
/**
 * chronomail Treelist widget class
 * @constructor
 */
function chronomail_treelist_widget(node, p)
{
  ...
  if (p.searchbox) {
    searchfield = $(p.searchbox).off('keyup.treelist').on('keyup.treelist', function(e) {
      var key = chronomail_event.get_keycode(e),
        mod = chronomail_event.get_modifier(e);
   ...
   * This is for consistency with chronomail_list_widget
   */
  ...
  {
    if (p.save_state && window.chronomail) {
      var key = 'treelist-' + list_id;
      if (!tree_state) {
        tree_state = chronomail.local_storage_get_item(key, {});
      }

      if (tree_state[id] != collapsed) {
        tree_state[id] = collapsed;
        chronomail.local_storage_set_item(key, tree_state);
      }
    }
  }
  ...
  function get_state(id)
  {
    if (p.save_state && window.chronomail) {
      if (!tree_state) {
        tree_state = chronomail.local_storage_get_item('treelist-' + list_id, {});
      }
    ...
    switch (keyCode) {
      ...
      case 39: // Right arrow key
        ...
        if (li.length) {
          ...
          if (node && node.children.length && node.collapsed != (keyCode == 37))
            toggle(id, chronomail_event.get_modifier(e) == SHIFT_KEY);  // toggle subtree
        }
        return false;
      ...
      case 9:  // Tab
        if (p.keyboard && p.tabexit) {
          ...
          var limit = chronomail_event.get_modifier(e) == SHIFT_KEY ? 'first' : 'last';
  ...
  function droppable(opts)
  {
    ...
    my_opts.over = function(e, ui) {
      intersects(chronomail_event.get_mouse_pos(e), false);
      if (opts.over)
        opts.over(e, ui);
    };
...
// use event processing functions from chronomail's chronomail_event_engine
chronomail_treelist_widget.prototype.addEventListener = chronomail_event_engine.prototype.addEventListener;
chronomail_treelist_widget.prototype.removeEventListener = chronomail_event_engine.prototype.removeEventListener;
chronomail_treelist_widget.prototype.triggerEvent = chronomail_event_engine.prototype.triggerEvent;
