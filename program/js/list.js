...
/**
 * chronomail List Widget class
 * @constructor
 */
function chronomail_list_widget(list, p)
{
  // static contants
  this.ENTER_KEY = 13;
  this.DELETE_KEY = 46;
  ...
  chronomail_list_widget._instances.push(this);
};


chronomail_list_widget.prototype = {
...
init: function()
{
  ...
  if (this.tbody) {
    ...
    // set body events
    if (this.keyboard) {
      chronomail_event.add_listener({event:'keydown', object:this, method:'key_press'});
  ...
  chronomail.triggerEvent('initlist', { obj: this.list });
    ...
    if ((bw.ie || bw.edge) && bw.pointer) {
      ...
      row.addEventListener('touchstart', function(e) {
        if (e.touches.length == 1) {
          ...
          self.drag_row(chronomail_event.touchevent(e.touches[0]), this.uid);
          self.touch_start_time = new Date().getTime();
        }
      }, false);
      row.addEventListener('touchend', function(e) {
        if (e.changedTouches.length == 1) {
          var duration = (new Date().getTime() - self.touch_start_time);
          if (!self.touchmoved && duration <= self.touch_event_time && !self.click_row(chronomail_event.touchevent(e.changedTouches[0]), this.uid))
            e.preventDefault();
        }
      }, false);
...
insert_checkbox: function(row)
{
  ...
  chbox.onmousedown = function(e) {
    key = chronomail_event.get_modifier(e);
  };
...
drag_column: function(e, col)
{
  if (this.colcount > 1) {
    ...
    this.drag_mouse_start = chronomail_event.get_mouse_pos(e);

    chronomail_event.add_listener({event:'mousemove', object:this, method:'column_drag_mouse_move'});
    chronomail_event.add_listener({event:'mouseup', object:this, method:'column_drag_mouse_up'});
...
drag_row: function(e, id)
{
  ...
  // accept right-clicks
  if (chronomail_event.get_button(e) == 2)
    return true;
  ...
  if (!this.in_selection_before) {
    var mod_key = chronomail_event.get_modifier(e);
  ...
  if (this.draggable && this.selection.length && this.in_selection(id)) {
    ...
    chronomail_event.add_listener({event:'mousemove', object:this, method:'drag_mouse_move'});
    chronomail_event.add_listener({event:'mouseup', object:this, method:'drag_mouse_up'});

    if (bw.touch) {
      chronomail_event.add_listener({event:'touchmove', object:this, method:'drag_mouse_move'});
      chronomail_event.add_listener({event:'touchend', object:this, method:'drag_mouse_up'});
    }
...
click_row: function(e, id)
{
  ...
  if (!this.drag_active && !dblclicked && this.in_selection_before == id)
    this.select_row(id, chronomail_event.get_modifier(e), true);
...
{
  var target = chronomail_event.get_target(e),
...
{
  var row = this.rows[id],
    evtarget = chronomail_event.get_target(e),
    mod_key = chronomail_event.get_modifier(e),
...
key_press: function(e)
{
  ...
  var keyCode = chronomail_event.get_keycode(e),
    mod_key = chronomail_event.get_modifier(e);
...
drag_mouse_move: function(e)
{
  ...
  if (e.type == 'touchmove') {
    if (e.touches.length == 1 && e.changedTouches.length == 1)
      e = chronomail_event.touchevent(e.changedTouches[0]);
    else
      return chronomail_event.cancel(e);
  }
  ...
  if (this.drag_start) {
    ...
    var m = chronomail_event.get_mouse_pos(e),
...
drag_mouse_up: function(e)
{
  ...
  chronomail_event.remove_listener({event:'mousemove', object:this, method:'drag_mouse_move'});
  chronomail_event.remove_listener({event:'mouseup', object:this, method:'drag_mouse_up'});

  if (bw.touch) {
    chronomail_event.remove_listener({event:'touchmove', object:this, method:'drag_mouse_move'});
    chronomail_event.remove_listener({event:'touchend', object:this, method:'drag_mouse_up'});
  }
...
column_drag_mouse_move: function(e)
{
  if (this.drag_start) {
    // check mouse movement, of less than 3 pixels, don't start dragging
    var i, m = chronomail_event.get_mouse_pos(e);
  ...
  if (this.col_drag_active && this.col_draglayer) {
    var i, cpos = 0, pos = chronomail_event.get_mouse_pos(e);
...
column_drag_mouse_up: function(e)
{
  ...
  chronomail_event.remove_listener({event:'mousemove', object:this, method:'column_drag_mouse_move'});
  chronomail_event.remove_listener({event:'mouseup', object:this, method:'column_drag_mouse_up'});
...
chronomail_list_widget.prototype.addEventListener = chronomail_event_engine.prototype.addEventListener;
chronomail_list_widget.prototype.removeEventListener = chronomail_event_engine.prototype.removeEventListener;
chronomail_list_widget.prototype.triggerEvent = chronomail_event_engine.prototype.triggerEvent;

// static
chronomail_list_widget._instances = [];
