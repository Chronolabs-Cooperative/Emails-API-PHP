/**
 * chronomail Webmail Client Script
 *
...
function chronomail_webmail()
{
  ...
  $.ajaxSetup({
    ...
    beforeSend: function(xmlhttp){ xmlhttp.setRequestHeader('X-chronomail-Request', ref.env.request_token); }
  });
  ...
  this.register_menu_button = function(button, menu_id)
  {
    if (this.menu_buttons[menu_id]) {
      ...
      $('#' + menu_id).find('a').each(function() {
        ...
        if (onclick && String(onclick).match(/chronomail\.command\(\'([^']+)/))
          command = RegExp.$1;
        else
  ...
  {
    this.gui_objects[name] = this.loaded ? chronomail_find_object(id) : id;
  };
  ...
  this.init = function()
  {
    ...
    for (n in this.gui_objects)
      this.gui_objects[n] = chronomail_find_object(this.gui_objects[n]);
    ...
    if (this.is_framed()) {
      parent.chronomail.unlock_frame();
    }
    ...
    switch (this.task) {
      ...
      case 'mail':
        ...
        if (this.gui_objects.messagelist) {
          ...
          this.message_list = new chronomail_list_widget(this.gui_objects.messagelist, {
        ...
        if (this.env.action == 'show' || this.env.action == 'preview') {
          ...
          if (this.env.action == 'preview' && this.is_framed()) {
            ...
            parent.chronomail.show_contentframe(true);
          }
          ...
          if (this.gui_objects.attachments)
            $('li > a', this.gui_objects.attachments).not('.drop').on('dragstart', function(e) {
              ...
              if (dt) {
                ...
                dt.setData('chronomail-uri', href);
                dt.setData('chronomail-name', $.trim(n.text()));
              }
            });
        }
          ...
          if (this.gui_objects.responseslist) {
            $('a.insertresponse', this.gui_objects.responseslist)
              ...
              .mousedown(function(e) { return chronomail_event.cancel(e); })
              .on('mouseup keypress', function(e) {
                if (e.type == 'mouseup' || chronomail_event.get_keycode(e) == 13) {
                  ref.command('insert-response', $(this).attr('rel'));
                  $(document.body).trigger('mouseup');  // hides the menu
                  return chronomail_event.cancel(e);
                }
              });

            // avoid textarea loosing focus when hitting the save-response button/link
            $.each(this.buttons['save-response'] || [], function (i, v) {
              $('#' + v.id).mousedown(function(e){ return chronomail_event.cancel(e); })
            });
          }
        ...
        if (this.gui_objects.contactslist) {
          this.contact_list = new chronomail_list_widget(this.gui_objects.contactslist,
      ...
      case 'addressbook':
        ...
        if (this.gui_objects.savedsearchlist) {
          this.savedsearchlist = new chronomail_treelist_widget(this.gui_objects.savedsearchlist, {
            id_prefix: 'rcmli',
      ...
      case 'settings':
        ...
        if (this.env.action == 'identities') {
          ...
          parent.chronomail.env.exists = this.env.messagecount;
          parent.chronomail.enable_command('purge', this.env.messagecount);
        }
        ...
        if (this.gui_objects.identitieslist) {
          this.identity_list = new chronomail_list_widget(this.gui_objects.identitieslist,
          ...
          this.sections_list = new chronomail_list_widget(this.gui_objects.sectionslist, {multiselect:false, draggable:false, keyboard:true});
          this.sections_list
          ...
          this.responses_list = new chronomail_list_widget(this.gui_objects.responseslist, {multiselect:false, draggable:false, keyboard:true});
          this.responses_list
    ...
    if (this.gui_objects.folderlist && window.chronomail_treelist_widget
      // some plugins may load chronomail_treelist_widget and there's one case
      // when this will cause problems - addressbook widget in compose,
      // which already has been initialized using chronomail_list_widget
      && this.gui_objects.folderlist != this.gui_objects.addressbookslist
    ) {
      this.treelist = new chronomail_treelist_widget(this.gui_objects.folderlist, {
          selectable: true,
          id_prefix: 'rcmli',
          parent_focus: true,
          id_encode: this.html_identifier_encode,
    ...
    chronomail_webmail.set_iframe_events({mouseup: body_mouseup});
  ...
  this.command = function(command, props, obj, event)
  {
    ...
    if (obj && obj.blur && !(event && chronomail_event.is_keyboard(event)))
      obj.blur();
    ...
    if ((obj && obj.href && String(obj.href).indexOf('#') < 0) && chronomail_event.get_modifier(event)) {
      return true;
    }
    ...
    if (!this.commands[command]) {
      ...
      if (this.is_framed())
        parent.chronomail.command(command, props);
        ...
        if (form) {
          ...
          // contacts/identities
            ...
            if (props == 'reload') {
              ...
              (input = $("[name='_email']", form)) && input.length && !chronomail_check_email(input.val())
            ) {
          ...
          if (parent.chronomail && parent.chronomail.env.source)
            form.action = this.add_url(form.action, '_orig_source', parent.chronomail.env.source);
        ...
        var dialog = $('<iframe>').attr('src', this.url('import', {_framed: 1, _target: this.env.source})),
          import_func = function(e) {
            var win = dialog[0].contentWindow,
              form = win.chronomail.gui_objects.importform;

            if (form) {
              var lock, file = win.$('#rcmimportfile')[0];
              if (file && !file.value) {
                win.chronomail.alert_dialog(win.chronomail.get_label('selectimportfile'));
                return;
              }

              lock = win.chronomail.set_busy(true, 'importwait');
              $('[name="_unlock"]', form).val(lock);
              form.submit();
              win.chronomail.lock_form(form, true);
  ...
  {
    if (this.is_framed())
      parent.chronomail.reload(delay);
    else if (delay)
  ...
  {
    return this.env.framed && parent.chronomail && parent.chronomail != this && typeof parent.chronomail.command == 'function';
  };
  ...
  {
    var modkey = chronomail_event.get_modifier(e),
      menu = this.gui_objects.dragmenu;

    if (menu && modkey == SHIFT_KEY && this.commands['copy']) {
      var pos = chronomail_event.get_mouse_pos(e);
  ...
  this.drag_end = function(e)
  {
    ...
    if (this.drag_active && model && this.env.last_folder_target && !chronomail_event.is_keyboard(e)) {
  ...
  this.drag_move = function(e)
  {
    if (this.gui_objects.folderlist) {
      var drag_target, oldclass,
        ...
        mouse = chronomail_event.get_mouse_pos(e);
  ...
  {
    var list, id, target = chronomail_event.get_target(e);
    ...
    if (window.chronomail_list_widget && chronomail_list_widget._instances.length) {
      $.each(chronomail_list_widget._instances, function(i,list) {
        if (list && !chronomail_mouse_is_over(e, list.list.parentNode))
          list.blur();
      });
    }
    ...
    setTimeout(function(e) {
      ...
      for (i = ref.menu_stack.length - 1; i >= 0; i--) {
        ...
        if (obj.is(':visible')
          ...
          && (obj.attr('data-sticky') != 'true' || !chronomail_mouse_is_over(e, obj.get(0)))
        ) {
          ref.hide_menu(id, e);
        }
  ...
  this.doc_keypress = function(e)
  {
    ...
    var target = e.target || {},
      keyCode = chronomail_event.get_keycode(e);
    ...
    switch (keyCode) {
      ...
      case 9:   // tab
        if (this.focused_menu) {
          var mod = chronomail_event.get_modifier(e);
  ...
  this.init_message_row = function(row)
  {
    ...
    $.each(fn, function(i, f) {
      row[i].onclick = function(e) { f(e); return chronomail_event.cancel(e); };
  ...
  this.add_message_row = function(uid, cols, flags, attop)
  {
    ...
    if (cols.subject) {
      ...
      query[uid_param] = uid;
      cols.subject = '<a href="' + this.url(action, query) + '" onclick="return chronomail_event.keyboard_only(event)"' +
        ' onmouseover="chronomail_webmail.long_subject_title(this,'+(message.depth+1)+')" tabindex="-1"><span>'+cols.subject+'</span></a>';
    }
    ...
    for (n in this.env.listcols) {
      ...
      if (c == 'flag') {
        ...
        html = '<span onmouseover="chronomail_webmail.long_subject_title(this)">' + cols[c] + '<span>';
      }
      else
        html = cols[c];
  ...
  {
    var rc = this.is_framed() ? parent.chronomail : this;
  ...
  this.delete_messages = function(event)
  {
    ...
    // if there is a trash mailbox defined and we're not currently in it
      ...
      if ((list && list.modkey == SHIFT_KEY) || (event && chronomail_event.get_modifier(event) == SHIFT_KEY)) {
  ...
  this.mailvelope_submit_messageform = function(draft, saveonly)
  {
    ...
    $.each(['to', 'cc', 'bcc'], function(i,field) {
      ...
      while (val.length && chronomail_check_email(val, true)) {
        rcpt = RegExp.$2;
  ...
  this.bounce = function(props, obj, event)
  {
    ...
    var uid = this.get_single_uid(),
      ...
      get_form = function() {
        var rc = $('iframe', dialog)[0].contentWindow.chronomail;
  ...
  {
    var key = chronomail_event.get_keycode(e),
      passwd = $('#rcmloginpwd');
  ...
  this.check_compose_address_fields = function(cmd, form)
  {
    ...
    if (input_from.prop('type') == 'text' && !chronomail_check_email(input_from.val(), true)) {
    ...
    if (!chronomail_check_email(get_recipients([input_to, input_cc, input_bcc]), true)) {
    ...
    if (limit && !this.env.disclosed_recipients_warned
      && chronomail_check_email(recipients = get_recipients([input_to, input_cc]), true, true) > limit
    ) {
  ...
  this.add2attachment_list = function(name, att, upload_id)
  {
    ...
    if (!att.complete) {
      ...
      att.html = '<a title="'+label+'" onclick="return chronomail.cancel_attachment_upload(\''+name+'\');" href="#cancelupload" class="cancelupload">'
    ...
    li.attr('id', name).addClass(att.classname).html(att.html)
      .find('.attachment-name').on('mouseover', function() { chronomail_webmail.long_subject_title_ex(this); });
  ...
  this.ksearch_keydown = function(e, obj, props)
  {
    ...
    switch (key) {
      ...
      case 9:   // tab
        if (chronomail_event.get_modifier(e) == SHIFT_KEY || !this.ksearch_visible()) {
          this.ksearch_hide();
          return;
        }
      ...
      case 13:  // enter
        ...
        return key == 9 ? null : chronomail_event.cancel(e);
  ...
  this.contacts_drag_menu = function(e, to)
  {
    ...
    else if (!this.commands.move && chronomail_event.get_modifier(e) != SHIFT_KEY) {
  ...
  this.advanced_search = function()
  {
    var dialog = $('<iframe>').attr('src', this.url('search', {_form: 1, _framed: 1})),
      search_func = function() {
        ...
        $.each($(dialog[0].contentWindow.chronomail.gui_objects.editform).serializeArray(), function() {
  ...
  this.init_subscription_list = function()
  {
    ...
    this.subscription_list = new chronomail_treelist_widget(this.gui_objects.subscriptionlist, {
        selectable: true,
        tabexit: false,
        parent_focus: true,
        id_prefix: 'rcmli',
  ...
  this.replace_folder_row = function(oldid, id, name, display_name, is_protected, class_name)
  {
    if (!this.gui_objects.subscriptionlist) {
      if (this.is_framed()) {
        ...
        return window.parent.chronomail.replace_folder_row(oldid, id, name, display_name, is_protected, class_name);
      }
  ...
  this.display_message = function(msg, type, timeout, key)
  {
    ...
    // pass command to parent window
    if (this.is_framed())
      return parent.chronomail.display_message(msg, type, timeout);
  ...
  this.hide_message = function(obj, fade)
  {
    // pass command to parent window
    if (this.is_framed())
      return parent.chronomail.hide_message(obj, fade);
  ...
  this.clear_messages = function()
  {
    // pass command to parent window
    if (this.is_framed())
      return parent.chronomail.clear_messages();
  ...
  this.show_popup_dialog = function(content, title, buttons, options)
  {
    ...
    if (this.is_framed()) {
      return parent.chronomail.show_popup_dialog(content, title, buttons, options);
    }
  ...
  this.show_menu = function(prop, show, event)
  {
    var name = typeof prop == 'object' ? prop.menu : prop,
      ...
      keyboard = chronomail_event.is_keyboard(event),
  ...
  this.hide_menu = function(name, event)
  {
    ...
    var obj, keyboard = chronomail_event.is_keyboard(event);
  ...
  {
    this.editor = new chronomail_text_editor(config, id);
  };
  ...
  this.redirect = function(url, lock)
  {
    ...
    if (this.is_framed()) {
      url = url.replace(/&_framed=1/, '');
      parent.chronomail.redirect(url, lock);
    }
    else {
      if (this.env.extwin) {
  ...
  this.update_state = function(query)
  {
    if (window.history.replaceState)
      try {
        ...
        window.history.replaceState({}, document.title, chronomail.url('', query));
      }
      catch(e) { /* ignore */ };
  };
  ...
  this.http_request = function(action, data, lock, type)
  {
    ...
    if (typeof data !== 'object')
      data = chronomail_parse_query(data);
  ...
  */
  ...
  this.file_dropped = function(e)
  {
    ...
    if (!files || !files.length) {
      // chronomail attachment, pass its uri to the backend and attach
      if (uri = e.dataTransfer.getData('chronomail-uri')) {
        var ts = 'upload' + new Date().getTime(),
          // jQuery way to escape filename (#1490530)
          content = $('<span>').text(e.dataTransfer.getData('chronomail-name') || this.get_label('attaching')).html();
  ...
  this.file_upload = function(files, post_args, props)
  {
    ...
    if (numfiles) {
      ...
      this.uploads[ts] = $.ajax({
        ...
        data: formdata,
        headers: {'X-chronomail-Request': this.env.request_token},
        xhr: function() {
  ...
  // get window.opener.chronomail if available
  this.opener = function(deep, filter)
  {
    var i, win = window.opener;

    // catch Error: Permission denied to access property chronomail
    try {
      if (win && !win.closed && win !== window) {
        // try parent of the opener window, e.g. preview frame
        if (deep && (!win.chronomail || win.chronomail.env.framed) && win.parent && win.parent.chronomail)
          win = win.parent;

        if (win.chronomail && filter)
          for (i in filter)
            if (win.chronomail.env[i] != filter[i])
              return;

        return win.chronomail;
      }
    }
    catch (e) {}
  };
  ...
  this.get_local_storage_prefix = function()
  {
    if (!this.local_storage_prefix)
      this.local_storage_prefix = 'chronomail.' + (this.env.user_id || 'anonymous') + '.';
...
}  // end object chronomail_webmail


// some static methods
chronomail_webmail.long_subject_title = function(elem, indent, text_elem)
{
  if (!elem.title) {
    var $elem = $(text_elem || elem);
    if ($elem.width() + (indent || 0) * 15 > $elem.parent().width())
      elem.title = chronomail_webmail.subject_text($elem[0]);
  }
};

chronomail_webmail.long_subject_title_ex = function(elem)
{
  if (!elem.title) {
    var $elem = $(elem),
      txt = $.trim($elem.text()),
    ...
    if (w + indent * 15 > $elem.width())
      elem.title = chronomail_webmail.subject_text(elem);
  }
};

chronomail_webmail.subject_text = function(elem)
{
  var t = $(elem).clone();
...
chronomail_webmail.set_iframe_events = function(events)
{
...
chronomail_webmail.prototype.get_cookie = getCookie;

// copy event engine prototype
chronomail_webmail.prototype.addEventListener = chronomail_event_engine.prototype.addEventListener;
chronomail_webmail.prototype.removeEventListener = chronomail_event_engine.prototype.removeEventListener;
chronomail_webmail.prototype.triggerEvent = chronomail_event_engine.prototype.triggerEvent;
