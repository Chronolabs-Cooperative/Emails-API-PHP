...
/*
 ...
 |                                                                       |
 | PURPOSE:                                                              |
 |   Update an existing chronomail installation with files from           |
 |   this version                                                        |
 +-----------------------------------------------------------------------+
...
if (!function_exists('system')) {
    chronomail::raise_error("PHP system() function is required. Check disable_functions in php.ini.", false, true);
}
...
if (empty($target_dir) || !is_dir(realpath($target_dir)))
    chronomail::raise_error("Invalid target: not a directory\nUsage: installto.sh [-y] <TARGET>", false, true);
...
if (!preg_match('/define\(.RCMAIL_VERSION.,\s*.([0-9.]+[a-z0-9-]*)/', $iniset, $m))
    chronomail::raise_error("No valid chronomail installation found at $target_dir", false, true);

$oldversion = $m[1];

if (version_compare(version_parse($oldversion), version_parse(RCMAIL_VERSION), '>'))
    chronomail::raise_error("Target installation already in version $oldversion.", false, true);
...
if (strtolower($input) == 'y') {
    ...
    foreach ($dirs as $dir) {
        ...
        if (system($command, $ret) === false || $ret > 0) {
            chronomail::raise_error("Failed to execute command: $command", false, true);
        }
    }
