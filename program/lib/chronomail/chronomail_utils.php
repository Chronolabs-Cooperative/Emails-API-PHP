...
class chronomail_utils
{
    ...
    public static function check_email($email, $dns_check=true)
    {
        ...
        if (preg_match('/^\[((IPv6:[0-9a-f:.]+)|([0-9.]+))\]$/i', $domain_part, $matches)) {
            ...
            if (!$dns_check || !function_exists('checkdnsrr') || !$chronomail->config->get('email_dns_check')) {
                return true;
            }
    ...
    public static function parse_input_value($value, $allow_html = false, $charset = null)
    {
        ...
        if ($output_charset == 'UTF-8') {
            $value = chronomail_charset::clean($value);
        }

        // convert to internal charset
        if ($charset && $output_charset) {
            $value = chronomail_charset::convert($value, $output_charset, $charset);
        }
     ...
     * Same functionality as done in app.js with chronomail_webmail.html_identifier()
     *
    ...
    {
        $last_pos     = 0;
        $replacements = new chronomail_string_replacer;
        ...
        if ($container_id || $prefix) {
            ...
            // from chronomail_string_replacer used above, this is needed for
    ...
    public static function https_check($port = null, $use_https = true)
    {
        ...
        if (!empty($_SERVER['HTTP_X_FORWARDED_PROTO'])
            ...
            && in_array($_SERVER['REMOTE_ADDR'], chronomail::get_instance()->config->get('proxy_whitelist', array()))
        ) {
            return true;
        }
        ...
        if ($use_https && chronomail::get_instance()->config->get('use_https')) {
            return true;
        }
    ...
    public static function server_name($type = null, $strip_port = true)
    {
        ...
        $chronomail    = chronomail::get_instance();
        $patterns = (array) $chronomail->config->get('trusted_host_patterns');
        ...
        if (!empty($name)) {
            ...
            $chronomail->raise_error(array('file' => __FILE__, 'line' => __LINE__,
    ...
    public static function remote_addr()
    {
        ...
        if (!empty($_SERVER['HTTP_X_FORWARDED_FOR']) || !empty($_SERVER['HTTP_X_REAL_IP'])) {
            $proxy_whitelist = chronomail::get_instance()->config->get('proxy_whitelist', array());
    ...
    public static function normalize_string($str, $as_array = false, $minlen = 2)
    {
        ...
        else if (chronomail_charset::convert(chronomail_charset::convert($str, 'UTF-8', 'ISO-8859-2'), 'ISO-8859-2', 'UTF-8') == $str) {
            // ISO-8859-2
    ...
    public static function preg_error($error = array(), $terminate = false)
    {
        if (($preg_error = preg_last_error()) != PREG_NO_ERROR) {
            ...
            chronomail::raise_error($error, true, $terminate);
    ...
    /**
     * Generate a temporary file path in the chronomail temp directory
     *
    ...
    {
        $temp_dir = chronomail::get_instance()->config->get('temp_dir');
