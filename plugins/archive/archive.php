 ...
 */
class archive extends chronomail_plugin
{
    ...
    function init()
    {
        ...
        chronomail_storage::$folder_types[] = 'archive';

        $this->archive_folder = $chronomail->config->get('archive_mbox');

        if ($chronomail->task == 'mail' && ($chronomail->action == '' || $chronomail->action == 'show') && $this->archive_folder) {
            ...
            $chronomail->output->set_env('archive_folder', $this->archive_folder);
            $chronomail->output->set_env('archive_type', $chronomail->config->get('archive_type',''));
        }
        else if ($chronomail->task == 'mail') {
            ...
            if ($chronomail->action == 'folders' && $this->archive_folder) {
                $this->include_stylesheet($this->local_skin_path() . '/archive.css');
                $this->include_script('archive.js');
                // set env variables for client
                $chronomail->output->set_env('archive_folder', $this->archive_folder);
    ...
    function render_mailboxlist($p)
    {
        ...
        if ($this->archive_folder && !chronomail::get_instance()->config->get('show_real_foldernames')) {
    ...
    function move_messages()
    {
        ...
        if (!$chronomail->output->ajax_call) {
            return;
        }
        ...
        $read_on_move   = (bool) $chronomail->config->get('read_on_archive');
        $archive_type   = $chronomail->config->get('archive_type', '');
        ...
        foreach (chronomail::get_uids(null, null, $multifolder, chronomail_utils::INPUT_POST) as $mbox => $uids) {
            if (!$this->archive_folder || $mbox === $this->archive_folder || strpos($mbox, $archive_prefix) === 0) {
                ...
                if ($uids == '*') {
                    $index = $storage->index(null, chronomail_sort_column(), chronomail_sort_order());
                    $uids  = $index->get();
                }
                ...
                foreach ($messages as $message) {
                    ...
                    switch ($archive_type) {
                    case 'year':
                        $subfolder = $chronomail->format_date($message->timestamp, 'Y');
                        break;

                    case 'month':
                        $subfolder = $chronomail->format_date($message->timestamp, 'Y')
                            . $delimiter . $chronomail->format_date($message->timestamp, 'm');
                        break;

                    case 'tbmonth':
                        $subfolder = $chronomail->format_date($message->timestamp, 'Y')
                            . $delimiter . $chronomail->format_date($message->timestamp, 'Y')
                            . '-' . $chronomail->format_date($message->timestamp, 'm');
                        break;
        ...
        if ($this->result['error']) {
            if ($_POST['_from'] != 'show') {
                $chronomail->output->command('list_mailbox');
            }

            $chronomail->output->show_message($this->gettext('archiveerror'), 'warning');
            $chronomail->output->send();
        }

        if (!empty($_POST['_refresh'])) {
            // FIXME: send updated message rows instead of reloading the entire list
            $chronomail->output->command('refresh_list');
        }
        else {
            $addrows = true;
        }
        ...
        if ($search_request && $chronomail->storage->get_search_set()) {
            $_SESSION['search'] = $chronomail->storage->refresh_search();
        }

        if ($_POST['_from'] == 'show') {
            if ($next = chronomail_utils::get_input_value('_next_uid', chronomail_utils::INPUT_GPC)) {
                $chronomail->output->command('show_message', $next);
            }
            else {
                $chronomail->output->command('command', 'list');
            }
        ...
        foreach ($this->result['sources'] as $folder) {
            chronomail_send_unread_count($folder, true);
        }

        // update message count display
        $chronomail->output->set_env('messagecount', $msg_count);
        $chronomail->output->set_env('current_page', $page);
        $chronomail->output->set_env('pagecount', $pages);
        $chronomail->output->set_env('exists', $exists);
        $chronomail->output->command('set_quota', $chronomail->quota_content(null, $quota_root));
        $chronomail->output->command('set_rowcount', chronomail_get_messagecount_text($msg_count), $mbox);
        ...
        if ($this->result['reload']) {
            $chronomail->output->show_message($this->gettext('archivedreload'), 'confirmation');
        }
        else {
            $chronomail->output->show_message($this->gettext('archived'), 'confirmation');
    ...
    {
        $storage = chronomail::get_instance()->get_storage();
    ...
    function prefs_table($args)
    {
        ...
        if ($args['section'] == 'folders' && !in_array('archive_mbox', $dont_override)) {
            $mbox = $chronomail->config->get('archive_mbox');
            $type = $chronomail->config->get('archive_type');

            // load folders list when needed
            if ($CURR_SECTION) {
                $select = $chronomail->folder_selector(array(
                        'noselection'   => '---',
                        'realnames'     => true,
                        'maxlength'     => 30,
                        'folder_filter' => 'mail',
            ...
            $args['blocks']['main']['options']['archive_mbox'] = array(
                'title'   => html::label('_archive_mbox', chronomail::Q($this->gettext('archivefolder'))),
            ...
            if ($chronomail->get_storage()->get_capability(chronomail_storage::DUAL_USE_FOLDERS)) {
                ...
                $args['blocks']['archive'] = array(
                    'name'    => chronomail::Q($this->gettext('settingstitle')),
                    'options' => array('archive_type' => array(
                            'title'   => html::label('ff_archive_type', chronomail::Q($this->gettext('archivetype'))),
            ...
            $args['blocks']['main']['options']['read_on_archive'] = array(
                'title'   => html::label('ff_read_on_archive', chronomail::Q($this->gettext('readonarchive'))),
                'content' => $chbox->show($chronomail->config->get('read_on_archive') ? 1 : 0)
            );
        }
    ...
    function save_prefs($args)
    {
        ...
        if ($args['section'] == 'folders' && !in_array('archive_mbox', $dont_override)) {
            $args['prefs']['archive_type'] = chronomail_utils::get_input_value('_archive_type', chronomail_utils::INPUT_POST);
        }
        else if ($args['section'] == 'server' && !in_array('read_on_archive', $dont_override)) {
            $args['prefs']['read_on_archive'] = (bool) chronomail_utils::get_input_value('_read_on_archive', chronomail_utils::INPUT_POST);
        }
    ...
    protected function sender_subfolder($from)
    {
        ...
        if ($delim === null) {
            ...
            $skip_hidden = $chronomail->config->get('imap_skip_hidden_folders');
        }
