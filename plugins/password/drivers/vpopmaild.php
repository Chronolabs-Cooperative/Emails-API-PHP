...
class chronomail_vpopmaild_password
{
    function save($curpass, $passwd, $username)
    {
        $chronomail    = chronomail::get_instance();
        $vpopmaild = new Net_Socket();
        $host      = $chronomail->config->get('password_vpopmaild_host');
        $port      = $chronomail->config->get('password_vpopmaild_port');
        ...
        $vpopmaild->setTimeout($chronomail->config->get('password_vpopmaild_timeout'),0);
