 ...
 */
class squirrelmail_usercopy extends chronomail_plugin
{
    public $task = 'login';
    ...
    public function create_user($p)
    {
        ...
        if (($this->identities_level == 0 || $this->identities_level == 2)
            && $chronomail->config->get('squirrelmail_set_alias')
        ...
        if ($this->prefs) {
            ...
            if (($this->identities_level == 0 || $this->identities_level == 1)
                ...
                for ($i = 1; $i < $this->prefs['identities']; $i++) {
                    ...
                    // insert identity
                    $chronomail->user->insert_identity($ident_data);
                }
            }

            // copy address book
            $contacts  = $chronomail->get_address_book(null, true);
            $addresses = array();
            $groups    = array();
            ...
            if ($contacts && !empty($this->abook)) {
                foreach ($this->abook as $rec) {
                    ...
                    foreach ($emails as $email) {
                        if (!($contact_id = $addresses[$email]) && chronomail_utils::check_email(chronomail_utils::idn_to_ascii($email))) {
                            $rec['email'] = chronomail_utils::idn_to_utf8($email);
    ...
    private function initialize()
    {
        ...
        $ilevel = $chronomail->config->get('squirrelmail_identities_level');
        if ($ilevel === null) {
            $ilevel = $chronomail->config->get('identities_level', 0);
        }
    ...
    private function read_squirrel_prefs($uname)
    {
        ...
        /**** File based backend ****/
        if ($chronomail->config->get('squirrelmail_driver') == 'file' && ($srcdir = $chronomail->config->get('squirrelmail_data_dir'))) {
            if (($hash_level = $chronomail->config->get('squirrelmail_data_dir_hash_level')) > 0) {
                $srcdir = slashify($srcdir).chunk_split(substr(base_convert(crc32($uname), 10, 16), 0, $hash_level), 1, '/');
            }
            $file_charset = $chronomail->config->get('squirrelmail_file_charset');
        ...
        // Database backend
        else if ($chronomail->config->get('squirrelmail_driver') == 'sql') { 
            $this->prefs = array();

            // connect to squirrelmail database
            $db = chronomail_db::factory($chronomail->config->get('squirrelmail_dsn'));

            $db->set_debug($chronomail->config->get('sql_debug'));
            $db->db_connect('r'); // connect in read mode

            // retrieve prefs
            $userprefs_table = $chronomail->config->get('squirrelmail_userprefs_table');
            $address_table   = $chronomail->config->get('squirrelmail_address_table');
            $db_charset      = $chronomail->config->get('squirrelmail_db_charset');
            ...
            while ($sql_array = $db->fetch_assoc($sql_result) ) { // fetch one row from result
                $this->prefs[$sql_array['prefkey']] = chronomail_charset::convert(rtrim($sql_array['prefval']), $db_charset);
            }
            ...
            // parse address book
            while ($sql_array = $db->fetch_assoc($sql_result) ) { // fetch one row from result
                $rec['name']      = chronomail_charset::convert(rtrim($sql_array['nickname']), $db_charset);
                $rec['firstname'] = chronomail_charset::convert(rtrim($sql_array['firstname']), $db_charset);
                $rec['surname']   = chronomail_charset::convert(rtrim($sql_array['lastname']), $db_charset);
                $rec['email']     = chronomail_charset::convert(rtrim($sql_array['email']), $db_charset);
                $rec['notes']     = chronomail_charset::convert(rtrim($sql_array['label']), $db_charset);
    ...
    private function convert_charset($str, $charset = null)
    {
        ...
        return chronomail_charset::convert($str, $charset, CHRONOMAIL_CHARSET);
    }
}
