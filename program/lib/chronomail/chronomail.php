...
/**
 * Base class of the chronomail Framework
...
class chronomail
{
    ...
    /** @var chronomail_config Stores instance of chronomail_config */
    public $config;

    /** @var chronomail_db Instance of database class */
    public $db;
    ...
    /** @var chronomail_session Instance of chronomail_session class */
    public $session;

    /** @var chronomail_smtp Instance of chronomail_smtp class */
    public $smtp;

    /** @var chronomail_storage Instance of chronomail_storage class */
    public $storage;

    /** @var chronomail_output Instance of chronomail_output class */
    public $output;

    /** @var chronomail_plugin_api Instance of chronomail_plugin_api */
    public $plugins;

    /** @var chronomail_user Instance of chronomail_user class */
    public $user;
    ...
    /** @var chronomail_cache[] Initialized cache objects */
    protected $caches = array();

    /** @var array Registered shutdown functions */
    protected $shutdown_functions = array();

    /** @var chronomail Singleton instance of chronomail */
     ...
     *
     * @param int    $mode Options to initialize with this instance. See chronomail::INIT_WITH_* constants
    ...
    {
        // load configuration
        $this->config  = new chronomail_config($env);
        $this->plugins = new chronomail_dummy_plugin_api;
    ...
    protected function init($mode = 0)
    {
        ...
        if ($this->config->get('log_driver') == 'syslog') {
            $syslog_id       = $this->config->get('syslog_id', 'chronomail');
        ...
        if ($mode & self::INIT_WITH_PLUGINS) {
            $this->plugins = chronomail_plugin_api::get_instance();
        }
    }
     ...
     *
     * @return chronomail_db Database object
     */
    public function get_dbh()
    {
        if (!$this->db) {
            $this->db = chronomail_db::factory(
    ...
    public function get_memcache()
    {
        if (!isset($this->memcache)) {
            $this->memcache = chronomail_cache_memcache::engine();
        }
    ...
    public function get_memcached()
    {
        if (!isset($this->memcached)) {
            $this->memcached = chronomail_cache_memcached::engine();
        }
    ...
    {
        if (!isset($this->redis)) {
            $this->redis = chronomail_cache_redis::engine();
        }
     ...
     *
     * @return chronomail_cache User cache object
     */
    public function get_cache($name, $type='db', $ttl=0, $packed=true)
    {
        if (!isset($this->caches[$name]) && ($userid = $this->get_user_id())) {
            $this->caches[$name] = chronomail_cache::factory($type, $userid, $name, $ttl, $packed);
        }
     ...
     * @param bool   $packed Enables/disables data serialization
     *
     * @return chronomail_cache Shared cache object
     */
    public function get_cache_shared($name, $packed = true)
    {
        ...
        if (!array_key_exists($shared_name, $this->caches)) {
            ...
            $this->caches[$shared_name] = chronomail_cache::factory($type, null, $name, $ttl, $packed);
        }
     ...
     *
     * @return chronomail_storage Storage object
     */
    ...
    public function storage_init()
    {
        ...
        $driver_class = "chronomail_{$driver}";
    ...
    public function session_init()
    {
        ...
        $is_secure   = $this->config->get('use_https') || chronomail_utils::https_check();
        ...
        ini_set('session.name', $sess_name ?: 'chronomail_sessid');
        ...
        // Start the session
        $this->session = chronomail_session::factory($this->config);
    ...
    {
        static $chronomail_languages, $chronomail_language_aliases;
        ...
        if (empty($chronomail_languages)) {
            @include(RCUBE_LOCALIZATION_DIR . 'index.inc');
        }

        // check if we have an alias for that language
        if (!isset($chronomail_languages[$lang]) && isset($chronomail_language_aliases[$lang])) {
            $lang = $chronomail_language_aliases[$lang];
        }
        // try the first two chars
        else if (!isset($chronomail_languages[$lang])) {
            $short = substr($lang, 0, 2);

            // check if we have an alias for the short language code
            if (!isset($chronomail_languages[$short]) && isset($chronomail_language_aliases[$short])) {
                $lang = $chronomail_language_aliases[$short];
            }
            // expand 'nn' to 'nn_NN'
            else if (!isset($chronomail_languages[$short])) {
                $lang = $short.'_'.strtoupper($short);
            }
        }

        if (!isset($chronomail_languages[$lang]) || !is_dir(RCUBE_LOCALIZATION_DIR . $lang)) {
            $lang = 'en_US';
        }
        ...
        if (!count($sa_languages)) {
            ...
            if ($dh = @opendir(RCUBE_LOCALIZATION_DIR)) {
                while (($name = readdir($dh)) !== false) {
                    ...
                    if ($label = $chronomail_languages[$name]) {
    ...
    public function encrypt($clear, $key = 'des_key', $base64 = true)
    {
        ...
        $iv     = chronomail_utils::random_bytes(openssl_cipher_iv_length($method), true);
    ...
    public function get_secure_url_token($generate = false)
    {
        if ($len = $this->config->get('use_secure_urls')) {
            if (empty($_SESSION['secure_token']) && $generate) {
                ...
                $length = $len > 1 ? $len : 16;
                $token  = chronomail_utils::random_bytes($length);
    ...
    public function get_request_token()
    {
        if (empty($_SESSION['request_token'])) {
            $plugin = $this->plugins->exec_hook('request_token', array(
                'value' => chronomail_utils::random_bytes(32)));
     ...
     */
    public function check_request($mode = chronomail_utils::INPUT_POST)
    {
        ...
        // ajax requests
        if (chronomail_utils::request_header('X-chronomail-Request') === $sess_tok) {
            return true;
        }

        // skip empty requests
        if (($mode == chronomail_utils::INPUT_POST && empty($_POST))
            || ($mode == chronomail_utils::INPUT_GET && empty($_GET))
        ) {
            return true;
        }

        // default method of securing requests
        $token   = chronomail_utils::get_input_value('_token', $mode);
     ...
     * Shortcut function for chronomail_utils::rep_specialchars_output()
     *
    ...
    {
        return chronomail_utils::rep_specialchars_output($str, 'html', $mode, $newlines);
    }
    ...
    {
        return chronomail_utils::rep_specialchars_output($str, 'js');
    }
    ...
    public static function console()
    {
        ...
        if (class_exists('chronomail', false)) {
            $chronomail  = self::get_instance();
            $plugin = $chronomail->plugins->exec_hook('console', array('args' => $args));
            if ($plugin['abort']) {
                return;
            }
    ...
    public static function write_log($name, $line)
    {
        ...
        $date = chronomail_utils::date_format($date_format);
    ...
    public static function raise_error($arg = array(), $log = false, $terminate = false)
    {
        ...
        // installer
        if (!$cli && class_exists('chronomail_install', false)) {
            $rci = chronomail_install::get_instance();
            $rci->raise_error($arg);
            return;
        }
     ...
     *
     * @param chronomail_user Current user instance
     */
    ...
    {
        $local_part  = md5(uniqid('chronomail'.mt_rand(), true));
        $domain_part = '';
        ...
        &$body_file = null, $options = null, $disconnect = false)
    {
        ...
        if ($message->getParam('delay_file_io')) {
            // use common temp dir
            $body_file   = chronomail_utils::temp_filename('msg');
        ...
        if (!$sent) {
            ...
            if ($this->config->get('smtp_log')) {
                ...
                $mailto = chronomail_mime::decode_address_list($mailto, null, false, null, true);

                self::write_log('sendmail', sprintf("User %s [%s]; Message %s for %s; %s",
                    $this->user->get_username(),
                    chronomail_utils::remote_addr(),
                    $headers['Message-ID'],
                    implode(', ', $mailto),
 ...
 * @subpackage Core
 */
class chronomail_dummy_plugin_api
{
    /**
     ...
     * @see chronomail_plugin_api::exec_hook()
     */
