...
{
    var ref = this,
        cookie_value = chronomail.get_cookie('language');
    ...
    this.show_change_lang_pic = false; // chronomail mod.
    ...
    this.use_ok_pic = false; // added by chronomail
    this.show_spell_img = false; // chronomail mod.
...
this.decorateTextarea = function(id)
{
    ...
    if (this.text_area) {
        ...
        chronomail.alert_dialog('Text area not found');
    }
};
...
this.setCurrentLanguage = function(lan_code)
{
    ...
    chronomail.set_cookie('language', lan_code, now);
};
    ...
    $.ajax({ type: 'POST', url: this.getUrl(), data: this.createXMLReq(req_text), dataType: 'text',
        error: function(o) {
            if (ref.custom_ajax_error) {
                ...
                chronomail.alert_dialog('An error was encountered on the server. Please try again later.');
            }
...
this.showErrorWindow = function(elm, id)
{
    ...
    // chronomail plugin api hook
    chronomail.triggerEvent('googiespell_create', {obj: this.error_window});
...
this.removeIndicator = function(elm)
{
    ...
    if (window.chronomail)
        chronomail.set_busy(false, null, this.rc_msg_id);
};

this.appendIndicator = function(elm)
{
    // modified by chronomail
    if (window.chronomail)
        this.rc_msg_id = chronomail.set_busy(true, 'checking');
/*
