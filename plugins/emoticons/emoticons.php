 ...
 */
class emoticons extends chronomail_plugin
{
    ...
    function message_part_after($args)
    {
        if ($args['type'] == 'plain') {
            ...
            $chronomail = chronomail::get_instance();
            if (!$chronomail->config->get('emoticons_display', false)) {
                return $args;
            }
    ...
    function message_outgoing_body($args)
    {
        if ($args['type'] == 'html') {
            ...
            $chronomail = chronomail::get_instance();
            if (!$chronomail->config->get('emoticons_compose', true)) {
                return $args;
            }
    ...
    function html2text($args)
    {
        ...
        if ($chronomail->action == 'html2text' || $chronomail->action == 'send') {
            $this->load_config();
    ...
    function preferences_list($args)
    {
        ...
        if ($args['section'] == 'mailview' && !in_array('emoticons_display', $dont_override)) {
            ...
            $args['blocks']['main']['options']['emoticons_display'] = array(
                    ...
                    'content' => $checkbox->show(intval($chronomail->config->get('emoticons_display', false)))
            );
        }
            ...
            $args['blocks']['main']['options']['emoticons_compose'] = array(
                    ...
                    'content' => $checkbox->show(intval($chronomail->config->get('emoticons_compose', true)))
            );
        }
    ...
    function preferences_save($args)
    {
        ...
        if ($args['section'] == 'mailview' && !in_array('emoticons_display', $dont_override)) {
            $args['prefs']['emoticons_display'] = chronomail_utils::get_input_value('_emoticons_display', chronomail_utils::INPUT_POST) ? true : false;
        }
        else if ($args['section'] == 'compose' && !in_array('emoticons_compose', $dont_override)) {
            $args['prefs']['emoticons_compose'] = chronomail_utils::get_input_value('_emoticons_compose', chronomail_utils::INPUT_POST) ? true : false;
        }
