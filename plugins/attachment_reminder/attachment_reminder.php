...
class attachment_reminder extends chronomail_plugin
{
    ...
    function init()
    {
        ...
        if ($chronomail->task == 'mail' && $chronomail->action == 'compose') {
            if ($chronomail->config->get('attachment_reminder')) {
                $this->include_script('attachment_reminder.js');
                $this->add_texts('localization/', array('keywords', 'forgotattachment', 'missingattachment'));
                $chronomail->output->add_label('addattachment', 'send');
            }
        }
    ...
    function prefs_list($args)
    {
        if ($args['section'] == 'compose') {
            ...
            $reminder = chronomail::get_instance()->config->get('attachment_reminder');
            ...
            $args['blocks']['main']['options']['attachment_reminder'] = array(
                'title' => html::label($field_id, chronomail::Q($this->gettext('reminderoption'))),
