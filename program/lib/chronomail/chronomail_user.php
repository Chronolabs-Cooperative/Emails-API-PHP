...
class chronomail_user
{
    ...
    /** @var chronomail_db Holds database connection */
    private $db;

    /** @var chronomail Framework object */
    private $rc;
    ...
    function save_prefs($a_user_prefs, $no_session = false)
    {
        ...
        // Warning: we use result of chronomail_config::all() here instead of just get() (#5782)
    ...
    function get_hash()
    {
        ...
        if (empty($prefs['client_hash'])) {
            $prefs['client_hash'] = chronomail_utils::random_bytes(16);
    ...
    function list_identities($sql_add = '', $formatted = false)
    {
        ...
        while ($sql_arr = $this->db->fetch_assoc($sql_result)) {
            if ($formatted) {
                ...
                $utf8_email  = format_email(chronomail_utils::idn_to_utf8($ascii_email));
     ...
     *
     * @return chronomail_user New user instance
     */
    static function query($user, $host)
    {
        $dbh    = chronomail::get_instance()->get_dbh();
        ...
        if ($sql_arr) {
            return new chronomail_user($sql_arr['user_id'], $sql_arr);
        }
    }

    /**
     * Create a new user record and return a chronomail_user instance
     *
     * @param string $user IMAP user name
    ...
    static function create($user, $host)
    {
        ...
        $data = $chronomail->plugins->exec_hook('user_create', array(
            'host'       => $host,
            'user'       => $user,
            'user_name'  => $user_name,
        ...
        if ($dbh->affected_rows($insert) && ($user_id = $dbh->insert_id('users'))) {
            // create chronomail_user instance to make plugin hooks work
            $user_instance = new chronomail_user($user_id, array(
                'user_id'   => $user_id,
                'username'  => $data['user'],
                'mail_host' => $data['host'],
                'language'  => $data['language'],
            ));
            $chronomail->user = $user_instance;
            $mail_domain = $chronomail->config->mail_domain($data['host']);
            ...
            else if (count($email_list) > 1 && $chronomail->config->get('identities_level', 0) > 1) {
            ...
            foreach ($email_list as $row) {
                ...
                $plugin = $chronomail->plugins->exec_hook('identity_create',
                    array('login' => true, 'record' => $record));

                if (!$plugin['abort'] && $plugin['record']['email']) {
                    $chronomail->user->insert_identity($plugin['record']);
                }
                $standard = 0;
            }
        }
    ...
    {
        $chronomail = chronomail::get_instance();
        $plugin = $chronomail->plugins->exec_hook('email2user',
    ...
    {
        $chronomail = chronomail::get_instance();
        $plugin = $chronomail->plugins->exec_hook('user2email',
