...
class enigma_engine
{
    ...
    function sign_message(&$message, $mode = null)
    {
        ...
        if ($pgp_mode == Crypt_GPG::SIGN_MODE_CLEAR) {
            ...
            if (strpos($text_charset, 'format=flowed')) {
                ...
                $body = chronomail_mime::unfold_flowed($body);
    ...
    function attach_public_key(&$message)
    {
        ...
        $from    = chronomail_mime::decode_address_list($headers['From'], 1, false, null, true);
        $from    = $from[1];
    ...
    {
        $keyid  = chronomail_utils::get_input_value('_keyid', chronomail_utils::INPUT_POST);
        $passwd = chronomail_utils::get_input_value('_passwd', chronomail_utils::INPUT_POST, true);
     ...
     *
     * @param chronomail_message      Message object
     * @param chronomail_message_part Message part
     */
    private function get_part_body($msg, $part)
    {
        ...
        // chronomail_message::get_part_body() as the body may be already in memory
        if (!$part->mime_id) {
            ...
            if ($part->charset && stripos($part->charset, 'ASCII') === false) {
                $body = chronomail_charset::convert($body, $part->charset, 'US-ASCII');
            }
        }
    ...
    private function parse_body(&$body)
    {
        ...
        $struct = chronomail_mime::parse_message($body);
     ...
     * @param chronomail_message_part Part structure
     * @param int                Part size
     */
    private function modify_structure(&$p, $struct, $size = 0)
    {
        ...
        // set some part params used by Chronomail core
        $struct->headers  = array_merge($p['structure']->headers, $struct->headers);
        $struct->size     = $size;
        $struct->filename = $p['structure']->filename;

        // modify the new structure to be correctly handled by Chronomail
     ...
     *
     * @param chronomail_message_part
     * @param chronomail_message
     */
    ...
    public static function delete_dir($dir)
    {
        ...
        if (!is_writable($dir)) {
            chronomail::raise_error("Unable to delete $dir", false, true);
            return false;
        }
