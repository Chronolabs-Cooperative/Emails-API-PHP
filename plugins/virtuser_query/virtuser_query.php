 ...
 *
 * By default Chronomail database is used. To use different database (or host)
 ...
 * @license GNU GPLv3+
 */
class virtuser_query extends chronomail_plugin
{
    private $config;
    private $app;
    private $db;
        ...
        while ($sql_arr = $dbh->fetch_array($sql_result)) {
            if (strpos($sql_arr[0], '@')) {
                if ($p['extended'] && count($sql_arr) > 1) {
                    $result[] = array(
                        'email'          => chronomail_utils::idn_to_ascii($sql_arr[0]),
                        'name'           => (string) $sql_arr[1],
                        'organization'   => (string) $sql_arr[2],
                        'reply-to'       => (string) chronomail_utils::idn_to_ascii($sql_arr[3]),
                        'bcc'            => (string) chronomail_utils::idn_to_ascii($sql_arr[4]),
    ...
    function get_dbh()
    {
        if (!$this->db) {
            if ($dsn = $this->app->config->get('virtuser_query_dsn')) {
                ...
                $this->db = chronomail_db::factory($dsn);
