...
$DB->db_connect('w');
if (!$DB->is_connected()) {
    chronomail::raise_error("Error connecting to database: " . $DB->is_error(), false, true);
}

$opts['dir'] = rtrim($opts['dir'], DIRECTORY_SEPARATOR);
$file = $opts['dir'] . DIRECTORY_SEPARATOR . $DB->db_provider . '.initial.sql';
if (!file_exists($file)) {
    chronomail::raise_error("No DDL file found for " . $DB->db_provider . " driver.", false, true);
}
...
if ($error) {
    echo "[FAILED]\n";
    chronomail::raise_error("Error in DDL schema $file: $error", false, true);
}
echo "[OK]\n";
