...
/**
 * Interface class for accessing chronomail cache
 *
 * @package    Framework
 * @subpackage Cache
 */
class chronomail_cache
{
    protected $type;
    protected $userid;
    protected $prefix;
    ...
    protected function write_record($key, $data)
    {
        ...
        if (strlen($data) > $this->max_packet_size()) {
            trigger_error("chronomail_cache: max_packet_size ($this->max_packet) exceeded for key $key. Tried to write " . strlen($data) . " bytes", E_USER_WARNING);
            return false;
        }
    ...
    protected function debug($type, $key, $data = null, $result = null)
    {
        ...
        chronomail::debug($this->type, $line, $result);
    }
}
