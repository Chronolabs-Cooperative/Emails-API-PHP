...
class chronomail_mime
{
    ...
    public static function get_charset()
    {
        ...
        if ($charset = chronomail::get_instance()->config->get('default_charset')) {
            return $charset;
        }
     ...
     * of chronomail_message_part objects.
     *
     * It makes use of the chronomail_mime_decode library
     *
     * @param string $raw_body The message source
     *
     * @return object chronomail_message_part The message structure
     */
    public static function parse_message($raw_body)
    {
        ...
        $mime = new chronomail_mime_decode($conf);
    ...
    public static function decode_mime_string($input, $fallback = null)
    {
        ...
        if (preg_match_all($re, $input, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER)) {
            ...
            foreach ($matches as $idx => $m) {
                ...
                if ($start != $pos) {
                    ...
                    $out   .= chronomail_charset::convert($substr, $default_charset);
                    $start  = $pos;
                }
                $start += $length;
                ...
                $out .= chronomail_charset::convert($text, $charset);
                $tmp = array();
            }

            // add the last part of the input string
            if ($start != strlen($input)) {
                $out .= chronomail_charset::convert(substr($input, $start), $default_charset);
            }
        ...
        return chronomail_charset::convert($input, $default_charset);
    }
    ...
    public static function get_mime_extensions($mimetype = null)
    {
        ...
        if ($mime_types = chronomail::get_instance()->config->get('mime_types')) {
        ...
        if (empty($mime_types)) {
            foreach (chronomail::get_instance()->config->resolve_paths('mimetypes.php') as $fpath) {
    ...
    {
        $parts = chronomail_utils::explode_quoted_string('@', $email);
