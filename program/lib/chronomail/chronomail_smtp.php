...
class chronomail_smtp
{
    ...
    public function connect($host = null, $port = null, $user = null, $pass = null)
    {
        ...
        $CONFIG = $chronomail->plugins->exec_hook('smtp_connect', array(
            'smtp_server'    => $host ?: $chronomail->config->get('smtp_server'),
            'smtp_port'      => $port ?: $chronomail->config->get('smtp_port', 587),
            'smtp_user'      => $user !== null ? $user : $chronomail->config->get('smtp_user', '%u'),
            'smtp_pass'      => $pass !== null ? $pass : $chronomail->config->get('smtp_pass', '%p'),
            'smtp_auth_cid'  => $chronomail->config->get('smtp_auth_cid'),
            'smtp_auth_pw'   => $chronomail->config->get('smtp_auth_pw'),
            'smtp_auth_type' => $chronomail->config->get('smtp_auth_type'),
            'smtp_helo_host' => $chronomail->config->get('smtp_helo_host'),
            'smtp_timeout'   => $chronomail->config->get('smtp_timeout'),
            'smtp_conn_options'   => $chronomail->config->get('smtp_conn_options'),
            'smtp_auth_callbacks' => array(),
        ));

        $smtp_host = chronomail_utils::parse_host($CONFIG['smtp_server']);
        ...
        chronomail_utils::parse_socket_options($CONFIG['smtp_conn_options'], $smtp_host);

        // Use valid EHLO/HELO host (#6408)
        $helo_host = $CONFIG['smtp_helo_host'] ?: chronomail_utils::server_name();
        $helo_host = chronomail_utils::idn_to_ascii($helo_host);
        if (!preg_match('/^[a-zA-Z0-9.:-]+$/', $helo_host)) {
            $helo_host = 'localhost';
        }

        // IDNA Support
        $smtp_host = chronomail_utils::idn_to_ascii($smtp_host);
        ...
        if ($chronomail->config->get('smtp_debug')) {
        ...
        $smtp_user = str_replace('%u', $chronomail->get_user_name(), $CONFIG['smtp_user']);
        $smtp_pass = str_replace('%p', $chronomail->get_user_password(), $CONFIG['smtp_pass']);
        ...
        if (($smtp_user && $smtp_pass) || ($smtp_auth_type == 'GSSAPI')) {
            // IDNA Support
            if (strpos($smtp_user, '@')) {
                $smtp_user = chronomail_utils::idn_to_ascii($smtp_user);
            }
            ...
            if (!in_array($err[0], array(354, 250, 221))) {
                ...
                if (strpos($msg, 'size exceeds')) {
                    ...
                    if ($limit) {
                        $msg .= " (Limit: $limit)";
                        $chronomail = chronomail::get_instance();
                        if (method_exists($chronomail, 'show_bytes')) {
                            $limit = $chronomail->show_bytes($limit);
                        }
    ...
    public function debug_handler(&$smtp, $message)
    {
        ...
        chronomail::write_log('smtp', preg_replace('/\r\n$/', '', $message));
    }
    ...
    private function _parse_rfc822($recipients)
    {
        ...
        $recipients = chronomail_utils::explode_quoted_string(',', $recipients);

        reset($recipients);
        foreach ($recipients as $recipient) {
            $a = chronomail_utils::explode_quoted_string(' ', $recipient);
            foreach ($a as $word) {
                $word = trim($word);
                $len  = strlen($word);
