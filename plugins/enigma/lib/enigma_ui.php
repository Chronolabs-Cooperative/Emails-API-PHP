...
class enigma_ui
{
    ...
    function init()
    {
        ...
        $action = chronomail_utils::get_input_value('_a', chronomail_utils::INPUT_GPC);
...
*/
    ...
    private function key_list()
    {
        ...
        $page     = max(intval(chronomail_utils::get_input_value('_p', chronomail_utils::INPUT_GPC)), 1);
        $search   = chronomail_utils::get_input_value('_q', chronomail_utils::INPUT_GPC);
    ...
    private function key_info()
    {
        ...
        $id  = chronomail_utils::get_input_value('_id', chronomail_utils::INPUT_GET);
    ...
    {
        return chronomail::Q($this->data->name);
    }
    ...
    function tpl_key_data($attrib)
    {
        ...
        $table->add(null, chronomail::Q($this->data->name));
        ...
        foreach ($this->data->users as $user) {
            ...
            $table->add('id', chronomail::Q(trim($username)));
    ...
    {
        $keys   = chronomail_utils::get_input_value('_keys', chronomail_utils::INPUT_POST);
        $priv   = chronomail_utils::get_input_value('_priv', chronomail_utils::INPUT_POST);
    ...
    {
        // Import process
        if ($data = chronomail_utils::get_input_value('_keys', chronomail_utils::INPUT_POST)) {
            $this->enigma->load_engine();
            ...
            if (is_array($result)) {
                if (chronomail_utils::get_input_value('_generated', chronomail_utils::INPUT_POST)) {
            ...
            if ($err == UPLOAD_ERR_INI_SIZE || $err == UPLOAD_ERR_FORM_SIZE) {
                $this->rc->output->show_message('filesizeerror', 'error',
                    array('size' => $this->rc->show_bytes(chronomail_utils::max_upload_size())));
            } else {
    ...
    function tpl_key_import_form($attrib)
    {
        ...
        if (empty($attrib['part']) || $attrib['part'] == 'import') {
            ...
            $upload_button = new html_button(array(
                    'class'   => 'button import',
                    'onclick' => "return chronomail.command('plugin.enigma-import','',this,event)",
            ));

            $form = html::div(null, html::p(null, chronomail::Q($this->enigma->gettext('keyimporttext'), 'show'))
                . $upload->show()
        ...
        if (empty($attrib['part']) || $attrib['part'] == 'search') {
            ...
            $search_button = new html_button(array(
                    'class'   => 'button search',
                    'onclick' => "return chronomail.command('plugin.enigma-import-search','',this,event)",
            ));

            $form = html::div(null,
                chronomail::Q($this->enigma->gettext('keyimportsearchtext'), 'show')
    ...
    private function key_generate()
    {
        ...
        $user = chronomail_utils::get_input_value('_user', chronomail_utils::INPUT_POST, true);
        $pass = chronomail_utils::get_input_value('_password', chronomail_utils::INPUT_POST, true);
        $size = (int) chronomail_utils::get_input_value('_size', chronomail_utils::INPUT_POST);

        if ($size > 4096) {
            $size = 4096;
        }

        $ident = chronomail_mime::decode_address_list($user, 1, false);
    ...
    function tpl_key_create_form($attrib)
    {
        ...
        foreach ($identities as $idx => $ident) {
            ...
            $identities[$idx] = html::tag('li', null, html::label(null, $checkbox->show($idx, $attr) . chronomail::Q($name)));
        }

        $table->add('title', html::label('key-name', chronomail::Q($this->enigma->gettext('newkeyident'))));
        ...
        $table->add('title', html::label('key-size', chronomail::Q($this->enigma->gettext('newkeysize'))));
        $table->add(null, $select->show());

        // Password and confirm password
        $table->add('title', html::label('key-pass', chronomail::Q($this->enigma->gettext('newkeypass'))));
        $table->add(null, chronomail_output::get_edit_field('password', '', array(
                'id'           => 'key-pass',
                'size'         => $attrib['size'],
                'required'     => true,
        ...
        $table->add('title', html::label('key-pass-confirm', chronomail::Q($this->enigma->gettext('newkeypassconfirm'))));
        $table->add(null, chronomail_output::get_edit_field('password-confirm', '', array(
                'id'           => 'key-pass-confirm',
                'size'         => $attrib['size'],
                'required'     => true,
                'autocomplete' => 'new-password',
    ...
    private function compose_ui()
    {
        ...
        // other skins
            ...
            $this->enigma->add_button(array(
                ...
                'onclick'  => "chronomail.command('menu-open', 'enigmamenu', event.target, event)",
                'class'    => 'button enigma',
    ...
    private function compose_ui_options($wrap = false)
    {
        ...
        $out = html::div('form-group form-check row',
            html::label(array('for' => 'enigmasignopt', 'class' => 'col-form-label col-6'),
                chronomail::Q($this->enigma->gettext('signmsg')))
        ...
        $out .= html::div('form-group form-check row',
            html::label(array('for' => 'enigmaencryptopt', 'class' => 'col-form-label col-6'),
                chronomail::Q($this->enigma->gettext('encryptmsg')))
            . html::div('form-check col-6',
        ...
        $out .= html::div('form-group form-check row',
            html::label(array('for' => 'enigmaattachpubkeyopt', 'class' => 'col-form-label col-6'),
                chronomail::Q($this->enigma->gettext('attachpubkeymsg')))
            . html::div('form-check col-6',
    ...
    function status_message($p)
    {
        ...
        if ($p['part'] instanceof chronomail_message) {
            return $p;
        }
        ...
        if (($found = $this->find_part_id($part_id, $engine->decryptions)) !== null
            ...
            if ($status instanceof enigma_error) {
                ...
                if ($code == enigma_error::KEYNOTFOUND) {
                    $msg = chronomail::Q(str_replace('$keyid', enigma_key::format_id($status->getData('id')),
                    ...
                    $msg     = chronomail::Q($this->enigma->gettext($label));
                    $this->password_prompt($status);
                }
                else if ($code == enigma_error::NOMDC) {
                    $msg = chronomail::Q($this->enigma->gettext('decryptnomdc'));
                }
                else {
                    $msg = chronomail::Q($this->enigma->gettext('decrypterror'));
                }
            }
            else if ($status === enigma_engine::ENCRYPTED_PARTIALLY) {
                $attrib['class'] = 'boxwarning enigmawarning encrypted';
                $msg = chronomail::Q($this->enigma->gettext('decryptpartial'));
            }
            else {
                $attrib['class'] = 'boxconfirmation enigmanotice encrypted';
                $msg = chronomail::Q($this->enigma->gettext('decryptok'));
            }
        ...
        if (($found = $this->find_part_id($part_id, $engine->signatures)) !== null
            ...
            if ($sig instanceof enigma_signature) {
                ...
                if ($sig->valid === enigma_error::UNVERIFIED) {
                    ...
                    $msg = chronomail::Q($msg);
                }
                else if ($sig->valid) {
                    $attrib['class'] = ($sig->partial ? 'boxwarning enigmawarning' : 'boxconfirmation enigmanotice') . ' signed';
                    $label = 'sigvalid' . ($sig->partial ? 'partial' : '');
                    $msg = chronomail::Q(str_replace('$sender', $sender, $this->enigma->gettext($label)));
                }
                else {
                    $attrib['class'] = 'boxwarning enigmawarning signed';
                    if ($sender) {
                        $msg = chronomail::Q(str_replace('$sender', $sender, $this->enigma->gettext('siginvalid')));
                    }
                    else {
                        $msg = chronomail::Q(str_replace('$keyid', enigma_key::format_id($sig->id),
                ...
                $attrib['class'] = 'boxwarning enigmawarning signed';
                $msg = chronomail::Q(str_replace('$keyid', enigma_key::format_id($sig->getData('id')),
                    $this->enigma->gettext('signokey')));
            }
            else {
                $attrib['class'] = 'boxwarning enigmaerror signed';
                $msg = chronomail::Q($this->enigma->gettext('sigerror'));
            }
/*
            $msg .= '&nbsp;' . html::a(array('href' => "#sigdetails",
                'onclick' => chronomail_output::JS_OBJECT_NAME.".command('enigma-sig-details')"),
                chronomail::Q($this->enigma->gettext('showdetails')));
*/
            // test
//            $msg .= '<br /><pre>'.$sig->body.'</pre>';
    ...
    function message_output($p)
    {
        foreach ($this->keys_parts as $part) {
            ...
            $p['content'] = html::p(array('class' => 'enigmaattachment boxinformation aligned-buttons'),
                html::span(null, chronomail::Q($this->enigma->gettext('keyattfound'))) .
                html::tag('button', array(
                        'onclick' => "return ".chronomail_output::JS_OBJECT_NAME.".enigma_import_attachment('".chronomail::JQ($part)."')",
                        'title'   => $this->enigma->gettext('keyattimport'),
                        'class'   => 'import',
                    ), chronomail::Q($this->rc->gettext('import')))
            ) . $p['content'];
    ...
    function message_ready($p)
    {
        ...
        $sign_enable    = (bool) chronomail_utils::get_input_value('_enigma_sign', chronomail_utils::INPUT_POST);
        $encrypt_enable = (bool) chronomail_utils::get_input_value('_enigma_encrypt', chronomail_utils::INPUT_POST);
        $pubkey_enable  = (bool) chronomail_utils::get_input_value('_enigma_attachpubkey', chronomail_utils::INPUT_POST);
        $locks          = (array) $this->rc->config->get('enigma_options_lock');
   ...
   /**
    ...
    function import_file()
    {
        ...
        if ($uid && $mime_id) {
            // Note: we get the attachment body via chronomail_message class
