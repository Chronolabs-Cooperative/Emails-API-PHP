 ...
 */
class chronomail_ldap extends chronomail_addressbook
{
    // public properties
    ...
    */
    function __construct($p, $debug = false, $mail_domain = null)
    {
        ...
        if ($cache_type = $chronomail->config->get('ldap_cache', 'db')) {
            $cache_ttl  = $chronomail->config->get('ldap_cache_ttl', '10m');
            $cache_name = 'LDAP.' . asciiwords($this->prop['name']);

            $this->cache = $chronomail->get_cache($cache_name, $cache_type, $cache_ttl);
        }
    ...
    private function _connect()
    {
        ...
        foreach ($this->prop['hosts'] as $host) {
            ...
            $chronomail = chronomail::get_instance();
            $conf  = $chronomail->plugins->exec_hook('ldap_connected', $this->prop + array('host' => $host));
            ...
            if ($conf['user_specific']) {
                ...
                if (empty($bind_pass)) {
                    $bind_pass = $chronomail->get_user_password();
                }

                // Get the pieces needed for variable replacement.
                if ($fu = ($chronomail->get_user_email() ?: $conf['username'])) {
                    list($u, $d) = explode('@', $fu);
                }
                else {
                    $d = $this->mail_domain;
                ...
                if ($conf['search_base_dn'] && $conf['search_filter']
                    ...
                    $cache_key = chronomail_cache::key_name('DN', array($host, $search_bind_dn, $search_base_dn, $search_filter, $conf['search_bind_pw']));
                    ...
                    if ($this->cache && ($dn = $this->cache->get($cache_key))) {
                        ...
                        if (!empty($search_bind_dn) && !empty($conf['search_bind_pw'])) {
                            ...
                            if (!empty($conf['vlv'])) {
                                $ldap = new chronomail_ldap_generic($conf);
        ...
        if (!is_resource($this->ldap->conn)) {
            chronomail::raise_error(array('code' => 100, 'type' => 'ldap',
                'file' => __FILE__, 'line' => __LINE__,
    ...
    function list_records($cols=null, $subset=0)
    {
        if ($this->prop['searchonly'] && empty($this->filter) && !$this->group_id) {
            $this->result = new chronomail_result_set(0);
        ...
        if ($this->group_id && $this->group_data['dn']) {
            ...
            $this->result = new chronomail_result_set($entries['count'], ($this->list_page-1) * $this->page_size);
        }
        else {
     ...
     * @param int     $mode     Matching mode. Sum of chronomail_addressbook::SEARCH_*
     * @param boolean $select   True if results are requested, False if count only
     * @param boolean $nocount  (Not used)
     * @param array   $required List of fields that cannot be empty
     *
     * @return chronomail_result_set List of contact records
     */
    function search($fields, $value, $mode = 0, $select = true, $nocount = false, $required = array())
    {
        ...
        if ($fields == 'ID' || $fields == $this->primary_key) {
            ...
            $result = new chronomail_result_set();
            foreach ($ids as $id) {
        ...
        $chronomail = chronomail::get_instance();
        $list_fields = $chronomail->config->get('contactlist_fields');

        if ($this->prop['vlv_search'] && $this->ready && implode(',', (array)$fields) == implode(',', $list_fields)) {
            $this->result = new chronomail_result_set(0);

            $this->ldap->config_set('fuzzy_search', intval($this->prop['fuzzy_search'] && !($mode & chronomail_addressbook::SEARCH_STRICT)));
        ...
        if (is_array($value)) {
            ...
            // set wildcards
            $wp = $ws = '';
            if (!empty($this->prop['fuzzy_search']) && !($mode & chronomail_addressbook::SEARCH_STRICT)) {
                $ws = '*';
                if (!($mode & chronomail_addressbook::SEARCH_PREFIX)) {
                    $wp = '*';
                }
            }
            ...
            foreach ((array)$fields as $idx => $field) {
                ...
                if ($attrs = $this->_map_field($field)) {
                    if (count($attrs) > 1)
                        $filter .= '(|';
                    foreach ($attrs as $f)
                        $filter .= "($f=$wp" . chronomail_ldap_generic::quote_string($val) . "$ws)";
                    if (count($attrs) > 1)
                        $filter .= ')';
                }
            }
            ...
            if ($fields == '*') {
                ...
                if (empty($this->prop['search_fields'])) {
                    ...
                    $this->result = new chronomail_result_set();
                    return $this->result;
                }
            ...
            $filter = chronomail_ldap_generic::fulltext_search_filter($value, $attributes, $mode & ~chronomail_addressbook::SEARCH_GROUPS);
        }
        ...
        if ($mode & chronomail_addressbook::SEARCH_GROUPS) {
            $filter = 'e:' . $filter;
        }
     ...
     *
     * @return object chronomail_result_set Resultset with values for 'count' and 'first'
     */
    function count()
    {
        $count = 0;
        ...
        return new chronomail_result_set($count, ($this->list_page-1) * $this->page_size);
    }
     ...
     *
     * @return object chronomail_result_set Current resultset or NULL if nothing selected yet
     */
    function get_result()
    {
        return $this->result;
     ...
     *
     * @return mixed  Hash array or chronomail_result_set with all record fields
     */
    function get_record($dn, $assoc=false)
    {
        ...
        if ($this->ready && $dn) {
            ...
            if (!empty($rec)) {
                ...
                $this->result = new chronomail_result_set(1);
                $this->result->add($res);
            }
        }
    ...
    function insert($save_cols, $check = false)
    {
        ...
        $dn = $this->prop['LDAP_rdn'].'='.chronomail_ldap_generic::quote_string($newentry[$this->prop['LDAP_rdn']], true).','.$this->base_dn;
        ...
        foreach ($xfields as $xidx => $xf) {
            $xdn = $xidx.'='.chronomail_ldap_generic::quote_string($xf).','.$dn;
            $xf = array(
                $xidx => $xf,
    ...
    function update($id, $save_cols)
    {
        ...
        if (!empty($replacedata)) {
            ...
            if ($replacedata[$this->prop['LDAP_rdn']]) {
                $newdn = $this->prop['LDAP_rdn'].'='
                    .chronomail_ldap_generic::quote_string($replacedata[$this->prop['LDAP_rdn']], true)
                    .','.$this->base_dn;
                if ($dn != $newdn) {
        ...
        if (!empty($subdeldata)) {
            foreach ($subdeldata as $fld => $val) {
                $subdn = $fld.'='.chronomail_ldap_generic::quote_string($val).','.$dn;
    ...
    private function _map_data($save_cols)
    {
        ...
        foreach ($this->formats as $fld => $format) {
            ...
            switch ($format['type']) {
            case 'date':
                if ($dt = chronomail_utils::anytodatetime($ldap_data[$fld])) {
     ...
     * @param int     Matching mode. Sum of chronomail_addressbook::SEARCH_*
     *
    ...
    private function _fetch_groups($search = null, $mode = 0, $vlv_page = null)
    {
        ...
        if ($vlv_page === null && $search === null && is_array($this->prop['group_filters'])) {
            ...
            if (!empty($this->prop['groups']['filter'])) {
                $id = '__groups__';
                $groups[$id] = array('ID' => $id, 'name' => $chronomail->gettext('groups'), 'virtual' => true) + $this->prop['groups'];
            }
        ...
        if ($search !== null) {
            ...
            $filter = "(&$filter($name_attr=$wp" . chronomail_ldap_generic::quote_string($search) . "$ws))";
    ...
    {
        $new_dn      = 'cn=' . chronomail_ldap_generic::quote_string($group_name, true) . ',' . $this->groups_base_dn;
    ...
    function rename_group($group_id, $new_name, &$new_gid)
    {
        ...
        $new_rdn     = "cn=" . chronomail_ldap_generic::quote_string($new_name, true);
