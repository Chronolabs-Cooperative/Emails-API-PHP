...
/**
 * Class to control the installation process of the chronomail Webmail package
 *
 * @category Install
 * @package  Webmail
 */
class chronomail_install
{
    public $step;
    public $last_error;
    ...
    public function load_config_file($file)
    {
        ...
        if (function_exists('token_get_all')) {
            ...
            for ($i = 0; $i < count($tokens); $i++) {
                $token = $tokens[$i];
                if ($token[0] == T_VARIABLE && ($token[1] == '$config' || $token[1] == '$chronomail_config')) {
                    $in_config = true;
        ...
        if (is_array($chronomail_config)) {
            return $chronomail_config;
        }
    ...
    public function getprop($name, $default = '')
    {
        ...
        if ($name == 'des_key' && !$this->configured && !isset($_REQUEST["_$name"])) {
            $value = chronomail_utils::random_bytes(24);
        }
    ...
    public function create_config()
    {
        ...
        foreach ($this->config as $prop => $default) {
            ...
            if ($prop == 'useragent' && stripos($value, 'chronomail Webmail/') !== false) {
        ...
        $out = "<?php\n\n";
        $out .= "/* Local configuration for chronomail Webmail */\n\n";
     ...
     * required for this version of chronomail
     *
     * @param chronomail_db $db Database object
     *
    ...
    public function db_schema_check($db)
    {
        ...
        if ($schema_version) {
            $version = chronomail_utils::db_version();
    ...
    private function db_read_schema($schemafile, &$version = null)
    {
        ...
        foreach ($lines as $line) {
            ...
            else if (preg_match('/insert into/i', $line) && preg_match('/\'chronomail-version\',\s*\'([0-9]+)\'/', $line, $m)) {
                $version = $m[1];
            }
    ...
    public function check_mime_detection()
    {
        ...
        foreach ($files as $path => $expected) {
            $mimetype = chronomail_mime::file_content_type(INSTALL_PATH . $path, basename($path));
    ...
    public function check_mime_extensions()
    {
        ...
        foreach ($types as $mimetype => $expected) {
            $ext = chronomail_mime::get_mime_extensions($mimetype);
    ...
    public function get_hostlist()
    {
        ...
        foreach ($default_hosts as $key => $name) {
            if (!empty($name)) {
                $out[] = chronomail_utils::parse_host(is_numeric($key) ? $name : $key);
            }
        }
    ...
    /**
     * Create a HTML dropdown to select a previous version of chronomail
     */
    ...
    {
        echo chronomail::Q($name) . ':&nbsp; <span class="success">OK</span>';
    ...
    public function fail($name, $message = '', $url = '', $optional=false)
    {
        ...
        echo chronomail::Q($name) . ':&nbsp; <span class="fail">NOT OK</span>';
    ...
    {
        echo chronomail::Q($name) . ':&nbsp; <span class="na">NOT OK</span>';
        $this->_showhint($message, $url);
    }
    ...
    {
        echo chronomail::Q($name) . ':&nbsp; <span class="na">NOT AVAILABLE</span>';
        $this->_showhint($message, $url);
    }

    private function _showhint($message, $url = '')
    {
        $hint = chronomail::Q($message);

        if ($url) {
            $hint .= ($hint ? '; ' : '') . 'See <a href="' . chronomail::Q($url) . '" target="_blank">' . chronomail::Q($url) . '</a>';
        }
...
*/
        ...
        if (is_array($var)) {
            if (empty($var)) {
                ...
                if ($isnum) {
                    return 'array(' . implode(', ', array_map(array('chronomail_install', '_dump_var'), $var)) . ')';
                }
            }
        }
     ...
     *
     * @param chronomail_db $db Database connection
     *
    ...
    {
        return chronomail_utils::db_update(INSTALL_PATH . 'SQL',
            'chronomail', $version, array('quiet' => true));
    }

    /**
     * Handler for chronomail errors
     */
