...
class chronomail_sieve_engine
{
    ...
    const VERSION  = '9.3';
    const PROGNAME = 'Chronomail (Managesieve)';
    const PORT     = 4190;
    ...
    {
        $this->rc      = chronomail::get_instance();
        $this->plugin  = $plugin;
    ...
    function start($mode = null)
    {
        ...
        if (!$error) {
            ...
            if ($mode != 'vacation' && $mode != 'forward') {
                if (!empty($_GET['_set']) || !empty($_POST['_set'])) {
                    $script_name = chronomail_utils::get_input_value('_set', chronomail_utils::INPUT_GPC, true);
                }
        ...
        if ($error) {
            switch ($error) {
                case chronomail_sieve::ERROR_CONNECTION:
                case chronomail_sieve::ERROR_LOGIN:
    ...
    public function connect($username, $password)
    {
        ...
        $host = chronomail_utils::parse_host($host);
        ...
        chronomail_utils::parse_socket_options($plugin['socket_options'], $plugin['host']);

        // try to connect to managesieve server and to fetch the script
        $this->sieve = new chronomail_sieve(
            $plugin['user'],
            $plugin['password'],
            $plugin['host'],
            $plugin['port'],
    ...
    function actions()
    {
        ...
        if ($action = chronomail_utils::get_input_value('_act', chronomail_utils::INPUT_GPC)) {
            $fid = (int) chronomail_utils::get_input_value('_fid', chronomail_utils::INPUT_POST);
            ...
            if ($action == 'delete' && !$error) {
                ...
                if (isset($this->script[$fid])) {
                    $to   = (int) chronomail_utils::get_input_value('_to', chronomail_utils::INPUT_POST);
                ...
                if (!in_array('enable_disable_set', $this->disabled_actions)) {
                    $script_name = chronomail_utils::get_input_value('_set', chronomail_utils::INPUT_POST, true);
                ...
                if (!in_array('download_set', $this->disabled_actions)) {
                    $this->rc->request_security_check(chronomail_utils::INPUT_GET);
                ...
                $rid = chronomail_utils::get_input_value('_rid', chronomail_utils::INPUT_POST);
                $id = $this->genid();
                ...
                $aid = chronomail_utils::get_input_value('_aid', chronomail_utils::INPUT_POST);
                $id = $this->genid();
            ...
            // Initialize the form
            $rules = chronomail_utils::get_input_value('r', chronomail_utils::INPUT_GET);
            if (!empty($rules)) {
                $tests = array();
    ...
    function saveraw()
    {
        ...
        $script_name = chronomail_utils::get_input_value('_set', chronomail_utils::INPUT_POST);
    ...
    function save()
    {
        ...
        // filters set add action
            ...
            $name       = chronomail_utils::get_input_value('_name', chronomail_utils::INPUT_POST, true);
            $copy       = chronomail_utils::get_input_value('_copy', chronomail_utils::INPUT_POST, true);
            $from       = chronomail_utils::get_input_value('_from', chronomail_utils::INPUT_POST);
                ...
                if (is_uploaded_file($_FILES['_file']['tmp_name'])) {
                    ...
                    if ($err == UPLOAD_ERR_INI_SIZE || $err == UPLOAD_ERR_FORM_SIZE) {
                        $msg = $this->rc->gettext(array('name' => 'filesizeerror',
                            'vars' => array('size' =>
                                $this->rc->show_bytes(chronomail_utils::max_upload_size()))));
                    }
                    else {
        ...
        // filter add/edit action
            ...
            $fid  = trim(chronomail_utils::get_input_value('_fid', chronomail_utils::INPUT_POST));
            $join = trim(chronomail_utils::get_input_value('_join', chronomail_utils::INPUT_POST));

            // and arrays
            $headers        = chronomail_utils::get_input_value('_header', chronomail_utils::INPUT_POST);
            $cust_headers   = chronomail_utils::get_input_value('_custom_header', chronomail_utils::INPUT_POST);
            $cust_vars      = chronomail_utils::get_input_value('_custom_var', chronomail_utils::INPUT_POST);
            $ops            = chronomail_utils::get_input_value('_rule_op', chronomail_utils::INPUT_POST);
            $sizeops        = chronomail_utils::get_input_value('_rule_size_op', chronomail_utils::INPUT_POST);
            $sizeitems      = chronomail_utils::get_input_value('_rule_size_item', chronomail_utils::INPUT_POST);
            $sizetargets    = chronomail_utils::get_input_value('_rule_size_target', chronomail_utils::INPUT_POST);
            $targets        = chronomail_utils::get_input_value('_rule_target', chronomail_utils::INPUT_POST, true);
            $mods           = chronomail_utils::get_input_value('_rule_mod', chronomail_utils::INPUT_POST);
            $mod_types      = chronomail_utils::get_input_value('_rule_mod_type', chronomail_utils::INPUT_POST);
            $body_trans     = chronomail_utils::get_input_value('_rule_trans', chronomail_utils::INPUT_POST);
            $body_types     = chronomail_utils::get_input_value('_rule_trans_type', chronomail_utils::INPUT_POST, true);
            $comparators    = chronomail_utils::get_input_value('_rule_comp', chronomail_utils::INPUT_POST);
            $indexes        = chronomail_utils::get_input_value('_rule_index', chronomail_utils::INPUT_POST);
            $lastindexes    = chronomail_utils::get_input_value('_rule_index_last', chronomail_utils::INPUT_POST);
            $dateheaders    = chronomail_utils::get_input_value('_rule_date_header', chronomail_utils::INPUT_POST);
            $dateparts      = chronomail_utils::get_input_value('_rule_date_part', chronomail_utils::INPUT_POST);
            $mime_parts     = chronomail_utils::get_input_value('_rule_mime_part', chronomail_utils::INPUT_POST);
            $mime_types     = chronomail_utils::get_input_value('_rule_mime_type', chronomail_utils::INPUT_POST);
            $mime_params    = chronomail_utils::get_input_value('_rule_mime_param', chronomail_utils::INPUT_POST, true);
            $message        = chronomail_utils::get_input_value('_rule_message', chronomail_utils::INPUT_POST);
            $dup_handles    = chronomail_utils::get_input_value('_rule_duplicate_handle', chronomail_utils::INPUT_POST, true);
            $dup_headers    = chronomail_utils::get_input_value('_rule_duplicate_header', chronomail_utils::INPUT_POST, true);
            $dup_uniqueids  = chronomail_utils::get_input_value('_rule_duplicate_uniqueid', chronomail_utils::INPUT_POST, true);
            $dup_seconds    = chronomail_utils::get_input_value('_rule_duplicate_seconds', chronomail_utils::INPUT_POST);
            $dup_lasts      = chronomail_utils::get_input_value('_rule_duplicate_last', chronomail_utils::INPUT_POST);
            $act_types      = chronomail_utils::get_input_value('_action_type', chronomail_utils::INPUT_POST, true);
            $mailboxes      = chronomail_utils::get_input_value('_action_mailbox', chronomail_utils::INPUT_POST, true);
            $act_targets    = chronomail_utils::get_input_value('_action_target', chronomail_utils::INPUT_POST, true);
            $domain_targets = chronomail_utils::get_input_value('_action_target_domain', chronomail_utils::INPUT_POST);
            $area_targets   = chronomail_utils::get_input_value('_action_target_area', chronomail_utils::INPUT_POST, true);
            $reasons        = chronomail_utils::get_input_value('_action_reason', chronomail_utils::INPUT_POST, true);
            $addresses      = chronomail_utils::get_input_value('_action_addresses', chronomail_utils::INPUT_POST, true);
            $intervals      = chronomail_utils::get_input_value('_action_interval', chronomail_utils::INPUT_POST);
            $interval_types = chronomail_utils::get_input_value('_action_interval_type', chronomail_utils::INPUT_POST);
            $from           = chronomail_utils::get_input_value('_action_from', chronomail_utils::INPUT_POST);
            $subject        = chronomail_utils::get_input_value('_action_subject', chronomail_utils::INPUT_POST, true);
            $flags          = chronomail_utils::get_input_value('_action_flags', chronomail_utils::INPUT_POST);
            $varnames       = chronomail_utils::get_input_value('_action_varname', chronomail_utils::INPUT_POST);
            $varvalues      = chronomail_utils::get_input_value('_action_varvalue', chronomail_utils::INPUT_POST);
            $varmods        = chronomail_utils::get_input_value('_action_varmods', chronomail_utils::INPUT_POST);
            $notifymethods  = chronomail_utils::get_input_value('_action_notifymethod', chronomail_utils::INPUT_POST);
            $notifytargets  = chronomail_utils::get_input_value('_action_notifytarget', chronomail_utils::INPUT_POST, true);
            $notifyoptions  = chronomail_utils::get_input_value('_action_notifyoption', chronomail_utils::INPUT_POST, true);
            $notifymessages = chronomail_utils::get_input_value('_action_notifymessage', chronomail_utils::INPUT_POST, true);
            $notifyfrom     = chronomail_utils::get_input_value('_action_notifyfrom', chronomail_utils::INPUT_POST);
            $notifyimp      = chronomail_utils::get_input_value('_action_notifyimportance', chronomail_utils::INPUT_POST);
            $addheader_name  = chronomail_utils::get_input_value('_action_addheader_name', chronomail_utils::INPUT_POST);
            $addheader_value = chronomail_utils::get_input_value('_action_addheader_value', chronomail_utils::INPUT_POST, true);
            $addheader_pos   = chronomail_utils::get_input_value('_action_addheader_pos', chronomail_utils::INPUT_POST);
            $delheader_name  = chronomail_utils::get_input_value('_action_delheader_name', chronomail_utils::INPUT_POST);
            $delheader_value = chronomail_utils::get_input_value('_action_delheader_value', chronomail_utils::INPUT_POST, true);
            $delheader_pos   = chronomail_utils::get_input_value('_action_delheader_pos', chronomail_utils::INPUT_POST);
            $delheader_index = chronomail_utils::get_input_value('_action_delheader_index', chronomail_utils::INPUT_POST);
            $delheader_op    = chronomail_utils::get_input_value('_action_delheader_op', chronomail_utils::INPUT_POST);
            $delheader_comp  = chronomail_utils::get_input_value('_action_delheader_comp', chronomail_utils::INPUT_POST);
...
//                      $this->errors['actions'][$i]['targetarea'] = $this->plugin->gettext('cannotbeempty');
                ...
                case 'redirect_copy':
                    ...
                    else if (!chronomail_utils::check_email($target))
...
// @TODO: vacation :mime, :handle
                    ...
                    foreach ((array)$this->form['actions'][$i]['addresses'] as $aidx => $address) {
                        ...
                        else if (!chronomail_utils::check_email($address)) {
                            $this->errors['actions'][$i]['addresses'] = $this->plugin->gettext('noemailwarning');
                            break;
                        }
                    }

                    if (!empty($this->form['actions'][$i]['from']) && !chronomail_utils::check_email($this->form['actions'][$i]['from'])) {
                ...
                case 'notify':
                    ...
                    if (!empty($notifyfrom[$idx]) && !chronomail_utils::check_email($notifyfrom[$idx])) {
                        $this->errors['actions'][$i]['from'] = $this->plugin->gettext('noemailwarning');
                    }
    ...
    function filtersets_list($attrib, $no_env = false)
    {
        ...
        if (!empty($attrib['type']) && $attrib['type'] == 'list') {
            ...
            $select = new html_select(array('name' => '_set', 'id' => $attrib['id'],
                'onchange' => $this->rc->task != 'mail' ? 'chronomail.managesieve_set()' : ''));
    ...
    function filterset_form($attrib)
    {
        ...
        $copy     = chronomail_utils::get_input_value('_copy', chronomail_utils::INPUT_POST);
        $selected = chronomail_utils::get_input_value('_from', chronomail_utils::INPUT_POST);

        // filter set name input
        $input_name = new html_inputfield(array('name' => '_name', 'id' => '_name', 'size' => 30,
            'class' => ($this->errors['name'] ? 'error' : '')));

        $table->add('title', html::label('_name', chronomail::Q($this->plugin->gettext('filtersetname'))));
        ...
        $filters .= '<li>' . html::label('from_none', html::tag('input', array(
            ...
            )) . chronomail::Q($this->plugin->gettext('none'))) . '</li>';
        ...
        if (is_array($list)) {
            ...
            $filters .= '<li>' . html::label('from_set', html::tag('input', array(
                ...
                )) .  chronomail::Q($this->plugin->gettext('fromset')) . ' ' . $select->show($copy)) . '</li>';
        }
        ...
        $filters .= '<li>' . html::label('from_file', html::tag('input', array(
            ...
            )) . chronomail::Q($this->plugin->gettext('fromfile')) . ' ' . $upload->show()) . '</li>';

        $filters .= '</ul>';

        $table->add('title', html::label('from_none', chronomail::Q($this->plugin->gettext('filters'))));
        $table->add('', $filters);
    ...
    function filter_form($attrib)
    {
        ...
        $fid     = chronomail_utils::get_input_value('_fid', chronomail_utils::INPUT_GPC);
        ...
        $out .= sprintf("\n" . '<div class="form-group row">'
            ...
            chronomail::Q($this->plugin->gettext('filtername')), $input_name);
        ...
        if ($this->rc->task == 'mail') {
            $out .= sprintf("\n" . '<div class="form-group row">'
                ...
                . '<div class="col-sm-8">%s</div></div>',
                'sievescriptname',
                chronomail::Q($this->plugin->gettext('filterset')),
            ...
            $out .= sprintf("\n" . '<div class="form-group row form-check">'
                ...
                chronomail::Q($this->plugin->gettext('filterdisabled')));
        }
        ...
        if ($compact) {
            ...
            $out .= sprintf("\n" . '<div class="form-group row">'
                ...
                . '<div class="col-sm-8">%s</div></div>',
                chronomail::Q($this->plugin->gettext('scope')), $select->show($join));

            $out .= '<div id="rules"'.($any ? ' style="display: none"' : '').'>';
            $out .= "\n<fieldset><legend>" . chronomail::Q($this->plugin->gettext('rules')) . "</legend>\n";
        }
        else {
            $out .= '<br><fieldset><legend>' . chronomail::Q($this->plugin->gettext('messagesrules')) . "</legend>\n";
            ...
            $out .= $input_join . html::label($field_id, chronomail::Q($this->plugin->gettext('filterallof')));
            ...
            $out .= $input_join . html::label($field_id, chronomail::Q($this->plugin->gettext('filteranyof')));
            ...
            $out .= $input_join . html::label($field_id, chronomail::Q($this->plugin->gettext('filterany')));
            $out .= '<div id="rules"'.($any ? ' style="display: none"' : '').'>';
        }
        ...
        $out .= '<fieldset><legend>' . chronomail::Q($label) . "</legend>\n";
    ...
    function rule_div($fid, $id, $div = true, $compact = false)
    {
        ...
        if (in_array('date', $this->exts) || in_array('currentdate', $this->exts)) {
            ...
            foreach ($date_parts as $part) {
                $select_dp->add(chronomail::Q($this->plugin->gettext($part)), $part);
            }
        ...
        if (in_array('duplicate', $this->exts)) {
            ...
            $select_msg->add(chronomail::Q($this->plugin->gettext('duplicate')), 'duplicate');
            $select_msg->add(chronomail::Q($this->plugin->gettext('notduplicate')), 'notduplicate');
        ...
        $select_size_op->add(chronomail::Q($this->plugin->gettext('filterover')), 'over');
        $select_size_op->add(chronomail::Q($this->plugin->gettext('filterunder')), 'under');
        ...
        $select_mod->add(chronomail::Q($this->plugin->gettext('none')), '');
        $select_mod->add(chronomail::Q($this->plugin->gettext('address')), 'address');
        if (in_array('envelope', $this->exts)) {
            $select_mod->add(chronomail::Q($this->plugin->gettext('envelope')), 'envelope');
        }

        $select_type = new html_select(array('name' => "_rule_mod_type[$id]", 'id' => 'rule_mod_type'.$id));
        $select_type->add(chronomail::Q($this->plugin->gettext('allparts')), 'all');
        $select_type->add(chronomail::Q($this->plugin->gettext('domain')), 'domain');
        $select_type->add(chronomail::Q($this->plugin->gettext('localpart')), 'localpart');
        if (in_array('subaddress', $this->exts)) {
            $select_type->add(chronomail::Q($this->plugin->gettext('user')), 'user');
            $select_type->add(chronomail::Q($this->plugin->gettext('detail')), 'detail');
        }
        ...
        $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('modifier'))));
        ...
        $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('modtype'))));
        ...
        $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('comparator'))));
        ...
        if (in_array('mime', $this->exts)) {
            ...
            foreach (array('contenttype', 'type', 'subtype', 'param') as $val) {
                ...
                $select_mime->add(chronomail::Q($this->plugin->gettext('mime-' . $val)), $val);
            }

            $select_mime_part = new html_select(array('name' => "_rule_mime_part[$id]", 'id' => 'rule_mime_part' . $id));
            $select_mime_part->add(chronomail::Q($this->plugin->gettext('mime-message')), '');
            $select_mime_part->add(chronomail::Q($this->plugin->gettext('mime-anychild')), 'anychild');

            $mout .= '<div id="rule_mime_part' .$id. '" class="adv input-group"' . (!$need_mime ? ' style="display:none"' : '') . '>';
            $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('mimepart'))));
            ...
            $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('mime'))));
        ...
        $select_mod->add(chronomail::Q($this->plugin->gettext('text')), 'text');
        $select_mod->add(chronomail::Q($this->plugin->gettext('undecoded')), 'raw');
        $select_mod->add(chronomail::Q($this->plugin->gettext('contenttype')), 'content');
        ...
        if (in_array('date', $this->exts)) {
            ...
            $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('dateheader'))));
            $mout .= html::tag('input', array(
                    'type'  => 'text',
        ...
        if (in_array('index', $this->exts)) {
            ...
            $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('index'))));
            $mout .= html::tag('input', array(
                    'type'  => 'text',
                    'name'  => "_rule_index[$id]",
                    'id'    => 'rule_index' . $id,
            ...
            $mout .= html::label('input-group-append',
                    ...
                    )) . chronomail::Q($this->plugin->gettext('indexlast')));
            $mout .= '</div>';
        }
        ...
        if (in_array('duplicate', $this->exts)) {
            ...
            foreach (array('handle', 'header', 'uniqueid') as $unit) {
                ...
                $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('duplicate.' . $unit))));
                $mout .= html::tag('input', array(
                        'type'  => 'text',
            ...
            $mout .= '<div class="input-group">';
            $mout .= html::span('label input-group-prepend', html::span('input-group-text', chronomail::Q($this->plugin->gettext('duplicate.seconds'))));
            $mout .= html::tag('input', array(
                    'type'  => 'text',
            ...
            $mout .= html::label('input-group-append',
                html::tag('input', array(
                    ...
                    'value'   => 1,
                    'checked' => !empty($rule['last']),
                )) . chronomail::Q($this->plugin->gettext('duplicate.last')));
            $mout .= '</div>';
            $mout .= '</div>';
        }

        $add_title = chronomail::Q($this->plugin->gettext('add'));
        $del_title = chronomail::Q($this->plugin->gettext('del'));
        $adv_title = chronomail::Q($this->plugin->gettext('advancedopts'));
        ...
        $out .= sprintf('<a href="#" id="ruleadd%s" title="%s" onclick="chronomail.managesieve_ruleadd(\'%s\'); return false" class="button create add">'
            . '<span class="inner">%s</span></a>', $id, $add_title, $id, $add_title);
        $out .= sprintf('<a href="#" id="ruledel%s" title="%s" onclick="chronomail.managesieve_ruledel(\'%s\'); return false" class="button delete del%s">'
    ...
    function action_div($fid, $id, $div=true)
    {
        ...
        $out .= html::tag('textarea', array(
            ...
            ), (in_array($action['type'], array('reject', 'ereject')) ? chronomail::Q($action['target'], 'strict', false) : ''));
        ...
        $out .= '<span class="label">'. chronomail::Q($this->plugin->gettext('vacationreason')) .'</span><br>';
        $out .= html::tag('textarea', array(
            ...
            ), chronomail::Q($action['reason'], 'strict', false));
        $out .= '<br><span class="label">' .chronomail::Q($this->plugin->gettext('vacationsubject')) . '</span><br>';
        ...
        $out .= '<br><span class="label">' .chronomail::Q($this->plugin->gettext('vacationfrom')) . '</span><br>';
        $out .= html::tag('input', array(
                'type'  => 'text',
        ...
        $out .= '<br><span class="label">' .chronomail::Q($this->plugin->gettext('vacationaddr')) . '</span><br>';
        $out .= $this->list_input($id, 'action_addresses', $action['addresses'], true,
                    $this->error_class($id, 'action', 'addresses', 'action_addresses'), 30)
            . html::a(array('href' => '#', 'onclick' => chronomail_output::JS_OBJECT_NAME . ".managesieve_vacation_addresses($id)"),
                chronomail::Q($this->plugin->gettext('filladdresses')));
        $out .= '<br><span class="label">' . chronomail::Q($this->plugin->gettext('vacationinterval')) . '</span><br>';
        $out .= '<div class="input-group">' . html::tag('input', array(
                'type'  => 'text',
                'name'  => '_action_interval[' . $id . ']',
                'id'    => 'action_interval' . $id,
                'value' => chronomail_sieve_vacation::vacation_interval($action, $this->exts),
                'size'  => 2,
        ...
        foreach ($flags as $fidx => $flag) {
            $flout .= html::label(null, html::tag('input', array(
                ...
                . chronomail::Q($this->plugin->gettext('flag'.$fidx))) . '<br>';
        }
        ...
        foreach (array('name', 'value') as $unit) {
            $out .= '<span class="label">' .chronomail::Q($this->plugin->gettext('setvar' . $unit)) . '</span><br>';
            $out .= html::tag('input', array(
                    'type'  => 'text',
        ...
        $out .= '<span class="label">' .chronomail::Q($this->plugin->gettext('setvarmodifiers')) . '</span>';
        foreach ($set_modifiers as $s_m) {
            ...
            $out .= '<br>' . html::tag('input', array(
                ...
                .chronomail::Q($this->plugin->gettext('var' . $s_m));
        }
        $out .= '</div>';
        ...
        foreach ($notify_methods as $m_n) {
            $select_method->add(chronomail::Q($this->rc->text_exists('managesieve.notifymethod'.$m_n) ? $this->plugin->gettext('managesieve.notifymethod'.$m_n) : $m_n), $m_n);
        }
        ...
        foreach ($importance_options as $io_v => $io_n) {
            $select_importance->add(chronomail::Q($this->plugin->gettext($io_n)), $io_v);
        }
        ...
        $out .= '<span class="label">' .chronomail::Q($this->plugin->gettext('notifytarget')) . '</span><br>';
        ...
        $out .= '</div>';
        $out .= '<br><span class="label">'. chronomail::Q($this->plugin->gettext('notifymessage')) .'</span><br>';
        $out .= html::tag('textarea', array(
            ...
            ), chronomail::Q($action['message'], 'strict', false));
        if (in_array('enotify', $this->exts)) {
            $out .= '<br><span class="label">' .chronomail::Q($this->plugin->gettext('notifyfrom')) . '</span><br>';
            $out .= html::tag('input', array(
                    'type'  => 'text',
        ...
        $out .= '<br><span class="label">' . chronomail::Q($this->plugin->gettext('notifyimportance')) . '</span><br>';
        $out .= $select_importance->show($action['importance'] ? (int) $action['importance'] : 2);
        $out .= '<div id="action_notifyoption_div' . $id  . '">'
            .'<span class="label">' . chronomail::Q($this->plugin->gettext('notifyoptions')) . '</span><br>'
        ...
        if (in_array('editheader', $this->exts)) {
            ...
            $out .= '<label class="label" for="action_addheader_name' . $id .'">' .chronomail::Q($this->plugin->gettext('headername')) . '</label><br>';
            $out .= html::tag('input', array(
                    'type'  => 'text',
            ...
            $out .= '<br><label class="label" for="action_addheader_value' . $id .'">'. chronomail::Q($this->plugin->gettext('headervalue')) .'</label><br>';
            $out .= html::tag('input', array(
                    'type'  => 'text',
            ...
            $out .= '<br><label class="label" for="action_addheader_pos' . $id .'">'. chronomail::Q($this->plugin->gettext('headerpos')) .'</label><br>';
            ...
            $out .= '<label class="label" for="action_delheader_name' . $id .'">' .chronomail::Q($this->plugin->gettext('headername')) . '</label><br>';
            $out .= html::tag('input', array(
                    'type'  => 'text',
            ...
            $out .= '<br><label class="label" for="action_delheader_value' . $id .'">'. chronomail::Q($this->plugin->gettext('headerpatterns')) .'</label><br>';
            ...
            $out .= html::span('label input-group-prepend', html::label(array(
                    'class' => 'input-group-text', 'for' => 'action_delheader_op'.$id), chronomail::Q($this->plugin->gettext('headermatchtype'))));
            $out .= $this->match_type_selector('action_delheader_op', $id, $action['match-type'], null, 'basic');
            $out .= '</div>';
            $out .= '<div class="adv input-group">';
            $out .= html::span('label input-group-prepend', html::label(array(
                    'class' => 'input-group-text', 'for' => 'action_delheader_comp_op'.$id), chronomail::Q($this->plugin->gettext('comparator'))));
            $out .= $this->comparator_selector($action['comparator'], 'action_delheader_comp', $id);
            $out .= '</div>';
            $out .= '<br><label class="label" for="action_delheader_index' . $id .'">'. chronomail::Q($this->plugin->gettext('headeroccurrence')) .'</label><br>';
            $out .= '<div class="input-group">';
            $out .= html::tag('input', array(
                    'type'  => 'text',
        ...
        // add/del buttons
        $add_label = chronomail::Q($this->plugin->gettext('add'));
        $del_label = chronomail::Q($this->plugin->gettext('del'));
        $out .= '<td class="rowbuttons">';
        $out .= sprintf('<a href="#" id="actionadd%s" title="%s" onclick="chronomail.managesieve_actionadd(%s)" class="button create add">'
            . '<span class="inner">%s</span></a>', $id, $add_label, $id, $add_label);
        $out .= sprintf('<a href="#" id="actiondel%s" title="%s" onclick="chronomail.managesieve_actiondel(%s)" class="button delete del%s">'
    ...
    protected function print_tips()
    {
        ...
        $script = chronomail_output::JS_OBJECT_NAME.'.managesieve_tip_register('.json_encode($this->tips).');';
    ...
    {
        $value = (array) $value;
        $value = array_map(array('chronomail', 'Q'), $value);
        $value = implode("\n", $value);
    ...
    protected function mod_mailbox($mailbox, $mode = 'out')
    {
        ...
        if ($mode == 'out') {
            $mailbox = chronomail_charset::convert($mailbox, $mbox_encoding, 'UTF7-IMAP');
            if ($replace_delimiter && $replace_delimiter != $delimiter)
                $mailbox = str_replace($replace_delimiter, $delimiter, $mailbox);
        }
        else {
            $mailbox = chronomail_charset::convert($mailbox, 'UTF7-IMAP', $mbox_encoding);
            if ($replace_delimiter && $replace_delimiter != $delimiter)
    ...
    public function list_scripts()
    {
        ...
        if (in_array('list_sets', $this->disabled_actions)) {
            $script_name = $this->rc->config->get('managesieve_script_name', 'chronomail');
    ...
    protected function match_type_selector($name, $id, $test, $rule = null, $mode = 'all')
    {
        ...
        $select_op->add(chronomail::Q($this->plugin->gettext('filtercontains')), 'contains');
        $select_op->add(chronomail::Q($this->plugin->gettext('filternotcontains')), 'notcontains');
        $select_op->add(chronomail::Q($this->plugin->gettext('filteris')), 'is');
        $select_op->add(chronomail::Q($this->plugin->gettext('filterisnot')), 'notis');
        if ($mode == 'all') {
            $select_op->add(chronomail::Q($this->plugin->gettext('filterexists')), 'exists');
            $select_op->add(chronomail::Q($this->plugin->gettext('filternotexists')), 'notexists');
        }
        $select_op->add(chronomail::Q($this->plugin->gettext('filtermatches')), 'matches');
        $select_op->add(chronomail::Q($this->plugin->gettext('filternotmatches')), 'notmatches');
        if (in_array('regex', $this->exts)) {
            $select_op->add(chronomail::Q($this->plugin->gettext('filterregex')), 'regex');
            $select_op->add(chronomail::Q($this->plugin->gettext('filternotregex')), 'notregex');
        }
        if ($mode == 'all' && in_array('relational', $this->exts)) {
            $select_op->add(chronomail::Q($this->plugin->gettext('countisgreaterthan')), 'count-gt');
            $select_op->add(chronomail::Q($this->plugin->gettext('countisgreaterthanequal')), 'count-ge');
            $select_op->add(chronomail::Q($this->plugin->gettext('countislessthan')), 'count-lt');
            $select_op->add(chronomail::Q($this->plugin->gettext('countislessthanequal')), 'count-le');
            $select_op->add(chronomail::Q($this->plugin->gettext('countequals')), 'count-eq');
            $select_op->add(chronomail::Q($this->plugin->gettext('countnotequals')), 'count-ne');
            $select_op->add(chronomail::Q($this->plugin->gettext('valueisgreaterthan')), 'value-gt');
            $select_op->add(chronomail::Q($this->plugin->gettext('valueisgreaterthanequal')), 'value-ge');
            $select_op->add(chronomail::Q($this->plugin->gettext('valueislessthan')), 'value-lt');
            $select_op->add(chronomail::Q($this->plugin->gettext('valueislessthanequal')), 'value-le');
            $select_op->add(chronomail::Q($this->plugin->gettext('valueequals')), 'value-eq');
            $select_op->add(chronomail::Q($this->plugin->gettext('valuenotequals')), 'value-ne');
        }
    ...
    protected function comparator_selector($comparator, $name, $id)
    {
        ...
        $select_comp->add(chronomail::Q($this->plugin->gettext('default')), '');
        $select_comp->add(chronomail::Q($this->plugin->gettext('octet')), 'i;octet');
        $select_comp->add(chronomail::Q($this->plugin->gettext('asciicasemap')), 'i;ascii-casemap');
        if (in_array('comparator-i;ascii-numeric', $this->exts)) {
            $select_comp->add(chronomail::Q($this->plugin->gettext('asciinumeric')), 'i;ascii-numeric');
        }
