...
{
  return chronomail_event.is_keyboard(e) ? true : chronomail_event.cancel(e);
},
...
/**
 * chronomail objects event interface
 */
function chronomail_event_engine()
{
  this._events = {};
};

chronomail_event_engine.prototype = {
...
};  // end chronomail_event_engine.prototype


// check if input is a valid email address
// By Cal Henderson <cal@iamcal.com>
// http://code.iamcal.com/php/rfc822/
function chronomail_check_email(input, inline, count, strict)
{
  if (!input)
...
function chronomail_clone_object(obj)
{
  var out = {};

  for (var key in obj) {
    if (obj[key] && typeof obj[key] === 'object')
      out[key] = chronomail_clone_object(obj[key]);
    else
      out[key] = obj[key];
  }
...
function chronomail_find_object(id, d)
{
  var n, f, obj, e;
  ...
  if (!obj && d.layers) {
    ...
    for (n=0; !obj && n<d.layers.length; n++)
      obj = chronomail_find_object(id, d.layers[n].document);
  }
...
function chronomail_mouse_is_over(ev, obj)
{
  var mouse = chronomail_event.get_mouse_pos(ev),
    pos = $(obj).offset();
...
// deprecated aliases, to be removed, use chronomail.set_cookie/chronomail.get_cookie
chronomail_browser.prototype.set_cookie = setCookie;
chronomail_browser.prototype.get_cookie = getCookie;

var bw = new chronomail_browser();
bw.set_html_class();
...
var chronomail_parse_query = function(query)
{
  if (!query)
    return {};
