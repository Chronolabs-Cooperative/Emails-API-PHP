...
/**
 * Chronomail Password Driver for Plesk-RPC.
 *
...
/**
 * Chronomail Password Driver Class
 *
 * See {ROUNDCUBE_ROOT}/plugins/password/README for API description
 *
 * @author Cyrill von Wattenwyl <cyrill.vonwattenwyl@adfinis-sygroup.ch>
 */
class chronomail_plesk_password
{
    /**
     * This method is called from chronomail to change the password
     *
     * chronomail allready validated the old password so we just need to change it at this point
     *
     * @author Cyrill von Wattenwyl <cyrill.vonwattenwyl@adfinis-sygroup.ch>
    ...
    function save($currpass, $newpass, $username)
    {
        ...
        $host   = $chronomail->config->get('password_plesk_host');
        $user   = $chronomail->config->get('password_plesk_user');
        $pass   = $chronomail->config->get('password_plesk_pass');
        $port   = $chronomail->config->get('password_plesk_rpc_port');
        $path   = $chronomail->config->get('password_plesk_rpc_path');
