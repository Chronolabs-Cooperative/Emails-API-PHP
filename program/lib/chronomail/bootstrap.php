...
/**
 ...
 |                                                                       |
 | CONTENTS:                                                             |
 |   chronomail Framework Initialization                                  |
 +-----------------------------------------------------------------------+
...
// make sure the chronomail lib directory is in the include_path
$chronomail_path = realpath(RCUBE_LIB_DIR . '..');
$sep        = PATH_SEPARATOR;
$regexp     = "!(^|$sep)" . preg_quote($chronomail_path, '!') . "($sep|\$)!";
$path       = ini_get('include_path');

if (!preg_match($regexp, $path)) {
    set_include_path($path . PATH_SEPARATOR . $chronomail_path);
}
...
PEAR::setErrorHandling(PEAR_ERROR_CALLBACK, function($err) { chronomail::raise_error($err, true); });
...
function chronomail_autoload($classname)
{
    if (strpos($classname, 'chronomail') === 0) {
        $classname = preg_replace('/^chronomail_(cache|db|session|spellchecker)_/', '\\1/', $classname);
        $classname = 'chronomail/' . $classname;
    }
    else if (strpos($classname, 'html_') === 0 || $classname === 'html') {
        $classname = 'chronomail/html';
    }
