 ...
 */
abstract class chronomail_plugin
{
    /**
     ...
     * Default constructor.
     *
     * @param chronomail_plugin_api $api Plugin API
     */
    ...
    public function load_config($fname = 'config.inc.php')
    {
        ...
        if (($is_local = is_file($fpath)) && !$chronomail->config->load_from_file($fpath)) {
            ...
            return $chronomail->config->load_from_file($this->ID . '.inc.php');
        }
    ...
    {
        $chronomail = chronomail::get_instance();
        $texts = $chronomail->read_localization(realpath(slashify($this->home) . $dir));
        ...
        if (!empty($texts)) {
            ...
            $chronomail->load_language($_SESSION['language'], $add);

            // add labels to client
            if ($add2client && method_exists($chronomail->output, 'add_label')) {
                ...
                $chronomail->output->add_label($js_labels);
            }
        }
    }
    ...
    public function add_label()
    {
        ...
        if (method_exists($chronomail->output, 'add_label')) {
            $args = func_get_args();
            ...
            $chronomail->output->add_label($args);
        }
    }

    /**
     * Wrapper for chronomail::gettext() adding the plugin ID as domain
     *
     * @param string $p Message identifier
     *
     * @return string Localized text
     * @see chronomail::gettext()
     */
    public function gettext($p)
    {
        return chronomail::get_instance()->gettext($p, $this->ID);
    }
     ...
     *
     * When parsing a template for display, tags like <chronomail:object name="plugin.myobject" />
     ...
     *
     * @see chronomail_remplate::button()
     */
    ...
    {
        $chronomail = chronomail::get_instance();
        $skins = array_keys((array)$chronomail->output->skins);

        if (empty($skins)) {
            $skins = (array) $chronomail->config->get('skin');
        }
