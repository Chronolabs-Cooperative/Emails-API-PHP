<?php
/**
 * Communigate driver for the Password Plugin for Chronomail 
 *
...
class chronomail_ximss_password
{
    function save($pass, $newpass, $username)
    {
        $chronomail = chronomail::get_instance();

        $host = $chronomail->config->get('password_ximss_host');
        $port = $chronomail->config->get('password_ximss_port');
