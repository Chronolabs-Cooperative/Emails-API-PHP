 ...
 * @subpackage Storage
 */
class chronomail_message_header
{
    /**
    ...
    // map header to chronomail_message_header object property
        ...
        if ($decode) {
            if (is_array($value)) {
                foreach ($value as $key => $val) {
                    $val         = chronomail_mime::decode_header($val, $this->charset);
                    $value[$key] = chronomail_charset::clean($val);
                }
            }
            else {
                $value = chronomail_mime::decode_header($value, $this->charset);
     ...
     * @return object chronomail_message_header instance filled with headers values
     */
    public static function from_array($arr)
    {
        $obj = new chronomail_message_header;
        foreach ($arr as $k => $v)
            $obj->set($k, $v);
...
/**
 * Class for sorting an array of chronomail_message_header objects in a predetermined order.
 *
 * @package    Framework
 * @subpackage Storage
 * @author  Aleksander Machniak <alec@alec.pl>
 */
class chronomail_message_header_sorter
{
    private $uids = array();
     ...
     *
     * @param array $headers Array of chronomail_message_header objects indexed by UID
     */
