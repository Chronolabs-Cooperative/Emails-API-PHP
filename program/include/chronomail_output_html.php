 ...
 * @subpackage View
 */
class chronomail_output_html extends chronomail_output
{
    public $type = 'html';
...
EOF;
        ...
        $this->add_script('var '.self::JS_OBJECT_NAME.' = new chronomail_webmail();', 'head_top');
    ...
    public function set_assets_path($path, $fs_dir = null)
    {
        ...
        if (!preg_match('|^https?://|', $path) && $path[0] != '/') {
            ...
            if ($len = intval($this->config->get('use_secure_urls'))) {
                $_base  = explode('/', $base);
                $last   = count($_base) - 1;
                $length = $len > 1 ? $len : 16; // as in chronomail::get_secure_url_token()
    ...
    public function set_skin($skin)
    {
        if (!$this->check_skin($skin)) {
            $skin = chronomail_config::DEFAULT_SKIN;
        }
    ...
    public function show_message($message, $type='notice', $vars=null, $override=true, $timeout=0)
    {
        if ($override || !$this->message) {
            if ($this->app->text_exists($message)) {
                if (!empty($vars))
                    $vars = array_map(array('chronomail','Q'), $vars);
     ...
     * @param bool  $secure Redirect to secure location (see chronomail::url())
     */
    ...
    public function send($templ = null, $exit = true)
    {
        if ($templ != 'iframe') {
            ...
            if ($exit != 'recur' && $this->app->plugins->is_processing('render_page')) {
                chronomail::raise_error(array('code' => 505, 'type' => 'php',
    ...
    public function write($template = '')
    {
        ...
        // script files and skip chronomail_webmail initialization (#1489792)
     ...
     *
     * @return string
     * @uses   chronomail_output_html::parse_xml()
     * @since  0.1-rc1
     */
    ...
    {
        $matches = preg_split('/<chronomail:(if|elseif|else|endif)\s+([^>]+)>\n?/is', $input, 2, PREG_SPLIT_DELIM_CAPTURE);
        if ($matches && count($matches) == 4) {
            ...
            if (isset($attrib['condition'])) {
                ...
                $submatches = preg_split('/<chronomail:(elseif|else|endif)\s+([^>]+)>\n?/is', $matches[3], 2, PREG_SPLIT_DELIM_CAPTURE);

                if ($condmet) {
                    $result = $submatches[0];
                    if ($submatches[1] != 'endif') {
                        $result .= preg_replace('/.*<chronomail:endif\s+[^>]+>\n?/Uis', '', $submatches[3], 1);
                    }
                    else {
                        $result .= $submatches[3];
                    }
                    ...
                    $result = "<chronomail:$submatches[1] $submatches[2]>" . $submatches[3];
                }
    ...
    protected function eval_expression($expression)
    {
        $expression = preg_replace(
            ...
            array(
                "\$_SESSION['\\1']",
                "\$this->app->config->get('\\1',chronomail_utils::get_boolean('\\3'))",
                "\$this->env['\\1']",
                "chronomail_utils::get_input_value('\\1', chronomail_utils::INPUT_GPC)",
                "\$_COOKIE['\\1']",
                "\$this->browser->{'\\1'}",
                "'{$this->template_name}'",
            ),
    ...
    protected function parse_variable($type, $name)
    {
        ...
        switch ($type) {
            ...
            case 'config':
                ...
                break;
            case 'request':
                $value = chronomail_utils::get_input_value($name, chronomail_utils::INPUT_GPC);
                break;
            case 'session':
                $value = $_SESSION[$name];
                break;
    ...
    {
        return preg_replace_callback('/<chronomail:([-_a-z]+)\s+((?:[^>]|\\\\>)+)(?<!\\\\)>/Ui', array($this, 'xml_command'), $input);
    }
        ...
        switch ($command) {
            ...
            case 'label':
                ...
                if ($attrib['name'] || $attrib['command']) {
                    ...
                    $quoting = !empty($attrib['quoting']) ? strtolower($attrib['quoting']) : (chronomail_utils::get_boolean((string)$attrib['html']) ? 'no' : '');
            ...
            case 'object':
                ...
                if (is_callable($handler)) {
                    ...
                    $name = $this->config->get('product_name', 'chronomail Webmail');
    ...
    protected function prepare_object_attribs(&$attribs)
    {
        ...
        array_walk($attribs, function(&$value, $key, $chronomail) {
            if (strpos($key, 'data-label-') === 0) {
                $value = $chronomail->gettext($value);
            }
        }, $this->app);
    }
    ...
    public function button($attrib)
    {
        ...
        if ($attrib['command']) {
            ...
            if (in_array($attrib['command'], chronomail::$main_tasks)) {
            ...
            else if ($attrib['task'] && in_array($attrib['task'], chronomail::$main_tasks)) {
    ...
    protected function _write($output = '')
    {
        ...
        if (!$hook['abort']) {
            if ($this->charset != RCUBE_CHARSET) {
                echo chronomail_charset::convert($hook['content'], RCUBE_CHARSET, $this->charset);
            }
            else {
                echo $hook['content'];
            }
    ...
    public function current_username($attrib)
    {
        ...
        return chronomail_utils::idn_to_utf8($username);
    }
    ...
    protected function login_form($attrib)
    {
        ...
        // save original url
        $url = chronomail_utils::get_input_value('_url', chronomail_utils::INPUT_POST);
        ...
        $table->add('input', $input_user->show(chronomail_utils::get_input_value('_user', chronomail_utils::INPUT_GPC)));
        ...
        if (is_object($input_host) && !$hide_host) {
            ...
            $table->add('input', $input_host->show(chronomail_utils::get_input_value('_host', chronomail_utils::INPUT_GPC)));
        }
        ...
        if (chronomail_utils::get_boolean($attrib['submit'])) {
    ...
    public function search_form($attrib)
    {
        ...
        $label   = html::label(array('for' => $attrib['id'], 'class' => 'voice'), chronomail::Q($this->app->gettext('arialabelsearchterms')));
        ...
        if (!empty($attrib['wrapper'])) {
            $header = html::tag($attrib['ariatag'] ?: 'h2', array(
                    ...
                    'class' => 'voice'
                ), chronomail::Q($this->app->gettext('arialabel' . $attrib['label'], $attrib['label-domain'])));
    ...
    public function charset_selector($attrib)
    {
        ...
        if ($post = chronomail_utils::get_input_value('_charset', chronomail_utils::INPUT_POST)) {
            $set = $post;
        }
