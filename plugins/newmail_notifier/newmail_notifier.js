...
if (window.chronomail && chronomail.env.task == 'mail') {
    chronomail.addEventListener('plugin.newmail_notifier', newmail_notifier_run)
        ...
        .addEventListener('init', function() {
            ...
            if (chronomail.message_list)
                chronomail.message_list.addEventListener('select', newmail_notifier_stop);
        });
}
...
function newmail_notifier_run(prop)
{
    ...
    if (prop.desktop)
        newmail_notifier_desktop(chronomail.get_label('body', 'newmail_notifier'));
}
...
function newmail_notifier_stop(prop)
{
    ...
    if (chronomail.env.favicon_href && chronomail.env.favicon_changed && (!prop || prop.action != 'check-recent')) {
        $('<link rel="shortcut icon" href="'+chronomail.env.favicon_href+'"/>').replaceAll('link[rel="shortcut icon"]');
        chronomail.env.favicon_changed = 0;
    }
...
{
    var w = chronomail.is_framed() ? window.parent : window,
        path = chronomail.assets_path('plugins/newmail_notifier');
    ...
    if (!chronomail.env.favicon_href)
        chronomail.env.favicon_href = oldlink.attr('href');

    chronomail.env.favicon_changed = 1;
    link.replaceAll(oldlink);

    // Add IE icon overlay if we're pinned to Taskbar
    try {
        if (window.external.msIsSiteMode()) {
            window.external.msSiteModeSetIconOverlay(path + '/overlay.ico', chronomail.get_label('title', 'newmail_notifier'));
        }
    } catch(e) {}
}
...
{
    var elem, src = chronomail.assets_path('plugins/newmail_notifier/sound'),
...
{
    var timeout = chronomail.env.newmail_notifier_timeout || 10,
        icon = chronomail.assets_path('plugins/newmail_notifier/mail.png'),
        success_callback = function() {
            var popup = new window.Notification(chronomail.get_label('title', 'newmail_notifier'), {
                dir: "auto",
                lang: "",
                body: body,
...
{
    var status = newmail_notifier_desktop(chronomail.get_label('testbody', 'newmail_notifier'), function() {
        chronomail.display_message(chronomail.get_label('desktopdisabled', 'newmail_notifier'), 'error');
    });

    if (!status) {
        chronomail.display_message(chronomail.get_label('desktopunsupported', 'newmail_notifier'), 'error');
    }
}
