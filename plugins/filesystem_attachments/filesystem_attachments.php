 ...
 * Chronomail core will trust the returned path if status=true.
 *
 * @license GNU GPLv3+
 * @author Ziba Scott <ziba@umich.edu>
 * @author Thomas Bruederli <chronomail@gmail.com>
 */
class filesystem_attachments extends chronomail_plugin
{
    public $task = '?(?!login).*';
    ...
    function upload($args)
    {
        ...
        $tmpfname = chronomail_utils::temp_filename('attmnt');
    ...
    function save($args)
    {
        ...
        if (!$args['path']) {
            $tmp_path = chronomail_utils::temp_filename('attmnt');
    ...
    function file_id()
    {
        $userid = chronomail::get_instance()->user->ID;
     ...
     * We allow only files in Chronomail temp dir
     */
    protected function verify_path($path)
    {
        ...
        $temp_dir  = $chronomail->config->get('temp_dir');
        ...
        if ($temp_dir !== $file_path) {
            ...
            chronomail::raise_error(array(
                    ...
                    'message' => sprintf("%s can't read %s (not in temp_dir)",
                        $chronomail->get_user_name(), substr($path, 0, 512))
                ), true, false);
