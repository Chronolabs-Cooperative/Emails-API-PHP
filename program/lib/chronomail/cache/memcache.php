 ...
 * @subpackage Cache
 */
class chronomail_cache_memcache extends chronomail_cache
{
    /**
    ...
    public function __construct($userid, $prefix = '', $ttl = 0, $packed = true)
    {
        ...
        $this->type  = 'memcache';
        $this->debug = chronomail::get_instance()->config->get('memcache_debug');
    ...
    public static function engine()
    {
        ...
        $pconnect       = $chronomail->config->get('memcache_pconnect', true);
        $timeout        = $chronomail->config->get('memcache_timeout', 1);
        $retry_interval = $chronomail->config->get('memcache_retry_interval', 15);
        $seen           = array();
        $available      = 0;
        ...
        foreach ((array) $chronomail->config->get('memcache_hosts') as $host) {
