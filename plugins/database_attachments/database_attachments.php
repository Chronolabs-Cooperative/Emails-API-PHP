 ...
 * is its compatibility with round-robin dns multi-server chronomail
 * installations.
 *
...
class database_attachments extends filesystem_attachments
{
    ...
    protected function get_cache()
    {
        if (!$this->cache) {
            ...
            $ttl    = $chronomail->config->get('database_attachments_cache_ttl', $ttl);
            $type   = $chronomail->config->get('database_attachments_cache', 'db');
            $prefix = self::PREFIX;
            ...
            $this->cache = $chronomail->get_cache($prefix, $type, $ttl, false);
        }
