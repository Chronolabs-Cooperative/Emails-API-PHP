...
// get arguments
$args = chronomail_utils::get_opt(array('h' => 'host', 'a' => 'age', 'd' => 'dry-run:bool'));
...
if (empty($args['host'])) {
    $hosts = $chronomail->config->get('default_host', '');
    if (is_string($hosts)) {
        $args['host'] = $hosts;
    }
...
$user = chronomail_user::query($username, $args['host']);
...
$plugin = $chronomail->plugins->exec_hook('user_delete_prepare', array('user' => $user, 'username' => $username, 'host' => $args['host']));

// let plugins cleanup their own user-related data
if (!$plugin['abort']) {
    $transaction = $db->startTransaction();
    $plugin = $chronomail->plugins->exec_hook('user_delete', $plugin);
}
...
if ($db->is_error()) {
    $chronomail->plugins->exec_hook('user_delete_rollback', $plugin);
    _die("DB error occurred: " . $db->is_error());
}
else {
    // inform plugins about executed user deletion
    $plugin = $chronomail->plugins->exec_hook('user_delete_commit', $plugin);
