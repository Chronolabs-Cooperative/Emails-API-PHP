...
/**
 * Password Plugin for Chronomail
 *
 ...
 */
class password extends chronomail_plugin
{
    public $task    = '?(?!logout).*';
    public $noframe = true;
    public $noajax  = true;
    ...
    function password_init()
    {
        ...
        if (chronomail_utils::get_input_value('_first', chronomail_utils::INPUT_GET)) {
    ...
    function password_save()
    {
        ...
        if (($confirm && !isset($_POST['_curpasswd'])) || !isset($_POST['_newpasswd']) || !strlen($_POST['_newpasswd'])) {
            ...
            $curpwd = $confirm ? chronomail_utils::get_input_value('_curpasswd', chronomail_utils::INPUT_POST, true, $charset) : $sespwd;
            $newpwd = chronomail_utils::get_input_value('_newpasswd', chronomail_utils::INPUT_POST, true);
            $conpwd = chronomail_utils::get_input_value('_confpasswd', chronomail_utils::INPUT_POST, true);
            ...
            $orig_pwd = $newpwd;
            $chk_pwd  = chronomail_charset::convert($orig_pwd, $rc_charset, $charset);
            $chk_pwd  = chronomail_charset::convert($chk_pwd, $charset, $rc_charset);

            // We're doing this for consistence with Chronomail core
            $newpwd = chronomail_charset::convert($newpwd, $rc_charset, $charset);
            $conpwd = chronomail_charset::convert($conpwd, $rc_charset, $charset);
            ...
            // try to save the password
                ...
                if ($this->rc->config->get('password_log')) {
                    chronomail::write_log('password', sprintf('Password changed for user %s (ID: %d) from %s',
                        $this->rc->get_user_name(), $this->rc->user->ID, chronomail_utils::remote_ip()));
                }
    ...
    function password_form()
    {
        ...
        if ($this->rc->config->get('password_confirm_current')) {
            ...
            $table->add('title', html::label($field_id, chronomail::Q($this->gettext('curpasswd'))));
        ...
        $table->add('title', html::label($field_id, chronomail::Q($this->gettext('newpasswd'))));
        ...
        $table->add('title', html::label($field_id, chronomail::Q($this->gettext('confpasswd'))));
    ...
    private function _load_driver($type = 'password')
    {
        ...
        if (!$this->drivers[$type]) {
            $class  = "chronomail_{$driver}_password";
    ...
    static function hash_password($password, $method = '', $prefixed = true)
    {
        ...
        if (empty($method) || $method == 'default') {
            $method   = $chronomail->config->get('password_algorithm');
            $prefixed = $chronomail->config->get('password_algorithm_prefix');
        }
        else if ($method == 'crypt') { // deprecated
            if (!($method = $chronomail->config->get('password_crypt_hash'))) {
                $method = 'md5';
            }
        ...
        case 'des':
        case 'des-crypt':
            $crypted = crypt($password, chronomail_utils::random_bytes(2));
            $prefix  = '{CRYPT}';
            break;

        case 'ext_des': // for BC
        case 'ext-des-crypt':
            $crypted = crypt($password, '_' . chronomail_utils::random_bytes(8));
            $prefix  = '{CRYPT}';
            break;

        case 'md5crypt': // for BC
        case 'md5-crypt':
            $crypted = crypt($password, '$1$' . chronomail_utils::random_bytes(9));
            $prefix  = '{CRYPT}';
            break;

        case 'sha256-crypt':
            $rounds = (int) $chronomail->config->get('password_crypt_rounds');
            $prefix = '$5$';
            ...
            $crypted = crypt($password, $prefix . chronomail_utils::random_bytes(16));
            $prefix  = '{CRYPT}';
            break;
        ...
        case 'blowfish-crypt':
            $cost   = (int) $chronomail->config->get('password_blowfish_cost');
            $cost   = $cost < 4 || $cost > 31 ? 12 : $cost;
            $prefix = sprintf('$2a$%02d$', $cost);

            $crypted = crypt($password, $prefix . chronomail_utils::random_bytes(22));
            $prefix  = '{CRYPT}';
            break;
        ...
        case 'ssha':
            $salt = chronomail_utils::random_bytes(8);
        ...
        case 'samba':
            if (function_exists('hash')) {
                $crypted = hash('md4', chronomail_charset::convert($password, CHRONOMAIL_CHARSET, 'UTF-16LE'));
        ...
        case 'ad':
            $crypted = chronomail_charset::convert('"' . $password . '"', CHRONOMAIL_CHARSET, 'UTF-16LE');
            break;
        ...
        case 'dovecot':
            if (!($dovecotpw = $chronomail->config->get('password_dovecotpw'))) {
                $dovecotpw = 'dovecotpw';
            }
            if (!($method = $chronomail->config->get('password_dovecotpw_method'))) {
                $method = 'CRAM-MD5';
            }
            ...
            if (!$prefixed) {
                $prefixed = (bool) $chronomail->config->get('password_dovecotpw_with_method');
            }
        ...
        case 'hash': // deprecated
            ...
            if (!($hash_algo = strtolower($chronomail->config->get('password_hash_algorithm')))) {
                $hash_algo = 'sha1';
            }

            $crypted = hash($hash_algo, $password);

            if ($chronomail->config->get('password_hash_base64')) {
    ...
    static function username($format = null)
    {
        ...
        if (!$format) {
            $format = $chronomail->config->get('password_username_format');
        }
        ...
        return strtr($format, array(
                '%l' => $chronomail->user->get_username('local'),
                '%d' => $chronomail->user->get_username('domain'),
                '%u' => $_SESSION['username'],
        ));
    }
}
