...
/**
 * chronomail Text Editor Widget class
 * @constructor
 */
function chronomail_text_editor(config, id)
{
  var ref = this,
    ...
    conf = {
      ...
      content_css: chronomail.assets_path(config.content_css),
      menubar: false,
      statusbar: false,
  ...
  if (!tinymce.registered_request_token) {
    ...
    tinymce.util.XHR.on('beforeSend', function(e) {
      // secure spellchecker requests with chronomail token
      e.xhr.setRequestHeader('X-chronomail-Request', chronomail.env.request_token);
      // A hacky way of setting spellchecker language (there's no API for this in Tiny)
      if (e.settings && e.settings.data)
        e.settings.data = e.settings.data.replace(/^(method=[a-zA-Z]+&lang=)([^&]+)/, '$1' + chronomail.env.spell_lang);
    });
  }
  ...
  // full-featured editor
    ...
    $.extend(conf, {
      ...
      spellchecker_language: chronomail.env.spell_lang,
  ...
  // add TinyMCE plugins/buttons from chronomail plugin
  ...
  // disable TinyMCE plugins/buttons from chronomail plugin
  ...
  if (window.chronomail_editor_settings)
    $.extend(conf, window.chronomail_editor_settings);
  ...
  conf.setup = function(ed) {
    ...
    ed.on('keypress', function() {
      chronomail.compose_type_activity++;
    });
  ...
  chronomail.triggerEvent('editor-init', {config: conf, ref: ref, id: id});
  ...
  this.init_callback = function(editor)
  {
    ...
    if (chronomail.env.action == 'compose') {
      ...
      var css = {},
        elem = chronomail_find_object('_from'),
        fe = chronomail.env.compose_focus_elem;

      if (chronomail.env.default_font)
        css['font-family'] = chronomail.env.default_font;

      if (chronomail.env.default_font_size)
        css['font-size'] = chronomail.env.default_font_size;
      ...
      if (elem && elem.type == 'select-one') {
        ...
        if (!chronomail.env.identities_initialized)
          chronomail.change_identity(elem);

        // Focus previously focused element
        if (fe && fe.id != this.id && fe.nodeName != 'BODY') {
          window.focus(); // for WebKit (#1486674)
          fe.focus();
          chronomail.env.compose_focus_elem = null;
        }
      }
    }

    chronomail.triggerEvent('editor-load', {config: conf, ref: ref});
  ...
  {
    if (chronomail.env.task == 'mail' && this.editor) {
  ...
  this.toggle = function(ishtml, noconvert)
  {
    var curr, content, result,
      ...
      sig_mark = "\u0002\u0003",
      input = $('#' + this.id),
      signature = chronomail.env.identity ? chronomail.env.signatures[chronomail.env.identity] : null,
    ...
    if (ishtml) {
      ...
      // convert to html
      if (!noconvert) {
        result = chronomail.plain2html(content, init_editor);
      }
      else {
        init_editor(content);
        result = true;
      ...
      var init_plaintext = function(data) {
        ...
        chronomail.set_caret_pos(input.get(0), 0);
      };

      // convert html to text
      if (!noconvert) {
        result = chronomail.html2plain(content, init_plaintext);
      }
      else {
  ...
  {
    return chronomail.env.spell_lang;
  };
  ...
  this.set_language = function(lang)
  {
    ...
    chronomail.env.spell_lang = lang;
  };
  ...
  this.replace = function(input)
  {
    ...
    if (ed) {
      ...
      if ($.type(input) == 'object' && ('html' in input)) {
        ...
        input = chronomail.quote_html(input).replace(/\r?\n/g, '<br/>');
        format = 'text';
      }
    ...
    // replace selection in compose textarea
    else if (ed = chronomail_find_object(this.id)) {
      var selection = chronomail.get_input_selection(ed),
        value = ed.value,
      ...
      chronomail.set_caret_pos(ed, selection.start + input.length);
      ed.focus();
    }
  };
  ...
  this.get_content = function(args)
  {
    ...
    // get selected text from compose textarea
      ...
      if (args.selection) {
        text = chronomail.get_input_selection(ed).text;
      }
  ...
  this.change_signature = function(id, show_sig)
  {
    var position_element, cursor_pos, p = -1,
      ...
      sig = chronomail.env.identity;

    if (!this.editor) { // plain text mode
      // remove the 'old' signature
      if (show_sig && sig && chronomail.env.signatures && chronomail.env.signatures[sig]) {
        sig = chronomail.env.signatures[sig].text;
        sig = sig.replace(/\r\n/g, '\n');

        p = chronomail.env.top_posting ? message.indexOf(sig) : message.lastIndexOf(sig);
        if (p >= 0)
          message = message.substring(0, p) + message.substring(p+sig.length, message.length);
      }

      // add the new signature string
      if (show_sig && chronomail.env.signatures && chronomail.env.signatures[id]) {
        sig = chronomail.env.signatures[id].text;
        sig = sig.replace(/\r\n/g, '\n');
        ...
        // empty message or new-message mode
        else if (!message || !chronomail.env.compose_mode) {
          cursor_pos = message.length;
          message += '\n\n' + sig;
        }
        else if (chronomail.env.top_posting && !chronomail.env.sig_below) {
          // at cursor position
          if (pos = chronomail.get_caret_pos(input_message.get(0))) {
          ...
          cursor_pos = !chronomail.env.top_posting && message.length ? message.length + 1 : 0;
          message += '\n\n' + sig;
        }
      }
      else {
        cursor_pos = chronomail.env.top_posting ? 0 : message.length;
      }

      input_message.val(message);

      // move cursor before the signature
      chronomail.set_caret_pos(input_message.get(0), cursor_pos);
    }
    else if (show_sig && chronomail.env.signatures) {  // html
      ...
      if (!sigElem) {
        ...
        if (chronomail.env.top_posting && !chronomail.env.sig_below
          && chronomail.env.compose_mode && (body.childNodes.length > 1 || $(body).text())
        ) {
          ...
          position_element = chronomail.env.top_posting && chronomail.env.compose_mode ? body.firstChild : $(sigElem).prev();
        }
      }

      sigElem.innerHTML = chronomail.env.signatures[id] ? chronomail.env.signatures[id].html : '';
    }
    else if (!chronomail.env.top_posting) {
  ...
  this.focus = function()
  {
    (this.editor || chronomail_find_object(this.id)).focus();
  };
  ...
  this.file_browser_callback = function(field_name, url, type)
  {
    ...
    this.editor.windowManager.open({
      title: chronomail.get_label('select' + type),
      width: 500,
    ...
    chronomail.env.file_browser_field = field_name;
    chronomail.env.file_browser_type = type;

    dialog = $('#image-selector');

    if (!form.length)
      form = this.file_upload_form(chronomail.gui_objects.uploadform);
    else
      form.find('button,a.button').slice(1).remove(); // need only the first button

    button = dialog.prepend(form).find('button,a.button')
      .text(chronomail.get_label('add' + type))
      .focus();

    // fill images list with available images
    for (i in chronomail.env.attachments) {
      if (elem = ref.file_browser_entry(i, chronomail.env.attachments[i])) {
        list.push(elem);
      }
    }
    ...
    list = $('#image-selector-list').append(list).on('keydown', 'li', function(e) {
        if (e.which == 9) {
          if (chronomail_event.get_modifier(e) == SHIFT_KEY) {
    ...
    button.keydown(function(e) {
      if (e.which == 9) { // Tab
        if (chronomail_event.get_modifier(e) == SHIFT_KEY || !list.find('li').first().focus().length) {
          cancel.focus();
        }
    ...
    cancel.keydown(function(e) {
      if (e.which == 9) {
        if (chronomail_event.get_modifier(e) != SHIFT_KEY || !list.find('li').last().focus().length) {
          button.focus();
        }
    ...
    if (window.FormData) {
      if (!chronomail.env.filedrop) {
        chronomail.env.filedrop = {};
      }
      if (chronomail.gui_objects.filedrop) {
        chronomail.env.old_file_drop = chronomail.gui_objects.filedrop;
      }

      chronomail.gui_objects.filedrop = $('#image-selector');
      chronomail.gui_objects.filedrop.addClass('droptarget')
        .on('dragover dragleave', function(e) {
          e.preventDefault();
          e.stopPropagation();
          $(this)[(e.type == 'dragover' ? 'addClass' : 'removeClass')]('hover');
        })
        .get(0).addEventListener('drop', function(e) { return chronomail.file_dropped(e); }, false);
    }

    // register handler for successful file upload
    if (!chronomail.env['file_dialog_event_' + type]) {
      chronomail.env['file_dialog_event+' + type] = true;
      chronomail.addEventListener('fileuploaded', function(attr) {
        var elem;
  ...
  {
    var input = $('#' + chronomail.env.file_browser_field);
    ...
    if (chronomail.env.old_file_drop)
      chronomail.gui_objects.filedrop = chronomail.env.old_file_drop;
  };
  ...
  this.file_browser_entry = function(file_id, file)
  {
    ...
    if (chronomail.file_upload_id) {
      chronomail.set_busy(false, null, chronomail.file_upload_id);
    }

    var rx, img_src;

    switch (chronomail.env.file_browser_type) {
      case 'image':
        rx = /^image\//i;
        break;
      ...
      case 'media':
        rx = /^video\//i;
        img_src = chronomail.assets_path('program/resources/tinymce/video.png');
        break;
    ...
    if (rx.test(file.mimetype)) {
      var path = chronomail.env.comm_path + '&_from=' + chronomail.env.action,
        action = chronomail.env.compose_id ? '&_id=' + chronomail.env.compose_id + '&_action=display-attachment' : '&_action=upload-display',
  ...
  this.file_upload_form = function(clone_form)
  {
    var hint = clone_form ? $(clone_form).find('.hint').text() : '',
      ...
      file = $('<input>').attr({name: '_file[]', type: 'file', multiple: true, style: 'opacity:0;height:1px;width:1px'})
        .change(function() { chronomail.upload_file(form, 'upload'); }),
      wrapper = $('<div class="upload-form">')
        .append($('<button>').attr({'class': 'btn btn-secondary attach', href: '#', onclick: "chronomail.upload_input('imageuploadform')"}));
    ...
    form.append(file).append($('<input>').attr({type: 'hidden', name: '_token', value: chronomail.env.request_token}));
