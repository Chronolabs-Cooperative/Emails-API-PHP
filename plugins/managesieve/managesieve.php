...
class managesieve extends chronomail_plugin
{
    ...
    {
        $uids = chronomail::get_uids(null, null, $multifolder, chronomail_utils::INPUT_POST);

        // handle fetching email headers for the new filter form
        if (!empty($uids)) {
            $mailbox = key($uids);
            $message = new chronomail_message($uids[$mailbox][0], $mailbox);
    ...
    public function get_engine($type = null)
    {
        if (!$this->engine) {
            ...
            $class_name   = 'chronomail_sieve_' . ($type ?: 'engine');
    ...
    private function parse_headers($headers)
    {
        ...
        if ($headers->subject)
            $result[] = array('Subject', chronomail_mime::decode_header($headers->subject));

        // @TODO: List-Id, others?
        foreach (array('From', 'To') as $h) {
            $hl = strtolower($h);
            if ($headers->$hl) {
                $list = chronomail_mime::decode_address_list($headers->$hl);
