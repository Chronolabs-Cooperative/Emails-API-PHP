 ...
 * @license GNU GPLv3+
 */
class new_user_identity extends chronomail_plugin
{
    public $task = 'login';
        ...
        if ($this->init_ldap($args['host'], $args['user'])) {
            ...
            if (count($results->records) == 1) {
                ...
                if (!$args['user_email'] && strpos($user_email, '@')) {
                    $args['user_email'] = chronomail_utils::idn_to_ascii($user_email);
                }
                ...
                foreach (array_keys($results[0]) as $key) {
                    ...
                    foreach ((array) $results->records[0][$key] as $alias) {
                        if (strpos($alias, '@')) {
                            $args['email_list'][] = chronomail_utils::idn_to_ascii($alias);
                        }
                    }
                }
...
class new_user_identity_ldap_backend extends chronomail_ldap
{
