...
if (window.chronomail) {
    chronomail.addEventListener('init', function() {
        if (chronomail.gui_objects.acltable) {
            chronomail.acl_list_init();
            // enable autocomplete on user input
            if (chronomail.env.acl_users_source) {
                var inst = chronomail.is_framed() ? parent.chronomail : chronomail;
                ...
                inst.set_env({ autocomplete_max:chronomail.env.autocomplete_max, autocomplete_min_length:chronomail.env.autocomplete_min_length });
                inst.add_label('autocompletechars', chronomail.labels.autocompletechars);
                inst.add_label('autocompletemore', chronomail.labels.autocompletemore);
        ...
        chronomail.enable_command('acl-create', 'acl-save', 'acl-cancel', 'acl-mode-switch', true);
        chronomail.enable_command('acl-delete', 'acl-edit', false);

        if (chronomail.env.acl_advanced)
            $('#acl-switch').addClass('selected').find('input').prop('checked', true);
    });
}

// Display new-entry form
chronomail_webmail.prototype.acl_create = function()
{
    this.acl_init_form();
}

// Display ACL edit form
chronomail_webmail.prototype.acl_edit = function()
{
...
// ACL entry delete
chronomail_webmail.prototype.acl_delete = function()
{
...
// Save ACL data
chronomail_webmail.prototype.acl_save = function()
{
...
// Cancel/Hide form
chronomail_webmail.prototype.acl_cancel = function()
{
    this.ksearch_blur();
...
chronomail_webmail.prototype.acl_update = function(o)
{
    // delete old row
    if (o.old)
...
chronomail_webmail.prototype.acl_mode_switch = function(elem)
{
...
chronomail_webmail.prototype.acl_list_init = function()
{
    ...
    this.acl_list = new chronomail_list_widget(this.gui_objects.acltable,
        {multiselect: true, draggable: false, keyboard: true});
    this.acl_list.addEventListener('select', function(o) { chronomail.acl_list_select(o); })
        .addEventListener('dblclick', function(o) { chronomail.acl_list_dblclick(o); })
        .addEventListener('keypress', function(o) { chronomail.acl_list_keypress(o); })
        .init();
}

// ACL table row selection handler
chronomail_webmail.prototype.acl_list_select = function(list)
{
    chronomail.enable_command('acl-delete', list.get_selection().length > 0);
    chronomail.enable_command('acl-edit', list.get_selection().length == 1);
    list.focus();
}

// ACL table double-click handler
chronomail_webmail.prototype.acl_list_dblclick = function(list)
{
    this.acl_edit();
}

// ACL table keypress handler
chronomail_webmail.prototype.acl_list_keypress = function(list)
{
...
// Reloads ACL table
chronomail_webmail.prototype.acl_list_update = function(html)
{
...
chronomail_webmail.prototype.acl_get_usernames = function()
{
...
chronomail_webmail.prototype.acl_remove_row = function(id)
{
...
// Adds ACL table row
chronomail_webmail.prototype.acl_add_row = function(o, sel)
{
    ...
    $('th', row).map(function() {
        ...
        if (cl == 'user')
            ...
            td.addClass(this.className + ' ' + chronomail.acl_class(o.acl, cl)).html('<span/>');
...
chronomail_webmail.prototype.acl_init_form = function(id)
{
    ...
    this.acl_popup = this.show_popup_dialog(
        ...
        buttons,
        {
            ...
            closeOnEscape: true,
            close: function(e, ui) {
                (me.is_framed() ? parent.chronomail : me).ksearch_hide();
...
chronomail_webmail.prototype.acl_class = function(acl1, acl2)
{
    var i, len, found = 0;
