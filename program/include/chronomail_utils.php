...
class chronomail_utils
{
     ...
     *
     * @return chronomail_db Database instance
     */
    public static function db()
    {
        if (self::$db === null) {
            $rc = chronomail::get_instance();
            $db = chronomail_db::factory($rc->config->get('db_dsnw'));
            ...
            if (!$db->is_connected()) {
                chronomail::raise_error("Failed to connect to database", false, true);
            }
    ...
    public static function db_init($dir)
    {
        ...
        if (!file_exists($file)) {
            chronomail::raise_error("DDL file $file not found", false, true);
        }
        ...
        if ($error) {
            echo "[FAILED]\n";
            chronomail::raise_error($error, false, true);
        }
        else {
            echo "[OK]\n";
        }
    ...
    public static function db_update($dir, $package, $ver = null, $opts = array())
    {
        ...
        if (!file_exists($dir)) {
            if ($opts['errors']) {
                chronomail::raise_error("Specified database schema directory doesn't exist.", false, true);
            }
            return false;
        }
        ...
        if (!file_exists($dir)) {
            if ($opts['errors']) {
                chronomail::raise_error("DDL Upgrade files for " . $db->db_provider . " driver not found.", false, true);
            }
            return false;
        }
        ...
        foreach ($result as $v) {
            ...
            if ($error) {
                ...
                if ($opts['errors']) {
                    chronomail::raise_error("Error in DDL upgrade $v: $error", false, true);
                }
                return false;
            }
            else if (!$opts['quiet']) {
     ...
     */
    public static function db_version($package = 'chronomail')
    {
        $db = self::db();
    ...
    public static function indexcontacts()
    {
        ...
        while ($sql_result && ($sql_arr = $db->fetch_assoc($sql_result))) {
            ...
            $contacts = new chronomail_contacts($db, $sql_arr['user_id']);
    ...
    public static function mod_pref($name, $value, $userid = null, $type = 'string')
    {
        ...
        if ($type == 'bool' || $type == 'boolean') {
            $value = chronomail_utils::get_boolean($value);
        }
        ...
        while ($sql_result && ($sql_arr = $db->fetch_assoc($sql_result))) {
            ...
            $user  = new chronomail_user($sql_arr['user_id'], $sql_arr);
