...
class chronomail_ldap_ppolicy_password
{
    ...
    public function save($currpass, $newpass, $username)
    {
        ...
        $cmd    = $chronomail->config->get('password_ldap_ppolicy_cmd');
        $uri    = $chronomail->config->get('password_ldap_ppolicy_uri');
        $baseDN = $chronomail->config->get('password_ldap_ppolicy_basedn');
        $filter = $chronomail->config->get('password_ldap_ppolicy_search_filter');
        $bindDN = $chronomail->config->get('password_ldap_ppolicy_searchDN');
        $bindPW = $chronomail->config->get('password_ldap_ppolicy_searchPW');
        $cafile = $chronomail->config->get('password_ldap_ppolicy_cafile');

        $log_dir = $chronomail->config->get('log_dir');

        if (empty($log_dir)) {
            $log_dir = CHRONOMAIL_INSTALL_PATH . 'logs';
        }
