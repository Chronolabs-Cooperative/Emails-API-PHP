 ...
 */
class krb_authentication extends chronomail_plugin
{
    ...
    function authenticate($args)
    {
        if (!empty($_SERVER['REMOTE_USER']) && !empty($_SERVER['KRB5CCNAME'])) {
            ...
            $host   = $chronomail->config->get('krb_authentication_host');
    ...
    private function gssapi_context($protocol)
    {
        ...
        $chronomail  = chronomail::get_instance();
        $context = $chronomail->config->get('krb_authentication_context');
        ...
        if (empty($context)) {
            chronomail::raise_error("Empty GSSAPI context ($protocol).", true);
        }
