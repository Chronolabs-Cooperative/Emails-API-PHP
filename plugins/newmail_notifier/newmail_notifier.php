...
class newmail_notifier extends chronomail_plugin
{
    ...
    function prefs_list($args)
    {
        ...
        foreach (array('basic', 'desktop', 'sound') as $type) {
            ...
            if (!in_array($key, $dont_override)) {
                ...
                $args['blocks']['new_message']['options'][$key] = array(
                    'title' => html::label($field_id, chronomail::Q($this->gettext($type))),
                    'content' => $content
                );
            }
        }
        ...
        if (!in_array($key, $dont_override)) {
            ...
            $args['blocks']['new_message']['options'][$key] = array(
                'title'   => html::label($field_id, chronomail::Q($this->gettext('desktoptimeout'))),
    ...
    function prefs_save($args)
    {
        ...
        foreach (array('basic', 'desktop', 'sound') as $type) {
            $key = 'newmail_notifier_' . $type;
            if (!in_array($key, $dont_override)) {
                $args['prefs'][$key] = chronomail_utils::get_input_value('_' . $key, chronomail_utils::INPUT_POST) ? true : false;
            }
        }

        $option = 'newmail_notifier_desktop_timeout';
        if (!in_array($option, $dont_override)) {
            if ($value = (int) chronomail_utils::get_input_value('_' . $option, chronomail_utils::INPUT_POST)) {
