...
class chronomail_tnef_decoder
{
    ...
    protected function _extractMapiAttributes($data, &$result)
    {
        ...
        while ((strlen($data) > 0) && $number--) {
            ...
            case self::MAPI_ATTACH_DATA:
                $this->_getx($value, 16);
                $att = new chronomail_tnef_decoder;
    ...
    protected function convertString($str, $use_codepage = false)
    {
        if ($use_codepage && $this->codepage
            && ($charset = chronomail_charset::$windows_codepages[$this->codepage])
        ) {
            $str = chronomail_charset::convert($str, $charset, RCUBE_CHARSET);
        }
        else if (($pos = strpos($str, "\0")) !== false && $pos != strlen($str)-1) {
            $str = chronomail_charset::convert($str, 'UTF-16LE', RCUBE_CHARSET);
        }
