...
class chronomail_image
{
    ...
    public function resize($size, $filename = null, $browser_compat = false)
    {
        ...
        $convert = $chronomail->config->get('im_convert_path', false);
        $props   = $this->props();
            ...
            if ($scale >= 1 && $p['intype'] == $type) {
                ...
                if (in_array($type, explode(',', $valid_types))) { // Valid type?
                    ...
                    if ($convert) {
                        ...
                        $result = chronomail::exec($convert . ' 2>&1 -flatten -auto-orient -colorspace sRGB -strip'
                    ...
                    // use PHP's Imagick class
                        ...
                        try {
                            ...
                            chronomail::raise_error($e, true, false);
                        }
                    }
                }
            }
    ...
    public function convert($type, $filename = null)
    {
        ...
        if ($convert) {
            ...
            $result = chronomail::exec($convert . ' 2>&1 -colorspace sRGB -strip -flatten -quality 75 {in} {type}:{out}', $p);
    ...
    public static function is_convertable($mimetype = null)
    {
        ...
        return class_exists('Imagick', false) || $chronomail->config->get('im_convert_path');
    }
    ...
    private function identify()
    {
        ...
        // use ImageMagick in command line
        if ($cmd = $chronomail->config->get('im_identify_path')) {
            $args = array('in' => $this->image_file, 'format' => "%m %[fx:w] %[fx:h]");
            $id   = chronomail::exec($cmd. ' 2>/dev/null -format {format} {in}', $args);
    ...
    private function mem_check($props)
    {
        ...
        return chronomail_utils::mem_check($size);
    }
}
