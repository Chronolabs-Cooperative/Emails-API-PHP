...
if (window.chronomail)
  chronomail.addEventListener('init', function() { hide_blockquote(); });

function hide_blockquote()
{
  var limit = chronomail.env.blockquote_limit;
  ...
  $('div.message-part div.pre > blockquote', $('#messagebody')).each(function() {
    ...
    link = $('<span class="blockquote-link"></span>')
      ...
      .text(chronomail.get_label('hide_blockquote.show'))
      .data('parent', div)
      .click(function() {
        var t = $(this), parent = t.data('parent'), visible = parent.is(':visible');

        t.text(chronomail.get_label(visible ? 'hide' : 'show', 'hide_blockquote'))
