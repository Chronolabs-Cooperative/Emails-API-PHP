...
class markasjunk_edit_headers
{
    ...
    private function _edit_headers(&$uids, $spam, $dst_mbox)
    {
        ...
        $args  = $chronomail->config->get($spam ? 'markasjunk_spam_patterns' : 'markasjunk_ham_patterns');
        ...
        $new_uids = array();
        foreach ($uids as $uid) {
            $raw_message = $chronomail->storage->get_raw_body($uid);
            $raw_headers = $chronomail->storage->get_raw_headers($uid);
            ...
            $saved = $chronomail->storage->save_message($dst_mbox, $raw_message);
