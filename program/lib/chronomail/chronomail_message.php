...
class chronomail_message
{
    ...
    function __construct($uid, $folder = null, $is_safe = false)
    {
        ...
        $this->app     = chronomail::get_instance();
        ...
        $this->mime    = new chronomail_mime($this->headers->charset);
    ...
    public static function format_part_body($body, $part, $default_charset = null)
    {
        ...
        if (!$part->charset || strtoupper($part->charset) == 'US-ASCII') {
            ...
            if ($part->ctype_secondary == 'html' && preg_match('/<meta[^>]+charset=([a-z0-9-_]+)/i', $body, $m)) {
                ...
                $part->charset = $chronomail->config->get('default_charset', RCUBE_CHARSET);
            }
        }

        // ..convert charset encoding
        $body = chronomail_charset::convert($body, $part->charset);
     ...
     * @param chronomail_message_part &$part    Reference to the part if found
     *
    ...
    public function first_html_part(&$part = null, $enriched = false)
    {
        if ($this->has_html_part($enriched, $part)) {
            ...
            if ($part->mimetype == 'text/enriched') {
                $body = chronomail_enriched::to_html($body);
            }
    ...
    public function first_text_part(&$part = null, $strict = false)
    {
        ...
        if (!$strict && ($body = $this->first_html_part($part, true))) {
            ...
            $h2t  = new chronomail_html2text($body);
            return $h2t->get_text();
        }
    }
     ...
     *
     * @return chronomail_message_part
     */
     ...
     *
     * @param chronomail_message_part $structure Message structure node
    ...
    private function parse_structure($structure, $recursive = false)
    {
        ...
        if ($structure->mimetype == 'message/rfc822' && $structure->real_mimetype) {
            ...
            if (!isset($structure->headers['subject']) && !isset($structure->headers['from'])) {
                ...
                $structure->headers = chronomail_mime::parse_headers($headers);

                if ($this->context == $structure->mime_id) {
                    $this->headers = chronomail_message_header::from_array($structure->headers);
                }
            }
        }
        else {
     ...
     *
     * @param chronomail_message_part $part Message body structure
     */
     ...
     *
     * @param chronomail_message_part $part Message part to decode
     *
     * @return chronomail_message_part[] List of message parts extracted from TNEF
     */
    function tnef_decode(&$part)
    {
        // @TODO: attachment may be huge, handle body via file
        $body     = $this->get_part_body($part->mime_id);
        $tnef     = new chronomail_tnef_decoder;
        ...
        foreach ($tnef_arr['attachments'] as $pid => $winatt) {
            $tpart = new chronomail_message_part;
     ...
     * @param chronomail_message_part $part Message part to decode
     *
     * @return chronomail_message_part[] List of message parts extracted from the file
     */
    function uu_decode(&$part)
    {
        ...
        while (preg_match($uu_regexp_begin, $part->body, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            // mark body as modified so it will not be cached by chronomail_imap_cache
            $part->body_modified = true;

            // add attachments to the structure
            $uupart = new chronomail_message_part;
            ...
            $ctype = chronomail_mime::file_content_type($uupart->body, $uupart->filename, 'application/octet-stream', true);
    ...
    {
        if ($name == chronomail_charset::clean($name)) {
            return $name;
        }
        ...
        if (empty($charsets)) {
            $chronomail      = chronomail::get_instance();
            $charsets[] = chronomail_charset::detect($name, $chronomail->config->get('default_charset', RCUBE_CHARSET));
        }

        foreach (array_unique($charsets) as $charset) {
            $_name = chronomail_charset::convert($name, $charset);

            if ($_name == chronomail_charset::clean($_name)) {
                if (!$part->charset) {
    ...
    {
        return chronomail_mime::unfold_flowed($text);
    }

    public static function format_flowed($text, $length = 72)
    {
        return chronomail_mime::format_flowed($text, $length);
    }
}
