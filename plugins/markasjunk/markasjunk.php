 ...
 *
 * This program is a Chronomail (https://chronomail.net) plugin.
 ...
 */
class markasjunk extends chronomail_plugin
{
    public $task = 'mail';
    ...
    public function init()
    {
        ...
        // Host exceptions
        $hosts = $this->chronomail->config->get('markasjunk_allowed_hosts');
        if (!empty($hosts) && !in_array($_SESSION['storage_host'], (array) $hosts)) {
            return;
        }

        $this->ham_mbox  = $this->chronomail->config->get('markasjunk_ham_mbox', 'INBOX');
        $this->spam_mbox = $this->chronomail->config->get('markasjunk_spam_mbox', $this->chronomail->config->get('junk_mbox'));
        $toolbar         = $this->chronomail->config->get('markasjunk_toolbar', true);
        $this->_init_flags();

        if ($this->chronomail->action == '' || $this->chronomail->action == 'show') {
            ...
            $this->chronomail->output->set_env('markasjunk_ham_mailbox', $this->ham_mbox);
            $this->chronomail->output->set_env('markasjunk_spam_mailbox', $this->spam_mbox);
            $this->chronomail->output->set_env('markasjunk_move_spam', $this->chronomail->config->get('markasjunk_move_spam', false));
            $this->chronomail->output->set_env('markasjunk_move_ham', $this->chronomail->config->get('markasjunk_move_ham', false));
            $this->chronomail->output->set_env('markasjunk_permanently_remove', $this->chronomail->config->get('markasjunk_permanently_remove', false));
            $this->chronomail->output->set_env('markasjunk_spam_only', $this->chronomail->config->get('markasjunk_spam_only', false));
    ...
    public function mark_message()
    {
        ...
        $is_spam    = $this->chronomail->action == 'plugin.markasjunk.junk' ? true : false;
        $messageset = chronomail::get_uids(null, null, $multifolder, chronomail_utils::INPUT_POST);
        $mbox_name  = chronomail_utils::get_input_value('_mbox', chronomail_utils::INPUT_POST);
        ...
        if ($result) {
            if ($dest_mbox && ($mbox_name !== $dest_mbox || $multifolder)) {
                $this->chronomail->output->command('markasjunk_move', $dest_mbox, $this->_messageset_to_uids($messageset, $multifolder));
            }
            else {
                $this->chronomail->output->command('command', 'list', $mbox_name);
            }

            $this->chronomail->output->command('display_message', $is_spam ? $this->gettext('reportedasjunk') : $this->gettext('reportedasnotjunk'), 'confirmation');
        }

        $this->chronomail->output->send();
    }
    ...
    private function _spam(&$messageset, $dest_mbox = null)
    {
        ...
        foreach ($messageset as $source_mbox => &$uids) {
            ...
            if ($this->chronomail->config->get('markasjunk_read_spam', false)) {
    ...
    private function _ham(&$messageset, $dest_mbox = null)
    {
        ...
        foreach ($messageset as $source_mbox => &$uids) {
            ...
            if ($this->chronomail->config->get('markasjunk_unread_ham', false)) {
    ...
    {
        $driver_name = $this->chronomail->config->get('markasjunk_learning_driver');
    ...
    {
        $configs = $this->chronomail->config->get('markasjunk_host_config');
    ...
    {
        $spam_flag = $this->chronomail->config->get('markasjunk_spam_flag');
        $ham_flag  = $this->chronomail->config->get('markasjunk_ham_flag');
